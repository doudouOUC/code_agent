// The stale variant: a workspace whose status was computed successfully, then
// times out. `startRefresh` does `if (!status) return;`, so the previous cache
// survives — the route should keep answering 200 with the old numbers.
//
// To make staleness undeniable the working tree is dirtied between the two
// calls, so a correct refresh would report different counts.
...
const SHIM_DIR = path.join(ROOT, 'slowgit-mutable');
const SHIM = path.join(SHIM_DIR, 'git');
const DIRT = path.join(WS, 'capacity-probe-untracked.txt');

const passthrough = '#!/bin/sh\nexec /usr/bin/git "$@"\n';
const sleeping = `#!/bin/sh
for a in "$@"; do
...
await fsp.writeFile(SHIM, passthrough);
await fsp.chmod(SHIM, 0o755);
await fsp.rm(DIRT, { force: true });

const fixture = path.join(ROOT, 'fixtures', 'probe-stale');
...
try {
  ...
  const git = `/workspaces/${encodeURIComponent(id)}/git?wait=1`;

  steps.push({ step: '1. fast git, clean tree', ...(await req(git)) });

  await fsp.writeFile(DIRT, 'capacity probe: this file makes the tree dirty\n');
  steps.push({ step: '2. fast git, tree now dirty', ...(await req(git)) });

  await fsp.writeFile(SHIM, sleeping);
  await fsp.chmod(SHIM, 0o755);
  await fsp.rm(DIRT, { force: true });
  steps.push({ step: '3. slow git, tree cleaned again', ...(await req(git)) });
  steps.push({ step: '4. slow git, repeat', ...(await req(git)) });

  await fsp.writeFile(SHIM, passthrough);
  await fsp.chmod(SHIM, 0o755);
  steps.push({ step: '5. fast git restored', ...(await req(git)) });
} catch (e) {
  ...
  await fsp.rm(DIRT, { force: true });
  await fsp.writeFile(
    path.join(ROOT, 'out', 'probe-stale.log'),
...
for (const s of steps) {
  const b = s.body ?? {};
  console.log(
    `${(s.step ?? 'error').padEnd(32)} status=${s.status} ms=${String(s.ms).padStart(5)} ` +
      `untracked=${b.untrackedCount ?? '-'} dirty=${b.filesCount ?? '-'} computedAt=${b.computedAt ?? 'ABSENT'}`,
  );
}
console.log('\nexit=' + child.exitCode + ' modelRequests=' + modelRequests);
console.log('dirt file removed: ' + !(await fsp.stat(DIRT).catch(() => false)));
await fsp.writeFile(
  path.join(ROOT, 'out', 'probe-stale-summary.json'),
  JSON.stringify({ steps, exitCode: child.exitCode, modelRequests }, null, 2) + '\n',
);
console.log('WROTE /root/cap/out/probe-stale-summary.json');
