...
> 本文件已整理 2026-07-20（Asia/Shanghai）的 @doudouOUC 个人 PR。口径为 `QwenLM/qwen-code` 中 author 为 @doudouOUC 且 createdAt 落在对应北京时间日窗口内的 PR；只在窗口内更新、关闭或合入，但创建时间不在窗口内的 PR 不计入新增统计。

**主题**: Plan mode entry boundary、workspace trust hot reload、SDK SSE request cleanup、lazy telemetry SDK / OTLP protocol split、ACP permission cancellation preservation、final tool response budget

**PR 统计**: 6 PRs - 3 merged / 3 open / 0 closed
**当前已合并 PR 代码量**: +1,280 / -138，17 个文件变更
**全量代码量**: +14,040 / -2,202，193 个文件变更
**类型分布**: fix ×4, feat ×1, perf ×1
**范围 (scope)**: core ×2, serve ×1, sdk ×1, telemetry ×1, cli ×1
...
| [#7268](https://github.com/QwenLM/qwen-code/pull/7268) | 🟡 open | @doudouOUC | feat(serve): Hot-reload workspace trust changes | +8902/-1019 | 117 | 07-20 03:16 | — |
...
| [#7276](https://github.com/QwenLM/qwen-code/pull/7276) | 🟡 open | @doudouOUC | perf(telemetry): lazy-load the SDK and split OTLP exporter chains by protocol | +1774/-672 | 18 | 07-20 03:50 | — |
...
| [#7323](https://github.com/QwenLM/qwen-code/pull/7323) | 🟡 open | @doudouOUC | fix(core): Enforce final tool response budgets | +2084/-373 | 41 | 07-20 09:00 | — |
...
| [daemon-serve-mode/](../../feature/daemon-serve-mode/) | #7268 / #7295 | 补 workspace trust hot reload 的 generation/reconciler/v2 status/capability/Web Shell/SDK 行为，以及 ACP permission cancellation 对 session lifecycle 的停止语义。 |
| [sdk.md](../../feature/sdk.md) | #7269 / #7268 | 补 REST SSE transport 的 active request abort/dispose 语义、trust v2 status 和 workspace client 行为。 |
