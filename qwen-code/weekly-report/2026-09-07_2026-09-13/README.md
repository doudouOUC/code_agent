...
**PR 统计**: 21 PRs - 21 merged / 0 open / 0 closed
**当前已合并 PR 代码量**: +32,642 / -1,392，338 个文件变更
**全量代码量**: +32,642 / -1,392，338 个文件变更
...
| [#11614](https://github.com/QwenLM/qwen-code/pull/11614) | ✅ merged | @doudouOUC | feat(cli): add bwrap kernel sandbox backend for Linux | +5238/-39 | 26 | 09-11 03:20 | 09-15 09:06 |
...
| [#11614](https://github.com/QwenLM/qwen-code/pull/11614) | Linux没有容器运行时时，现有sandbox选择无法提供内核级文件系统约束，worktree外置Git元数据又要求精确可写授权。 | 最终新增显式 whole-CLI `bwrap` backend：宿主根只读、规范化最小可写根并核验linked-worktree反向指针；project env不能决定backend/image/网络/代理或扩宽temp/cache可写根，可信Qwen `.env`在子进程中只读，非法来源和探针失败均fail closed，并提供`qwen sandbox`检查/验证入口。 | 已更新 Linux kernel sandbox 最终基线及后续工具级迁移边界。完整实现见 [implementations/pr-11614.md](implementations/pr-11614.md)。 |
...
| [WebUI and transport](../../feature/daemon-serve-mode/11-webui-and-transport.md) | #11207/#11208/#11311/#11322/#11323/#11339/#11398/#11447/#11455(merged) | 登记writer-blocked恢复、连续历史/rail、generic MCP参数预览、导航timer/首帧请求修复、live-state协商、浏览器通知及点击导航最终实现，以及WebShell显式submitted prompt producer。 |
...
| [Linux kernel sandbox](../../feature/linux-kernel-sandbox.md) | #11614(merged) | 更新bwrap最小可写root、project-env隔离、自检真实性与网络/PID/Unix socket边界，并登记后续真实CI与工具级迁移方案。 |
