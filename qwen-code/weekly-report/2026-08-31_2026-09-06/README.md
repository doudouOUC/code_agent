...
**主题**: provider/SDK 错误详情、Channel 命名任务与 worktree 隔离、Mem0 分发、standalone 模型与 WebShell 恢复、session turn 导航协议、relaxed Conversations ownership 设计

**PR 统计**: 13 PRs - 9 merged / 3 open / 1 closed
**当前已合并 PR 代码量**: +3,475 / -705，85 个文件变更
**全量代码量**: +13,866 / -1,152，172 个文件变更
**类型分布**: feat ×5, fix ×6, docs ×2
**范围 (scope)**: serve/daemon ×7, sdk ×4, acp-bridge ×4, web-shell ×3, channels ×2, external-context ×2, core/transcript ×1, core/worktree ×1, release ×1, docs/design ×1, docs/deployment ×1
...
| [#10643](https://github.com/QwenLM/qwen-code/pull/10643) | 🟡 open | @doudouOUC | feat(channels): Add worktree-isolated named tasks | +5070/-283 | 35 | 08-31 14:07 | — |
...
| [#10751](https://github.com/QwenLM/qwen-code/pull/10751) | 🟡 open | @doudouOUC | feat(serve): add session turn navigation protocol | +3345/-61 | 30 | 09-01 23:56 | — |
...
| [#10828](https://github.com/QwenLM/qwen-code/pull/10828) | 🟡 open | @doudouOUC | docs(design): Define relaxed standalone daemon ownership | +729/-0 | 5 | 09-02 12:54 | — |
...
| [#10828](https://github.com/QwenLM/qwen-code/pull/10828) | process-global Conversations runtime owner 会让一个 daemon 阻塞另一个 daemon 的全部 standalone surface，即使它们从不同时操作同一 session；共享 scheduled task 与 deletion journal 也需要明确跨进程权威。 | 当前 docs-only 方案拟让更新后的 daemon 各自挂载共享 Conversations root，把完整性边界收窄到强制 per-session writer lease，并让 controller binding transaction 先于 unbound task 执行、journal lease 冲突按另一 daemon 正在处理跳过。local stale reclaim 绑定 hostname/boot/PID namespace/process identity；WebShell 错误改为 section/session 内联。旧 owner 仅保留迁移检查，部署要求 drain-and-cutover，不承诺 multi-master 或 exactly-once task。 | 已在 daemon lifecycle 总览登记 open 设计及其未落地边界。完整方案观察见 [implementations/pr-10828.md](implementations/pr-10828.md)。 |

## PR 对应 feature 覆盖

| feature 文档 | 本周新增/复核 PR | 文档动作 |
|---|---|---|
| [daemon-serve-mode/](../../feature/daemon-serve-mode/README.md) | #10569 / #10576 / #10643(open) / #10706 / #10719 / #10751(open) / #10824 / #10828(open design) | 刷新 worktree、standalone options，补 turn navigation Phase 1、重复 New task reload 与 relaxed ownership 设计边界。 |
| [daemon-serve-mode/03-session-lifecycle.md](../../feature/daemon-serve-mode/03-session-lifecycle.md) | #10643(open) / #10706 / #10828(open design) | 记录 persisted-v1 restore/AUQ、provisional config 以及 process-global owner 向 per-session lease 收窄的提案。 |
