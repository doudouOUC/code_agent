...
| Channel adapters | [channel-adapters.md](channel-adapters.md) | #5978 / #6031 / #6098 / #6165 / #6182 / #6309 / #6598 / #6635 / #6741(open) | `ChannelAgentBridge` adapter-facing 合约，把 channel adapter 从具体 `AcpBridge` 实现中解耦；daemon-managed channel worker 由 `qwen serve --channel` 托管，并补 prompt turn barrier、session listing、batch load replay、settings reload、multi-workspace worker grouping 和 runtime daemon channel control open 方案。 |
...
| 文件读取 / 大文本范围 / PDF 预算 | [file-reading.md](file-reading.md) | #6404 / #6409 / #6585 / #6846(open) | 大文本范围读取与 PDF 读取预算：超过旧 10MB 文本 guard 时返回有界行范围，大型 PDF 不带 `pages` 时返回 guidance/reference；文本提取失败或单页超预算时可用 bounded PDF vision bridge fallback。 |
| Shell 工具执行语义 | [shell-tool.md](shell-tool.md) | #6864(open) / #6876(open) | 前台 shell timeout 归类为结构化工具错误，保留部分输出给模型；静默命令发不进模型上下文的 liveness heartbeat 给 ACP/stream-json。 |
...
_生成于 2026-05-31；按个人 PR 口径更新于 2026-07-15_
