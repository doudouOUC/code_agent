...
| 主题 | 文档 | 关键 PR | 一句话 |
|---|---|---|---|
| daemon/serve 模式 | [daemon-serve-mode/](daemon-serve-mode/) 📁 | epic #4175 / #3803 / #5977 / #5989 / #5995（~50+ PR） | Mode B：agent core 常驻 HTTP daemon，多客户端经 REST+SSE 并发附着，ACP bridge 解耦；近期补 standalone fast path、source/bundle guard 与 daemon model filtering（README + 8 篇深入 + 09 路线图 + 10 客户端/SDK + 11 WebUI/传输） |
...
| MCP resources / prompts | [mcp-resources-prompts.md](mcp-resources-prompts.md) | #5367 #5544 #5561 #5589 #5635 #5733 #5774 #5781 #5812 #5879 | MCP OAuth token file 首次写入 + 宽松发现 MCP prompts/resources，`ResourceRegistry` + TUI/Web Shell `/mcp` resource browser + `@server:uri`/裸 `@` 补全注入 + model-callable resource read tool + live settings reconcile |
...
| Channel adapters | [channel-adapters.md](channel-adapters.md) | #5202 #5414-#5417 #5919 #5888 #5978 | QQ Bot channel adapter + 稳定性修复；Telegram bot command menu；qwen tag Phase 0 群共享身份；`ChannelAgentBridge` adapter-facing 合约 |
...
| 后台 agent/会话恢复 | [background-agent-resume.md](background-agent-resume.md) | #3739 #4222 #5030 #5556 #5679 #5972 #6015 | 背景 agent paused/resume(transcript-fork) + daemon session load/resume + SDK/stream-json interrupted turn continuation + completed agent revive / transcript TTL + background agent cap 严格解析 + subagent output-token display / non-VP scrollback |
...
| 工具调用 ID 完整性 | [tool-call-id-integrity.md](tool-call-id-integrity.md) | #5107 #5279 #5564 #5573 #5624 #5657 #5934 #5944 | OpenAI-compatible provider 的 `tool_call.id` 规范化、去重执行、跨轮复用 suffix、per-turn cap / identical-call always-on guard、repeated duplicate response 熔断、dangling replay 终止化、截断 write/edit retry-loop backstop，以及 repeated shell inspection stagnation guard |
| 诊断 / creator skills / extension activation | [diagnostic-skills.md](diagnostic-skills.md) | #3404 #4133 #3079 #5826 #5828 #5849 #4943 #6008 | /doctor 代码命令 + /stuck /batch /extension-creator prompt 技能 + session skill usage stats + safe mode + CLI/daemon/WebShell `@ext:name` activation |
| auth/provider | [auth-providers.md](auth-providers.md) | #3212 #3495 #3623 #3624 #4255 #4291 #4305 #5179 #5404 #5478 #5539 #5553 #5632 #5637 #5638 #5654 #5729 #5769 #5778 #5793 #5827 #5835 #5845 #5946 #5962 #5993 | provider 配置/apiKey 保留/auth status 识别 + daemon 设备流(PKCE) + Requesty/custom provider + bare fast-model auth scope + workspace provider defaults + providerProtocol + OpenAI stream idle timeout/env knob + vision fallback model + TLS insecure escape hatch + daemon specialized model filtering |
| Plan Mode | [plan-mode.md](plan-mode.md) | #5311 #5433 #5595 | Plan Mode entry opt-in：普通规划不自动切只读模式；进入后自动 reveal `exit_plan_mode`，手动进入后退出必须确认，并在 tool declaration 同步失败时回滚 |
| 权限系统 | [permission-system.md](permission-system.md) | #3467 #3726 #4335 #5085 #5105 #5196 #5218 #5258 #5260 #5409 #5622 #5743 #5754 #5791 #6009 | 规则解析+畸形规则守卫 + 多客户端权限协调 + ACP 取消停止语义 + broad shell self-kill guard + workspace permissions API + auto 模式破坏性命令硬拦截 + ask_user_question custom input / Stop hook visible-message boundary |

## 昨日 PR 覆盖核对（2026-06-29，全作者）

> 逐 PR 明细见 [../weekly-report/2026-06-29_2026-07-05/README.md](../weekly-report/2026-06-29_2026-07-05/README.md)，feature 判断明细见 [recent-pr-feature-coverage.md](recent-pr-feature-coverage.md)。覆盖口径：北京时间 2026-06-29 创建或合入的全作者 PR，共 49 个：26 merged / 15 open / 8 closed。open PR 只登记观察，不写成已落地实现。

| feature 文档 | 本日新增/复核 PR | 本次文档动作 |
|---|---|---|
| [channel-adapters.md](channel-adapters.md) | #5888 #5978 | 补 qwen tag Phase 0 群共享身份、`/who`、群 `/clear confirm`、输入净化，以及 `ChannelAgentBridge` 抽象和 router/bridge 生命周期修复。 |
| [daemon-serve-mode/](daemon-serve-mode/) | #5977 #5989 #5993 #5995 | 补 standalone `qwen serve` 走 `cli-entry.js`、serve fast-path source/bundle guard，以及 daemon provider surface 过滤专用模型。 |
| [daemon-serve-mode/11-webui-and-transport.md](daemon-serve-mode/11-webui-and-transport.md) | #5992 #5996 #6008 #6025 | 补 raw file diff 优先、follow-up suggestion send/session scoping、WebShell `@ext:name` mention、Esc 两次确认与取消后 queue drain。 |
| [auth-providers.md](auth-providers.md) | #5962 #5993 | 补 TLS insecure escape hatch 的风险边界，以及 daemon-backed `fastOnly` / `voiceOnly` 过滤。 |
| [diagnostic-skills.md](diagnostic-skills.md) | #4943 #6008 | 补 safe mode 排障 baseline，以及 daemon/WebShell extension mention 与 CLI 共用 resolver 的边界。 |
| [background-agent-resume.md](background-agent-resume.md) | #5972 #6015 | 补 subagent output-token 展示口径，以及非 VP multi-agent transcript scrollback 修复。 |
| [permission-system.md](permission-system.md) | #5791 #6009 | 补 `ask_user_question` 多选自定义输入 Enter 行为，以及 Stop hook `last_assistant_message` 过滤 thought parts。 |
...
| [plan-mode.md](plan-mode.md) | #5595 | 新增专题并补手动 Shift+Tab / `/plan` 进入 Plan Mode 后 `exit_plan_mode` 必须显示确认框，只有模型自主进入 AUTO/YOLO Plan Mode 才保留自动 Plan Approval Gate。 |
...
| 完善权限/ACP 取消语义 / shell 边界 | #5085 #5105 #5196 #5218 #5258 #5260 #5409 | [permission-system.md](permission-system.md) |
| 完善 auth/provider | #5179 #5404 #5478 #5539 #5553 | [auth-providers.md](auth-providers.md) |
| 完善 MCP OAuth token storage | #5367 | [mcp-resources-prompts.md](mcp-resources-prompts.md) |
| 完善 tool-call / loop safety | #5279 #5564 #5573 | [tool-call-id-integrity.md](tool-call-id-integrity.md) |
| 完善 Plan Mode UX/safety | #5311 #5433 | [plan-mode.md](plan-mode.md) |
...
| 周报记录，后续观察 | #5122 #5126 #5145 #5146 #5187 #5203 #5220 #5284 #5289 #5401 | 已记录 feature 归属；暂不单开深度专题或属于平台/CI/小功能面；#5561 已在本周合入后移入 MCP 专题 |
...
_生成于 2026-05-31；最后更新 2026-06-30_
