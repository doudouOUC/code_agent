...
| telemetry 可观测性 | [telemetry-observability/](telemetry-observability/) 📁 | epic #3731 / #4384 / #5904（~26 PR） | 层级 span 树 + 上下文传播 + OTLP 路由 + 敏感属性门控 + LLM request phase breakdown + daemon 端到端追踪（README + 7 篇深入） |
...
| loop wakeup | [loop-wakeup.md](loop-wakeup.md) | #5182 #5197 #5808 #5844 #5921 #5927 | prompt-only `/loop` 改成 self-paced wakeup，秒级单次唤醒 + 24h session budget + abort 清理 + monitor/background-task notification guidance + scheduled-task 可见性/搜索 |
...
| Managed memory | [managed-memory.md](managed-memory.md) | #5616 #5814 #5886 | auto-generated skills staged confirmation + `/remember` 与 auto-extract 解耦，避免 QWEN.md 被半自动写入；新增 git-shared team memory tier |
...
| 工具调用 ID 完整性 | [tool-call-id-integrity.md](tool-call-id-integrity.md) | #5107 #5624 #5657 #5934 | OpenAI-compatible provider 的 `tool_call.id` 规范化、去重执行、跨轮复用 suffix、repeated duplicate response 熔断、dangling replay 终止化，以及截断 write/edit retry-loop backstop |
...
| auth/provider | [auth-providers.md](auth-providers.md) | #3212 #3495 #3623 #3624 #4255 #4291 #4305 #5179 #5404 #5478 #5539 #5632 #5637 #5654 #5729 #5769 #5778 #5793 #5827 #5835 #5845 | provider 配置/apiKey 保留/auth status 识别 + daemon 设备流(PKCE) + Requesty/custom provider + providerProtocol + OpenAI stream idle timeout/env knob + vision fallback model + provider install model preservation |
...
## 本周 PR 覆盖核对（2026-06-22 ~ 2026-06-27，全作者）

> 逐 PR 明细见 [recent-pr-feature-coverage.md](recent-pr-feature-coverage.md)。覆盖口径：`created:2026-06-22..2026-06-27` 全作者、全状态 PR；统计以 2026-06-28 查询为准，共 238 个 PR：186 merged / 26 open / 26 closed。2026-06-27 创建 26 个 PR（当前 19 merged / 4 open / 3 closed），当天 `mergedAt` 非空合入 25 个 PR；06-25~27 合入但创建更早的 #5561/#5778/#5886/#5903 等也按本周落地能力纳入。
...
| [daemon-serve-mode/](daemon-serve-mode/) | #5613 #5638 #5741 #5743 #5752 #5753 #5755 #5765 #5784 #5785 #5797 #5825 #5857 #5874 #5892 #5903 #5906 #5938 #5945 | 补 Web Shell branch/fork、workspace defaults、permissions/LSP/status/voice/control APIs、startup fast path、benchmark、single-session status route、ACP `/cd`、Windows PTY tree-kill、settings minimum validation 和 serve compile-cache/version 延迟加载。 |
| [daemon-serve-mode/11-webui-and-transport.md](daemon-serve-mode/11-webui-and-transport.md) | #5650 #5818 #5822 #5864 #5869 #5876 #5893 #5900 #5917 #5931 #5943 | 补 enhanced Markdown tables、active prompt loading、streaming turn command queue、thinking duration、stream-highlight code blocks、中文工具组文案、Web Shell chat polish、host loading phrase customization、session sidebar 与 error boundaries。 |
| [loop-wakeup.md](loop-wakeup.md) | #5808 #5844 #5921 #5927 | 补 abort 清理 pending wakeup、monitor/background-task notification 优先于短轮询的 self-paced loop 指导、footer scheduled task count 与 cron tool-search intent。 |
| [auth-providers.md](auth-providers.md) | #5778 #5793 #5827 #5835 #5845 | 补 `/model --vision` fallback、providerProtocol 映射、stream idle timeout、provider install plan preserve selected model、`QWEN_STREAM_IDLE_TIMEOUT_MS` 部署级 override。 |
| [telemetry-observability/](telemetry-observability/) | #5804 #5904 | 补 sensitive span attribute max length 设置 / env override、新 1 MiB default，以及 LLM request phase breakdown metric。 |
| [context-compression.md](context-compression.md) | #5865 | 补 compression side-query streaming，避免 gateway read timeout。 |
| [tool-call-id-integrity.md](tool-call-id-integrity.md) | #5657 #5934 | 补 repeated duplicate provider response circuit breaker、per-prompt ACP tracking reset，以及截断 write/edit 重试循环 backstop。 |
...
_生成于 2026-05-31；最后更新 2026-06-28_
