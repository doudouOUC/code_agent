# 诊断 / creator skills / extension activation 技术方案

> 适用范围：`/doctor`（代码命令）、`/stuck`（prompt 技能）、`/batch`（prompt 技能）、`/extension-creator`（prompt 技能）、safe mode、CLI/WebShell `@ext:name` extension mention 等自助诊断 / 编排 / creator / extension activation 能力。
> 涉及 PR：#3404（`/doctor`，Close #3018）、#4133（`/stuck`）、#3079（`/batch`，Close #3043）、#5826（skill usage stats）、#5828（bundled extension creator skill）、#5849（CLI `@extension` mention）、#4943（safe mode）、#6008（daemon/WebShell `@extension` mention）。
...
针对这三类诉求，本方案引入两种**截然不同的实现范式**，这也是整个文档的主线对比。#5828 在同一 bundled skill 机制上补了 `/extension-creator`：它不是诊断命令，而是把 extension 创建流程做成一方 prompt 技能，用来指导 agent 复用现有 `qwen extensions new` 模板完成 scaffold、定制和本地验证。#5849 则补齐“创建/安装后如何显式使用 extension”的 CLI 输入面：用户可以在 prompt 中写 `@ext:name`，把 extension 的 skills、MCP servers、agents、context files 注入当前 turn。#6008 把同一 mention 语义补到 daemon/WebShell。#4943 另起一条 deterministic 排障路径：safe mode 不是命令或技能，而是启动级开关，用来绕过用户自定义项并建立干净 baseline。

| 范式 | 代表 | 本质 | 确定性 | 安全边界 |
| --- | --- | --- | --- | --- |
| **代码命令**（code command） | `/doctor` | 编译进 CLI 的 TypeScript 逻辑，直接读 `Config` / 跑系统探针 | 高（同样输入同样输出） | 由代码控制，无注入面 |
| **prompt 技能**（prompt skill） | `/stuck`、`/batch`、`/extension-creator` | 一份带 frontmatter 的 `SKILL.md`，body 作为 prompt 注入给 LLM 执行 | 低（依赖模型遵循指令） | 靠 prompt 内的约束 + `allowedTools` 声明 |
| **启动级排障模式** | `qwen --safe-mode` / `QWEN_CODE_SAFE_MODE` | 在 config / loader / manager 初始化阶段跳过用户自定义 | 高（同样 flag 同样禁用面） | 默认关闭，显式 opt-in |
...
2. `BundledSkillLoader` —— **内置 prompt 技能**（`/stuck`、`/batch`、`/extension-creator`、`/review`、`/loop`、`/qc-helper`、`/new-app`），来自 `packages/cli/src/services/BundledSkillLoader.ts`。
...
### 2.1.1 skill usage stats 观测面（#5826）

#5826 不改变技能注册或执行语义，而是给“技能是否真的被用到”增加 per-session 观测面。统计源放在真实 skill body load 的入口，而不是 slash command 文本解析或 MCP prompt/resource 入口，因此它能回答“模型/用户实际加载了哪个 `SKILL.md`、成功还是失败”，而不是“用户输入里看起来像技能”。

统计暴露两条路径：

- daemon session stats：`GET /session/:id/stats` 的 `skills` block，包含 `totalCalls`、`totalSuccess`、`totalFail`、`byName`。
- CLI `/stats skills`：交互式与非交互输出都读取同一 stats snapshot，展示每个 skill 的调用、成功、失败和最近错误。

兼容性处理：SDK 类型里 `skills` 字段保持 optional，旧 daemon 不返回该 block 时客户端仍可降级；MCP prompt、文件命令 fallback 这类没有加载 skill body 的路径不计入统计，避免误报。

### 2.1.2 extension-creator bundled skill（#5828）

#5828 新增 `packages/core/src/skills/bundled/extension-creator/SKILL.md`，通过既有 bundled skill loader 暴露 `/extension-creator`。它的定位是**guidance-only creator workflow**，不会改变 `qwen extensions new` 的 scaffold 行为，也不新增 extension 运行时能力。

它覆盖的工作流边界：

| 阶段 | skill 指导内容 | 关键边界 |
|---|---|---|
| 需求澄清 | 让 agent 先判断 extension 要提供 context、slash commands、skills、agents、MCP server 还是组合能力。 | 只做方案选择，不直接绕过现有 extension 模板。 |
| scaffold | 复用 `qwen extensions new` 生成模板，再按需求修改 manifest、context、commands、skills、agents、MCP 配置。 | scaffold 仍由 CLI 模板负责，skill 不复制一套生成逻辑。 |
| 本地验证 | 指导本地 link / reload / 调用对应 slash command 或 skill，并根据模板类型跑必要检查。 | PR 未手动验证运行中 TUI 里的 `/extension-creator` 交互；测试覆盖 frontmatter 与 bundled-level discovery。 |

这类 creator skill 和 `/stuck`、`/batch` 的共同点是：它们都是 prompt asset，主路径靠模型遵循 markdown SOP；差异在于 `/extension-creator` 的风险主要是**生成建议质量和模板选择准确性**，而不是 PID/shell 注入或批量改文件的半成功状态。因此它更适合归入 bundled skill 生态，而不是新增 daemon 或 extension runtime 专题。

### 2.1.3 CLI `@extension` mention（#5849）

#5849 把 installed + active extensions 接入 CLI 输入框的 `@` autocomplete。用户输入 `@` 时，extension 与文件、MCP resources 一起进入候选列表；候选项展示 display name、description 和 `Extension` badge。选择后在输入里插入 `@ext:<extension-name>`，提交该 turn 时解析器把对应 extension 的能力注入消息上下文。

注入内容包括：

| 能力 | 作用 |
|---|---|
| skills | 让模型知道该 extension 可提供哪些 skill，以及何时应触发。 |
| MCP servers | 让当前 turn 能显式把 extension 关联的 MCP 能力纳入上下文。 |
| agents | 让 extension-provided agents 对当前任务可见。 |
| context files | 将 extension 声明的 context 文件读入 turn context；单文件有 50KB cap，避免大文件把 prompt 撑爆。 |

这和 `/extension-creator` 形成一前一后的闭环：#5828 负责指导“怎么创建 extension”，#5849 负责在 CLI 中让用户“怎么显式激活 extension”。它不改变 extension 安装/启停/加载的 daemon 管理面；那些仍归 daemon `workspace/extensions` 文档覆盖。

### 2.1.4 safe mode（#4943）

#4943 增加 `qwen --safe-mode` 与 `QWEN_CODE_SAFE_MODE=true`，用于排查启动失败、hook/MCP 崩溃、项目级 context 污染或 extension/skill/subagent 问题。它的目标不是“修复配置”，而是提供一个可复现的最小 baseline：

| 被禁用的自定义面 | safe mode 行为 |
|---|---|
| context files | 跳过 `QWEN.md` / `AGENTS.md` 等用户/项目上下文注入。 |
| hooks | 不加载用户 hooks；复用已有 disable-all hooks 路径。 |
| extensions | 跳过 extension discovery / activation。 |
| custom skills | `/skills` 只保留 bundled/system skills，项目/用户/extension skills 不进库。 |
| custom subagents | agent tool 只看到 builtin agents。 |
| MCP servers / conditional rules | 不加载用户 MCP server 与条件规则，避免坏 server 或规则污染启动。 |

CLI flag 优先于环境变量：`QWEN_CODE_SAFE_MODE=true qwen --no-safe-mode` 会显式关闭 safe mode。UI 会在启动通知和 footer 显示 safe mode 状态。后续可把 safe mode 和 `/doctor` 串联，但 #4943 本身没有新增 `/doctor` 诊断项。

### 2.1.5 daemon/WebShell `@extension` mention（#6008）

#6008 把 #5849 的 CLI extension mention 语义补到 daemon/WebShell：

- WebShell `@` completion 会把 active extensions 放在 workspace file suggestions 前；输入 `@ext:` 后只展示 extension suggestions；
- 选择后插入 canonical `@ext:<extension.name>`，daemon ACP prompt path 解析该引用并把 extension capabilities / allowed context files 注入模型请求；
- 解析、清洗、capability formatting、context file 读取边界复用 shared helper，CLI 和 daemon 路径不再各写一套；
- 用户可见 prompt text 不变；未知、未激活、重复 mention 或越界/不可读 context file 会被跳过；
- daemon extension status 增加可选 description 字段用于 autocomplete detail，保持向后兼容。

这让“创建/安装 extension → 在 CLI/WebShell 中显式激活 extension”的闭环跨客户端一致。Marketplace discovery、extension management UI 和新的 prompt wire shape 不在 #6008 范围。
...
- **prompt 技能**：bundled 技能的 `action` 返回 `{ type: 'submit_prompt', content }`（`BundledSkillLoader.ts:100-104`），即把 `SKILL.md` 的 body 作为 prompt 提交给模型，由模型按指令调用工具。`SKILL.md` 的 frontmatter（`name` / `description` / `argument-hint` / `allowedTools`）由 `parseSkillContent` 解析。#5828 的 `/extension-creator` 也走这条路径，只是 body 内容从“诊断 / 批量编排 SOP”换成“extension 创建 SOP”。
...
flowchart TD
  ...
  subgraph Paradigm2["范式二：prompt 技能 (/stuck /batch /extension-creator)"]
  ...
  CS -.->|/doctor| DC
  CS -.->|/stuck /batch /extension-creator| SK
```
...
| 使用统计 | `/doctor` 只进入命令执行与普通 stats | `GET /session/:id/stats.skills` / `/stats skills` 统计真实 skill body load |
...
| **#5826** `feat(cli): Add skill usage stats` | 诊断观测 | 在 session stats 和 `/stats skills` 中统计真实 skill body load 的总调用、成功、失败、按技能分组与最近错误；SDK 字段 optional 兼容旧 daemon。 |
| **#5828** `feat(core): add bundled extension creator skill` | creator prompt 技能 | 新增 bundled `/extension-creator`，指导 agent 复用 `qwen extensions new` 创建、定制、本地测试 extension，覆盖 context / commands / skills / agents / MCP server wiring / local linking；不改变现有 scaffold 行为。 |
| **#5849** `feat(cli): support @extension mention in input autocomplete` | extension activation UX | CLI `@` autocomplete 展示 active extensions，选择后插入 `@ext:name`，提交 turn 时注入 extension 的 skills、MCP servers、agents、context files；context file 单文件 50KB cap，Desktop/WebUI 不在范围。 |
| **#4943** `feat(cli): add --safe-mode flag to disable all customizations for troubleshooting` | 启动级排障模式 | `--safe-mode` / `QWEN_CODE_SAFE_MODE` 禁用 context files、hooks、extensions、custom skills、MCP servers、subagents 和 conditional rules，建立只含核心内置能力的 baseline。 |
| **#6008** `feat(daemon): support @extension mentions` | daemon/WebShell extension activation | WebShell `@` completion 展示 active extensions；daemon ACP prompt path 解析 `@ext:name` 并注入 extension capabilities/context files，复用 shared helper 和 bounded file handling。 |
...
### #5826 skill usage stats
- **实现模式**：诊断观测，不改变 skill command 或 Skill tool 的执行路径。统计点放在真实 `SKILL.md` body load 成功/失败处，避免把 MCP prompt、文件命令 fallback 或普通 slash 文本误算为 skill 使用。
- **session stats schema**：`ServeSessionStatsStatus.skills` 新增 `totalCalls`、`totalSuccess`、`totalFail`、`byName`。`byName` 以 skill name 聚合调用次数、成功/失败计数和最近错误，便于定位某个 skill 是否频繁加载失败。
...
### #5828 bundled extension-creator skill
- **实现模式**：prompt 技能（`SKILL.md`），新增目录 `packages/core/src/skills/bundled/extension-creator/`。它复用 bundled skill discovery、frontmatter parser、`BundledSkillLoader` 的 `submit_prompt` action；没有新增 TS 命令或 scaffold API。
- **工作流内容**：skill body 指导 agent 先澄清 extension 目标，再选择 `qwen extensions new` 模板，并按 extension context、slash commands、skills、agents、MCP server wiring、本地 link/testing 等维度逐步落地。
- **边界**：它是 guidance-only；模板生成仍由现有 CLI scaffold 负责，PR 不改变 `qwen extensions new` 输出，也不改变 extension runtime 加载/权限语义。
- **验证**：新增 skill 依赖 `bundled-skills.integration.test.ts` 的真实 parser 护栏，并用 `skill-manager.test.ts -t "bundled skills"` 复核 bundled-level discovery；PR 未覆盖运行中 TUI 里的人工 `/extension-creator` 使用。

### #5849/#6008 CLI / daemon @extension mention
- **实现模式**：CLI 输入补全与 turn context 注入。`useAtCompletion` 把 active extensions 纳入 `@` 候选；`extension-mention-ref.ts` 负责解析/引用 `@ext:name`；`atCommandProcessor.ts` 在提交时解析这些 mention 并注入 extension capabilities。
- **用户体验**：输入 `@` 时 extension 与 files / MCP resources 同列展示，带 display name、description、`Extension` badge；按 Tab/Enter 选择后插入 `@ext:<name>`，高亮为 accent color。支持 partial filter、多 extension、与 `@file` / `@server:uri` 混用。
- **注入内容**：当前 turn 得到 extension 的 skills、MCP servers、agents、context files，使模型能优先使用用户显式提到的 extension 能力。context files 按单文件 50KB cap 裁剪风险，避免 extension context 过大。
- **daemon/WebShell follow-up**：#6008 让 WebShell completion 暴露 active extensions，选择后插入 canonical `@ext:name`；daemon ACP prompt path 复用 shared helper 解析 mention 并注入 bounded extension context，同时保持用户可见 prompt 原文不变。
- **边界**：#5849 不覆盖 Desktop/WebUI；#6008 补 daemon/WebShell 但仍不做 marketplace discovery、新 prompt wire shape 或 extension management UI。现有 `@file`、MCP resource mention 行为保持 additive，不改变解析语义。

### #4943 safe mode
- **入口**：`--safe-mode` 和 `QWEN_CODE_SAFE_MODE=true`；CLI flag 可用 `--no-safe-mode` 覆盖 env。
- **禁用面**：context files、hooks、extensions、custom skills、MCP servers、custom subagents、conditional rules。
- **用户反馈**：启动通知和 footer 显示 safe mode 状态，避免用户误以为配置正常加载。
- **边界**：safe mode 只跳过加载，不修复或迁移配置；`/doctor` integration 延后。
...
- **`/extension-creator` 不保证生成质量**：#5828 只提供 creator SOP，不改变 scaffold 代码，也不强制 agent 必须按模板验证 checklist 执行。后续若 extension 创建成为高频路径，应补交互式 smoke 验证或模板级回归测试，而不是只依赖 prompt 文案。
- **`@ext:name` 仍未覆盖所有客户端**：#5849 覆盖 CLI，#6008 覆盖 daemon/WebShell；Desktop 或其它 host 若需要同等显式 activation，需要复用 extension mention resolver 或实现同等 context 注入。
- **safe mode 是排障 baseline，不是修复器**：#4943 只跳过加载用户自定义项；它不会告诉用户哪个 hook/MCP/extension 坏了，也不会自动迁移配置。后续若和 `/doctor` 集成，应把“safe mode 下能启动、正常模式失败”作为诊断线索，而不是让 safe mode 静默吞掉问题。
