...
> 适用范围：`/doctor`（代码命令）、`/stuck`（prompt 技能）、`/batch`（prompt 技能）、`/extension-creator`（prompt 技能）、CLI `@ext:name` extension mention 等自助诊断 / 编排 / creator / extension activation 能力。
> 涉及 PR：#3404（`/doctor`，Close #3018）、#4133（`/stuck`）、#3079（`/batch`，Close #3043）、#5826（skill usage stats）、#5828（bundled extension creator skill）、#5849（CLI `@extension` mention）。
...
- **边界**：只覆盖 CLI input；Desktop/WebUI `@` mention 系统和 extension marketplace discovery 不在 #5849 范围。现有 `@file`、MCP resource mention 行为保持 additive，不改变解析语义。
...
- **`@ext:name` 当前只覆盖 CLI**：#5849 不覆盖 Desktop/WebUI 的 `@` mention，host 如果需要同等显式 activation，需要复用 extension mention resolver 或在各自输入系统实现同等 context 注入。
