...
> 当前记录：#6404 大文本范围读取、#6409 大型 PDF 文本提取预算、#6585 PDF page image fallback，均已按 merged diff 复核。
...
- dense/scanned PDF 在 `pdftotext` 失败或超过 token guard 后，旧链路只能报错；即使配置了 vision model，也没有 bounded page image fallback。

#6404 解决“大文本应该能按范围有界读取”；#6409 解决“大 PDF 全文 fallback 不应进入上下文，但显式页码读取仍应可用”；#6585 解决“文本提取失败/超预算时，vision-capable 环境应能按页渲染图片继续读取”。
...
4. **PDF page image fallback**：当 PDF text extraction 失败或超过输出预算，且有 vision bridge 可消费时，用 `pdftoppm` 把页面渲染成 bounded JPEG image parts。
...
flowchart TD
    ...
    PDFTEXT -->|"text overflow/fail + vision available"| PDFIMG["pdftoppm JPEG page images"]
    PDFPAGE -->|"text overflow/fail + vision available"| PDFIMG
...
- **PDF image fallback 需要 vision 消费路径**：没有 vision bridge 或模型不支持图片时，仍返回 text extraction 的 guidance/error；不会把 JPEG base64 硬塞给纯文本模型。
