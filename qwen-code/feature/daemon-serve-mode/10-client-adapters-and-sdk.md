...
#6740 给 `WorkspaceDaemonClient` 增加 workspace-qualified `getSessionTranscriptPage()`。它要求 `workspace_persisted_transcript`，并强制使用 native REST transport，即使 `DaemonClient` 配了 replaceable ACP transport，也不会走会启动 ACP 的路径。#6769 open 方案不改变 SDK shape，只要求客户端把 `transcript_page_too_large` 作为 413 terminal page error 处理，并理解 `limit` 只是 record-count ceiling。
