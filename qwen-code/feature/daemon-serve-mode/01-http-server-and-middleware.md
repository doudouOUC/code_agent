...
| #5175/#5266 | mid-turn message injection | `POST /session/:id/mid-turn-message`；`mid_turn_message_injected` 事件与 drain timeout recovery |
| #5392 | serve Web Shell static hosting | release 版 `qwen serve` 默认同源托管 built Web Shell SPA；`--open` / `--no-web` 控制启动行为 |
| #5398 | extension management endpoints | `workspace_extensions` 从只读诊断扩展为 install/enable/disable/update/uninstall/refresh mutation 面 |
| #5484 | session reaper timeout validation | daemon boot 校验 `sessionReapIntervalMs` / `sessionIdleTimeoutMs`；`0` 是 disabled sentinel |
| #5541 | Web Shell static sendFile dotfiles | static sendFile 显式允许 dotfiles，避免 `.nvm` 等安装路径下 `index.html` 404 |
...
| 3.5 | #5392 | `[release Web Shell 启用时]` static SPA routes（`/` + built assets，**bearer 之前**注册） | 路由 | 命中即返回 HTML/assets；API 路由仍在 bearer 之后 |
...
### Web Shell static hosting（#5392/#5541）

#5392 让 release 版 `qwen serve` 默认同源托管已构建的 Web Shell SPA。它和 `/demo` 的定位不同：`/demo` 是调试页，Web Shell 是正式浏览器客户端，因此 static routes 挂在 CORS/host allowlist 之后、bearer 之前，保证浏览器能先加载 HTML/JS；真正的 daemon API 仍在 bearer 之后，页面里的请求必须按当前 daemon 配置携带 token。

控制面：

- `--no-web` 关闭静态 Web Shell 托管，只保留 REST/SSE/ACP API。
- `--open` 在 listen 成功后打开浏览器到同源 Web Shell URL。
- 开发态仍可使用 Vite `dev:daemon`；release 托管只服务打包产物。

安全细节：静态路由不应 shadow API 路由，API 404/401 仍走业务路由与统一错误处理；HTML/assets 带 CSP 等安全头。#5541 修复了一个部署路径细节：`sendFile` 默认会拒绝 dotfile 路径，用户通过 `.nvm` 等目录安装时，`index.html` 的绝对路径包含 dot segment，需显式允许 dotfiles，否则 `/` 会 404。
...
| --- | --- | --- | --- | --- | --- |
| GET | `/` / built Web Shell assets | static（CORS/host 后、bearer 前） | — | #5392 | release 版默认同源托管 Web Shell SPA；`--no-web` 关闭，`--open` 启动后打开浏览器 |
...
| POST/DELETE | `/workspace/extensions/*` | `mutate(strict)` | `workspace_extensions` | #5398 | extension install/enable/disable/update/uninstall/refresh 等管理动作 |
...
| POST | `/session/:id/mid-turn-message` | `mutate()` | — | #5175 | live turn 中 text-only 输入注入；成功后发 `mid_turn_message_injected` |
...
### Session reaper timeout validation（#5484）

#5484 把 session reaper 相关时间配置前移到 daemon boot 阶段校验：`sessionReapIntervalMs` 与 `sessionIdleTimeoutMs` 只接受有限非负整数，`0` 是显式 disabled sentinel。非法值启动即失败，而不是在后台 timer 注册时退化成不可预测行为。

这和 `--permission-response-timeout-ms` 的姿态一致：操作者写错资源回收配置时 fail-closed，避免长寿 daemon 因 `NaN`、负数或隐式 1ms timer overflow 进入高频扫描/永不回收等状态。文档层面要把 `0` 和缺省区分开：缺省走产品默认值，`0` 是操作者明确关闭该 reaper。
...
### #4236 — mutation gating

- `auth.ts:createMutationGate`：工厂闭包，按 `tokenConfigured` / `requireAuth` / `strict` 三态返回 passthrough 或 strictDenier；`strictDenier` / `passthrough` 缓存复用避免 N 闭包。
- `auth.ts:bearerAuth`：boot 时 SHA-256 预哈希 token，每请求 `timingSafeEqual` 常数时间比较；手写 `indexOf` 切分替代 regex 防 ReDoS。
- `runQwenServe.ts`：四条 boot-loud 拒绝（非 loopback 无 token、`--require-auth` 无 token、host:port 误用、`--allow-origin '*'` 无 token）。

### #4527 — CORS allowlist

- `auth.ts:parseAllowOriginPatterns`：严格往返 `parsed.origin === entry` 校验 + `Set<lowercase>`；`*` 映射 `allowAny`。
- `auth.ts:allowOriginCors`：回显请求 Origin（非字面 `*`）+ `Vary: Origin`；`Origin: null` 一律 403；OPTIONS 预检 204 短路。
- `auth.ts:denyBrowserOriginCors`：默认策略——任何 `Origin` 头即 403，防 daemon 自我 CSRF。
- `runQwenServe.ts`：boot 期 `parseAllowOriginPatterns` 校验 + stderr 输出（`*` 时附 WARNING）。

### #4530 — deadline + idle timeout

- `server.ts:resolvePromptDeadlineMs`：server 旗标与请求 override 取 `Math.min`（只能缩短不能延长）。
- `server.ts:PromptDeadlineExceededError`：abort reason 类（docstring "→504" 已陈旧，实际 202 非阻塞）。
...
- `server.ts` access-log middleware：注册在 bearer 之前，`res.on('finish')` 记录 `route` / `sessionId` / `clientId` / `status` / `durationMs`；排除 `GET /health` 和 heartbeat 高频噪声。
- `daemonLogger.ts:renderCtx`：按 `FIXED_CTX_ORDER` 固定顺序输出上下文字段；`renderCtxValue` 用 `JSON.stringify` 包裹含 `=` / 空白的值防日志注入。
- 整个回调 try/catch——"日志失败绝不影响请求"。

### #4585 — non-blocking 202（@chiga0）

- `server.ts` prompt handler：`bridge.sendPrompt` fire-and-forget（`.then` 成功日志 / `.catch` 失败日志），**立即** `res.status(202).json({promptId, lastEventId})`。
- `capabilities.ts`：广告 `non_blocking_prompt` 能力 tag。
- deadline-exceeded 仅经 SSE `prompt_cancelled` / turn_error 的 `errorKind` 抵达客户端，不产生同步 504。

### #4861 — per-tier HTTP rate limiting（@doudouOUC）

- 新增 `rateLimiter.ts`：token-bucket 算法，continuous drip refill；三层限速（prompt 10/min、mutation 30/min、read 120/min，均可 `--rate-limit-*` CLI 覆盖）。
- key 策略：loopback 按 `x-qwen-client-id` 区分（无 header 回退 `"anonymous"`），non-loopback 按 `remoteAddress + clientId` 组合。
...
### #5174 — daemon status API（@ytahdn）

- `server.ts` 新增 `GET /daemon/status`，默认 summary 模式只读 daemon 内存计数，不 spawn ACP child。
- `detail=full` 聚合 session、ACP connection、auth device-flow、workspace diagnostics；每个 workspace section 独立降级，避免单项失败拖垮整个 status 响应。
- `capabilities.ts` 注册 baseline 能力 `daemon_status`；普通 bearer 鉴权生效，不沿用 `/health` 的 loopback 免鉴权语义。

### #5175/#5266 — mid-turn message route（@wenshao）

- `server.ts` 新增 `POST /session/:id/mid-turn-message`，用于 live turn text-only 输入；普通 prompt 仍走 `POST /prompt`。
- bridge 侧维护 mid-turn queue，ACP child 通过 `craft/drainMidTurnQueue` 拉取；成功注入后广播 `mid_turn_message_injected`。
- #5266 把事件常量集中到 `daemonEventTypes.ts`，并补 drain timeout recovery，避免 child 迟到响应导致消息静默丢失。

### #5392 — release Web Shell hosting（@wenshao）

- `qwen serve` 默认服务 built Web Shell SPA：`/` 返回 `index.html`，静态 assets 同源加载。
- 新增 `--open` / `--no-web`：前者 listen 后打开浏览器，后者关闭 static hosting。
- static routes 放在 bearer 之前，API routes 仍在 bearer 之后；同源页面可加载，但调用 daemon API 仍受 token / mutate / CORS / host allowlist 保护。
- 静态响应带 CSP/security headers，并确保 static fallback 不 shadow `/session/*`、`/workspace/*`、`/acp` 等 API。

### #5398 — extension management endpoints（@ytahdn）

- 在 `workspace_extensions` 只读诊断基础上增加 mutation routes，覆盖 install / enable / disable / update / uninstall / refresh。
- 写操作走 `mutate(strict)`，并做 workspace client、extension id/source、URL credentials/private network/local link 等校验。
- mutation 异步执行，完成后发 `extensions_changed`；SDK/web-shell 刷新 workspace extension snapshot 与相关 commands/hooks/settings。

### #5484 — session reaper timeout validation（@wenshao）

- daemon boot 校验 `sessionReapIntervalMs` / `sessionIdleTimeoutMs` 为有限非负整数。
- `0` 作为显式 disabled sentinel 保留；非法值启动失败，避免后台 timer 退化成高频扫描或永不回收。

### #5541 — Web Shell static dotfile paths（@wenshao）

- static `sendFile` 显式允许 dotfiles，使通过 `.nvm` 等 dotfile 目录安装时的 Web Shell `index.html` 能正常返回。
- 修复点限定在 static hosting；API 路由与 bearer/mutate 门控不变。
