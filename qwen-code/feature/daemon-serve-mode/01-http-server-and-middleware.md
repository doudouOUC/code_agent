...
本文用大量篇幅锁定**中间件链的精确顺序**与每道闸的**短路语义**，并给出完整路由表、鉴权/变更门控矩阵、CORS/host allowlist 细节、请求生命周期与超时（prompt route 非阻塞 202，deadline 由 bridge 负责终态和 FIFO 释放）。
...
| --- | --- | --- | --- | --- | --- |
| GET | `/health` | bearer*（条件豁免） | `health` | L865/L943 | `?deep=1` 聚合所有 managed runtime 的 session/permission/prompt 计数；#8588 另 additive 返回 `activeWork`；豁免见上 |
...
## 请求生命周期与超时（prompt 202 + bridge deadline / SSE idle / access log）

### Prompt 绝对 deadline —— route 非阻塞 202，bridge 负责终态

`POST /session/:id/prompt` 的当前形态：

1. 校验 `prompt` 为非空对象数组；可选 `deadlineMs` 必须是正整数，否则 `400 invalid_deadline_ms`。
2. 从 body 中删除 route-only `deadlineMs`，防止未声明字段透传到 ACP prompt body。
3. `effectiveDeadlineMs = resolvePromptDeadlineMs(opts.promptDeadlineMs, requestDeadlineMs)`：server flag 未设则 `undefined`；请求 override 只能**缩短不能延长**（`Math.min`），操作者守住上界。
4. route 不再设置 `setTimeout(...abort...)`。#7400 后 deadline 所有权在 bridge：handler 把 `{clientId, promptId, deadlineMs: effectiveDeadlineMs}` 放进 `bridge.sendPrompt()` 的 request context。
5. HTTP 仍立即返回 `202 {promptId,lastEventId}`；后续结果全部通过 SSE `turn_complete` / `turn_error` 跟进。

bridge 在 prompt admission（也就是 202 语义点）arm deadline，因此 budget 覆盖 queue wait 和 agent execution。deadline 到期时，bridge 会发布 `turn_error{code:'prompt_deadline_exceeded'}`，清理 active prompt state，通过 deadline race 释放 per-session FIFO，并 best-effort `connection.cancel()`。一个不配合 cancel 的 agent 可能仍在后台执行，但不会再永久钉住该 session 的 prompt FIFO。

`PromptDeadlineExceededError` 类定义也随 #7400 从 server-side route helper 移到 `@qwen-code/acp-bridge`，`packages/cli/src/serve/server/prompt-deadline.ts` 只保留 re-export，保证既有 server/test import 与 `instanceof` 兼容。
...
| `SessionRestoreTimeoutError` | 504 | `restore_timeout` + `Retry-After:5` | sessionId, action, timeoutMs（#8691） |
...
5. **prompt deadline 的 route/bridge 分工**（#4530/#7400）。HTTP route 保持 non-blocking 202 和“请求只能缩短不能延长”解析；bridge 才拥有 deadline race、formal terminal、FIFO release 和 best-effort agent cancel。这样卡死 agent 不再永久阻塞同 session 后续 prompt，但 deadline 结果仍只通过 SSE terminal 抵达，集成方不能从 202 HTTP 响应判断最终结果。
...
4. **`/health?deep=1` 非真实 liveness**。#6961 已把计数范围从 primary bridge 扩到所有 managed runtime；#8588 已合入的 `activeWork` 能补 restart idle guard，但仍只读 getter/state snapshot，检测不出 wedged 会话。
...
### #4530 / #7400 — deadline + idle timeout

- `server.ts:resolvePromptDeadlineMs`：server 旗标与请求 override 取 `Math.min`（只能缩短不能延长）。
- `routes/session.ts`：prompt route 只解析/校验 `deadlineMs` 并透传到 bridge context；HTTP 仍立即返回 202。
- `acp-bridge/src/bridge.ts`：deadline 在 admission 点 arm，参与 dispatch race；超时发布 `turn_error{code:'prompt_deadline_exceeded'}`、释放 FIFO、清 active state 并 best-effort cancel agent。
...
### #8572 — REST SSE stream/client observability（merged）
...
### #8588 — activeWork deep health（merged）
...
### #10179 — standalone daemon session API（merged）
...
### #10403 — trusted-loopback operator authority（merged）

- 最终实现把 Bearer credential authentication 与 deployment-boundary authority 分离。只有主 listener 实际绑定 loopback、未配置 token 且未启用 `--require-auth` 时，primary-listener 请求才通过 `requestHasOperatorAuthority()`；该请求不会被伪装成 Bearer authenticated。
