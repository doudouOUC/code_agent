...
- 新增 `rateLimiter.ts`：token-bucket 算法，continuous drip refill；三层限速（prompt tier 5/min burst 10、mutation tier 30/min burst 60、read tier 60/min burst 120，均可 `--rate-limit-*` CLI 覆盖）。
