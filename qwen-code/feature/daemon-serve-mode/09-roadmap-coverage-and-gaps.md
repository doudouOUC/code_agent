...
2026-08-19 的最新增量继续收敛 restore/WebUI 一致性、activeWork、Conversations runtime boundary、live-state、daemon resource guard 与工具调用安全边界：#8691 已合入，先把大型 session restore 的 timeout、late cleanup、quarantine 与 telemetry 安全化；#9055 已把 #8743 的 selective session restore 从 docs-only design 推进为 runtime 实现，cold load/resume 不再先全量 materialize persisted transcript 后按 `historyPageSize` 裁剪。#8833/#8882 已合入 same-id attachment stale-work fencing 与 cross-session visible-state transaction；#8933/#8939 已合入 restore shape fencing 与 same-session refresh transaction；#8955/#8990 继续补齐 prompt admission ownership 和 same-session refresh race closeout；#9048 closed diff 只作为 authoritative resync / live-journal repair 的未合入观察。#8588 已合入 activeWork lifecycle gate，#9042 已合入 background shell `shell` hold，#9134 已合入 automatic active-work close 授权拒绝前不得破坏 queued work、deferred spawn-owner kill 避开 close/probe in-flight，以及 child definitive close refusal 可重试保留而不是 channel kill。#8891/#8931 已合入 WebShell shared catalog 和 prompt-safe navigation；#9180 已合入 Web Shell text file attachments、file chips 和 daemon prompt `resource` 投递。#8892/#8932 已合入 persisted session catalog cache 与 continuation admission logs；#9261 已合入 workspace session live-state route/capability/SDK，用纯内存 selected-runtime live snapshot 与 catalog version 降低高频 catalog polling 成本；#9366 已合入 WebShell live-state consumer，#9396 已合入 live-state activity watermark，#9476 已合入 WebShell completion-sequence consumer。#8911/#8947/#9007 已合入 daemon ACP NDJSON、handler/outbound/request guard 与 ACP HTTP pre-attach byte budgets；#9380 已合入 ACP child old-generation peak measurement，仍是 observe-only。#9435/#9436 已合入 duplicate provider tool-call id 的可见 loop stop 与 argument-aware replay detection。#8890 已合入 Conversations runtime foundation，不再是 docs-only draft；#9181 已合入单 owner record、ServeAppLifecycle release proof、activity gate 和 ordinary runtime visibility guard；#9362 已合入 runtime record transient I/O retryability；#9341 仍是 standalone source/identity/admission primitives open diff；#8824 为 closed draft 前身；durable checkpoint follow-up 仍是独立路线图项。
