...
- **standalone PR2B internal core 与 public API**：#9978 已合入 `StandaloneSessionService`，在唯一 Conversations runtime 中提供 create/get/list/load/resume/prompt/continue、确定性 private directory containment、projectless Live task 迁移和 terminal quarantine；#10179 当前 open diff 在其上增加 public standalone route/capability 与 journaled delete。
...
- **ACP process-tree ownership**：#10142 当前 open diff 让标准 ACP spawn 管理整个进程树，使用 bounded process snapshot、TERM→KILL 与 root-exit cleanup；legacy attach 保持 direct-child。
...
| [#10142](https://github.com/QwenLM/qwen-code/pull/10142) | open | ACP process-tree reaping | 标准 ACP spawn 的 tree ownership、bounded snapshot、TERM→KILL、root-exit cleanup 与 SIGHUP lifecycle |
...
| [#10179](https://github.com/QwenLM/qwen-code/pull/10179) | open | standalone public daemon API | 条件 capability、public route family、exact owner admission 与 journaled delete recovery |
...
#10179 当前 open diff 在该 internal core 上增加 `/standalone/sessions` public lifecycle。所有 route 要求 top-level exact standalone owner，不允许 child/Live/project/worktree/ambiguous/foreign owner，也不 fallback primary。delete 用私有 journal 把 transcript unlink 作为 commit point：commit 前恢复 staged directory，commit 后继续 sidecar/attachment/directory cleanup，并通过 `fileCleanupPending` 暴露未完成清理。

#10142 当前 open diff 把标准 ACP child 注册为 process-tree owner。POSIX 用有界 `/bin/ps` snapshot 发现后代 PGID，graceful shutdown 先 TERM 后 KILL，root 退出也回收已知组；同步退出只做一次 snapshot。Windows 使用 taskkill tree，legacy direct `ProcessRegistry.attach(child)` 不自动升级。
...
6. **#9513/#9665/#9687/#9763/#9819/#9820/#9838/#9976/#9978/#10144 已合入，#9626/#10142/#10179 仍为 open**。archive race recovery、AUQ restore、per-session model、Live identity、close holds、current-session task、channel liveness、standalone internal core 与 empty persistence 已按 merged diff 记录；persisted maintenance、ACP process-tree reaping 和 standalone public API 只能记录当前方案。#9978 本身没有 public route/capability/SDK/UI，#10179 当前 open diff 承接 route/capability。若后续 PR 调整这些契约，需要按最终 diff 再同步。
