...
- **restore ask_user_question HITL**：#9665 已合入默认关闭的 serve flag；load/resume 只对尾部纯 AUQ batch 跳过 orphan finalize，并在 session 注册后通过 trusted tracked prompt 生成新 request id、复用既有 permission vote 继续原 turn。
...
#9626 当前是 open。当前 19 文件 diff 已收窄为 delete/archive/unarchive maintenance：physical classifier 分离 storage ownership 与 replay readability，在 writer lease、runtime generation 和 identity snapshot 下复核 exact owned regular file。空、损坏、torn 和 legacy orphan transcript 可由 owning runtime 维护；foreign workspace、symlink/directory/FIFO、case ambiguity 与无法证明 identity 的候选继续拒绝。archive/unarchive 默认 conflict；只有 capability-gated `resolveConflicts:true` 才执行 keep-destination repair 并返回 `resolvedConflicts`，不合并 transcript。session list、transcript/export、organization、Live/task identity 等广域统一已不在当前 diff。
...
6. **#9513/#9665/#9687 已合入，#9626 仍为 open**。archive race recovery、default-off AUQ restore 与 per-session model restore 已按 merged diff 记录；persisted maintenance/conflict repair 只能记录当前方案，不能视为 `main` 已落地能力。#9341/#9512 已合入 internal standalone primitives/hardening，但仍没有 public standalone route/SDK/UI。若后续 PR 调整 repair semantics、restore hint/capability、model projection/replay/failure contract、runtime ownership、live-state version/cache 或 activity consumer，需要按最终 diff 再同步。
...
- **Conversations standalone primitives / maintenance / restore state**：#9341/#9512 已合入 source classification、standalone directory identity/ensure hardening、session id admission 与 JSONL integrity；#9513/#9665/#9687 已合入 archive race recovery、AUQ re-hang 与 per-session model binding restore。#9626 仍为 open，覆盖 maintainable storage/conflict repair；#9362/#9396/#9476 已合入 runtime I/O retry、live-state `updatedAt` 与 WebShell completion settle。
