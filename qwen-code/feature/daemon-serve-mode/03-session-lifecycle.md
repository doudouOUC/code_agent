...
- **standalone PR2B internal core**：#9978 当前 open diff 增加 `StandaloneSessionService`，在唯一 Conversations runtime 中提供 create/get/list/load/resume/prompt/continue、确定性 private directory containment、projectless Live task 迁移和 terminal quarantine；仍不注册 public standalone route/capability/SDK/UI。
- **archive race recovery 与 persisted maintenance**：#9513 已合入 exact-spelling active/archive 双副本的 active-first load/resume/transcript/export、delete-both、metadata move、legacy parent、目录消失与单次 catalog lookup；#9626 当前 open diff 只扩展 delete/archive/unarchive 的 physical maintenance classification、identity revalidation 与 capability-gated conflict repair。
...
- **current-session scheduled task**：#9838 当前 open diff 允许 durable `cron_create` 显式复用发起调用的 daemon 顶层普通 session；Core prompt context、private ACP caller identity、Serve owner/lifecycle admission、generation rollback 与条件 capability 共同 gate，默认仍创建 dedicated session。
...
| [#9626](https://github.com/QwenLM/qwen-code/pull/9626) | open | persisted maintenance | empty/malformed/orphan owned regular transcript 的 lifecycle maintenance、identity fencing 与 capability-gated conflict repair |
...
| [#9838](https://github.com/QwenLM/qwen-code/pull/9838) | open | current-session scheduled task | durable task 显式绑定当前普通 session，受 prompt/owner/source/binding/generation gate 约束 |
...
| [#9978](https://github.com/QwenLM/qwen-code/pull/9978) | open | standalone session PR2B internal core | fail-closed service、canonical persisted owner、private directory containment、projectless Live task 迁移与 terminal quarantine；无 public standalone API |
...
#9838 当前仍为 open。`cron_create{durable:true,sessionMode:'current'}` 从 prompt context 取得精确 active prompt，经 ACP Session/bridge 证明 connection、session、prompt 和 top-level ordinary source，再由 Serve host 复核 owning runtime、pending interaction、parent/source、既有 task binding 与 task limits。成功后持久化 existing-session binding，标记 `sessionOwnedByTask:false`；runtime generation 在 commit 后关闭会回滚 task。旧请求和 capability 缺失保持 dedicated session，不能把该方案写成 `main` 已落地能力。
...
#9978 当前仍为 open。`packages/cli/src/serve/conversations/standalone-session-service.ts:StandaloneSessionService` 在唯一 Conversations runtime 的 activity gate 内提供 create/get/list/load/resume/prompt/continue；top-level 和 child session 都保留 canonical persisted ID 与 explicit `sourceType:'standalone'`，但 child 不进入顶层 list。每次创建先 reserve ID、准备 owner-only/non-symlink/direct-child private directory，再持久化 source 并接纳 initial prompt；load/resume/follow-up 重新验证 pinned device/inode/realpath。projectless Live task 改走同一 service，generic routes 与 Config/permission/cron/workflow guards 在副作用前拒绝 workspace 能力越界。

若 directory compromise、spawn outcome 不明或 close refusal 使 containment 无法证明，service 会冻结 creation entry 并 quarantine Conversations runtime；已知 standalone owner 继续暴露 unavailable，而不是 fallback primary。该 diff 没有 `/standalone/*`、`standalone_sessions_v1`、SDK/WebUI，也没有补齐 archive/delete/repair、attachment、durable cron、parent-child cascade 或 move/fork-to-project，不能写成公开能力。
...
6. **#9513/#9665/#9687/#9763/#9819/#9820/#9976 已合入，#9626/#9838/#9978 仍为 open**。archive race recovery、default-off AUQ restore/hardening、per-session model restore、Live task canonical bridge identity、bounded close-refusal holds 与 channel transport liveness 已按 merged diff 记录；persisted maintenance/conflict repair、current-session scheduled task 和 standalone PR2B internal core 只能记录当前方案，不能视为 `main` 已落地能力。#9341/#9512 已合入 standalone primitives/hardening，但 #9978 仍没有 public standalone route/capability/SDK/UI。若后续 PR 调整 repair semantics、restore hint/capability、model projection/replay/failure contract、runtime ownership、live-state version/cache、activity consumer、scheduled-task binding 或 standalone containment，需要按最终 diff 再同步。
