...
> 代码分支：早期 `file:symbol`(+line) 锚点以集成分支 **`daemon_mode_b_main`** 为准（`git -C <repo> show daemon_mode_b_main:<path>`）；#4490 合入 main 后的 #5035/#5175/#5266 等近期会话语义以当前 `main` 实现为准。

---

## 概述

Mode B 把"会话"提升为 daemon 内的一等资源：一个 `qwen serve` 进程绑定单一工作区（`boundWorkspace`，#3803 §02 "1 daemon = 1 workspace"），内部用一张 `byId: Map<sessionId, SessionEntry>` 登记所有活跃会话，并通过 **ACP core 子进程**（`AcpChannel`）真正承载模型/工具状态。本篇覆盖会话的完整生命周期：
...
- **title / mid-turn 输入**：ACP child 侧自动标题通过 side channel 推成 session metadata；运行中输入通过 mid-turn queue 注入当前 turn（#5035/#5175/#5266）。
...
| [#4694](https://github.com/QwenLM/qwen-code/pull/4694) | — | compacted session replay | `TurnBoundaryCompactionEngine` turn 级压缩 + `loadSession` 返回 `compactedReplay`/`liveJournal`/`lastEventId`，替代 #4678 的无界 JSONL 方案 |
...
| [#5035](https://github.com/QwenLM/qwen-code/pull/5035) | merged | session title push | ACP child 记录自动标题后，经 side channel 转成 `session_metadata_updated`，客户端不再等 session-list 轮询。 |
| [#5175](https://github.com/QwenLM/qwen-code/pull/5175) | merged | mid-turn injection | web-shell 运行中输入通过 daemon queue / `craft/drainMidTurnQueue` 注入当前 turn，并发 `mid_turn_message_injected`。 |
| [#5266](https://github.com/QwenLM/qwen-code/pull/5266) | merged | mid-turn recovery | `mid_turn_message_injected` 常量集中；ACP child drain 超时后恢复迟到响应，避免消息静默丢失。 |
...
| `workspaceCwd` | `string` | 规范化后的工作区（恒等于 `boundWorkspace`） |
...
#5035 把 ACP child 侧生成的自动标题也接进了同一条 metadata 语义：child 在记录标题后不扩展 ACP 标准 `SessionUpdate` union，而是经项目私有 side channel 发 `qwen/notify/session/title-update`，bridge 再转换为 daemon 公共事件 `session_metadata_updated`。这样做的好处是外部客户端仍只消费既有事件，不需要理解项目私有 ACP 变体；坏处是标题更新链路不是标准 ACP 面，排障时要同时看 child side channel 和 daemon SSE。

---

## mid-turn 输入注入（#5175/#5266）

#5175 为 web-shell 的“模型仍在跑时继续输入”补了一条 daemon 生命周期分支：`POST /session/:id/mid-turn-message` 只在当前 session 有 live turn 时接受 text-only 消息，把内容放进 bridge 侧队列；ACP child 在 turn loop 中通过 `craft/drainMidTurnQueue` 拉取并注入当前 turn。注入成功后 daemon 广播 `mid_turn_message_injected`，让多个 tab 可以把“这条用户输入已经进入正在运行的 turn”渲染成确定状态。

这条路径和普通 `POST /prompt` 不同：

- 它不创建新 turn，也不走 prompt FIFO；目标是补充当前活跃 turn。
- 它只接受当前实现能安全合并的文本消息；图片等富媒体会退回下一轮普通 prompt，避免把二进制/多 part 内容塞进运行中的模型调用。
- 它有 exactly-once 语义：bridge 侧队列被 child drain 后才对外发 `mid_turn_message_injected`，客户端不应仅凭 HTTP 接受就认为已经注入。

#5266 做了两个硬化：一是把 `mid_turn_message_injected` 事件名集中到 `acp-bridge/src/daemonEventTypes.ts` 并由 SDK re-export，减少 web-shell / SDK / bridge 手写字符串漂移；二是补了 drain 超时恢复，若 child 对某次 `craft/drainMidTurnQueue` 迟到响应，bridge 不再静默丢消息，而是在下一批 drain 时恢复处理。
...
**问题**：EventBus 的 ring buffer 容量有限（默认 8000 帧）；长会话（3h+ 重度使用）产生的 streaming chunk 轻松超过 ring 上限，eviction 后 `loadSession` 无法从 ring 回放完整历史。#4678 尝试无界 JSONL 持久化但引入了 watermark 竞态和无界文件增长。
...
### #5035 — session title push（@chiga0）

- ACP child 在自动标题生成/记录后，通过私有 `qwen/notify/session/title-update` side channel 通知 bridge。
- bridge 将该通知归一化为既有 `session_metadata_updated`，复用 metadata event/reducer，不扩展 ACP 标准 `SessionUpdate` union。
- web-shell/session list 订阅 SSE 后可立即刷新标题，不再依赖轮询 workspace sessions。

### #5175 — mid-turn message injection（@wenshao）

- `server.ts` 新增 `POST /session/:id/mid-turn-message`，只在 live turn 接受 text-only 输入。
- bridge 为 session 维护 mid-turn queue；ACP child 通过 `craft/drainMidTurnQueue` 拉取并注入当前 turn。
- 注入完成后发 `mid_turn_message_injected`，给 web-shell 多 tab 和 transcript reducer 一个确认点；富媒体内容回退下一轮普通 prompt。

### #5266 — mid-turn recovery（@wenshao）

- `mid_turn_message_injected` 事件名集中到 `acp-bridge/src/daemonEventTypes.ts`，SDK/web-shell 通过 re-export 消费，减少手写字符串漂移。
- drain 超时后若 ACP child 迟到返回，bridge 在下一批 drain 中恢复处理，避免 mid-turn 消息被静默丢弃。
