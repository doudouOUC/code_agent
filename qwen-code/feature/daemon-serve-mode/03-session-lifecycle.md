...
- **restore regressions 与 persisted storage lifecycle**：#9513 当前 open diff 只恢复同拼写 active+archived 双副本 load/resume、metadata archive race、legacy parent、目录并发消失与 restore 单次异步 catalog lookup；#9626 当前 open draft 再把 logical/storage identity 扩展到 delete/rename/archive/unarchive/transcript/export/organization/sidecar，并以 identity snapshot 与 maintenance fencing 支撑 capability-gated conflict repair。
- **restore ask_user_question HITL**：#9665 当前 open diff 提供默认关闭的 serve flag；load/resume 只对尾部纯 AUQ batch 跳过 orphan finalize，并在 session 注册后通过 trusted tracked prompt 生成新 request id、复用既有 permission vote 继续原 turn。
...
| [#9513](https://github.com/QwenLM/qwen-code/pull/9513) | open | PR2A restore regressions | 同拼写 active+archived 双副本 load/resume 选 active；metadata race、legacy parent、目录消失与重复 catalog scan 同步修复，异拼写 conflict/mutation 继续 fail-closed |
| [#9626](https://github.com/QwenLM/qwen-code/pull/9626) | open draft | persisted storage lifecycle | logical/storage identity、maintainable transcript ownership、maintenance fencing 与 capability-gated conflict repair |
| [#9665](https://github.com/QwenLM/qwen-code/pull/9665) | open | restore ask_user_question HITL | 默认关闭的 serve flag 在 load/resume 后重新挂起尾部纯 AUQ batch，不做 boot-time auto-resume |
...
#9513 当前 open diff 已缩窄到 #9489 的五项 PR2A 回归：请求 spelling 与存储 spelling 完全一致、active+archived 都可读时，REST/ACP load/resume 选 active copy；creation metadata 按 active 后 archived 容忍 archive move；legacy parent 移除过严 UUID 形状 gate 后交给 storage resolution；identity probe 间目录消失按 already gone；restore 去掉锁前重复扫描，并把剩余 case-insensitive lookup 改为 async readdir。异拼写 conflict、mutation、archived-only read 与跨 workspace ownership 仍 fail-closed；此前更广的 mixed-case list/organization/scheduled-task/Live-task 扩展已经撤回。
...
#9626 当前是 open draft。它把 caller-visible logical id 与 filesystem-only `storageSessionId` 分离到完整生命周期：`resolvePersistedSessionStorage()` 枚举 active/archive 物理 spelling，维护路径在 writer lease、runtime generation、Live activity 和 maintenance guard 下复核 regular-file identity 与候选集合。空、损坏和 orphan regular transcript 可以被 owning runtime 维护；foreign workspace、symlink/directory、真实 case twins 和无法证明 identity 的候选继续拒绝。archive/unarchive 默认 conflict；只有 capability-gated `resolveConflicts:true` 才执行 keep-destination repair，并返回 `resolvedConflicts`，不合并 transcript。该 draft 未合入，不能视为已发布行为。

#9665 当前是 open diff。默认关闭的 `--restore-ask-user-question` 只识别 transcript 尾部全部为结构合法 AUQ calls 的 batch；replay 跳过这些 call 的 orphan finalize，bridge 在 session entry 注册后通过 trusted tracked prompt 重新生成 permission request id。用户投票后的真实 function response 继续原 turn；cancel/timeout 沿用 stop-after-cancel，worktree notice 延迟到 post-answer message。混合 dangling tools、不开 flag、通用 `continueLastTurn` 和 daemon boot scan 保持既有行为；v1 不新增 capability tag。
...
6. **#9513/#9665 仍为 open，#9626 仍为 open draft**。五项 PR2A load/resume 回归修复、完整 storage lifecycle/conflict repair 与 AUQ restore 只能记录当前方案，不能视为 `main` 已落地能力。#9341/#9512 已合入 internal standalone primitives/hardening，但仍没有 public standalone route/SDK/UI；#7967、#9055/#9134/#9181/#9261/#9362/#9396/#9476 已按 merged diff 更新。若后续 PR 调整 storage identity、repair semantics、restore hint/capability、projection/replay/failure contract、runtime ownership/release proof、live-state version/cache 或 activity consumer，需要按最终 diff 再同步。
...
- **Conversations standalone primitives / storage lifecycle / AUQ restore / live-state recency**：#9341/#9512 已合入 source classification、standalone directory identity/ensure hardening、session id admission 与 JSONL integrity。#9513/#9626/#9665 仍为 open/open draft，分别覆盖五项 PR2A load/resume 回归、maintainable storage/conflict repair 与 AUQ re-hang；#9362/#9396/#9476 已合入 runtime I/O retry、live-state `updatedAt` 与 WebShell completion settle。
