...
| #10924 | mandatory Conversations writer fences | merged private bridge/runtime fence | 只有在 exact Conversations marker 与 channel factory child-env forwarding 同时证明时才发布 runtime；缺证明的新 candidate 拒绝、existing runtime 终态 quarantine。 |
| #11120 | failed conditional-close probe control | merged reclaim suppression | 最终实现为 unanswered probe增加指数退避、JSON-RPC detail提取、无 completion promise的有界轮询与 condemned/no-capability本地收敛；hold taxonomy根因未修复。 |
...
### #10554 — sessionless user-language fan-out（merged）
...
### #10924 — Conversations bridge mandatory-lease attestation（merged）
...
- 最终实现在 bridge entry记录连续 unanswered probe次数和 next retry time：首次恢复重试保持即时，第二次起60秒指数退避到1小时；child任意回答、重新报告 hold或从 snapshot消失即清零。
- candidate gate在发 probe前生效，因此 backoff期间不占用 drain budget也不重复日志；失败日志使用 `extractErrorMessage()`保留跨 JSON-RPC的 plain-object code/message/detail。
- 未协商 active-work或已 condemned的 channel绕过 probe backoff并按既有授权本地收敛，避免 retained ghost entry阻塞 channel排空；已经 dying但没有本地关闭授权的 channel仍保留。
- child没有 completion promise时，CLI Session settle改为10ms timer poll而不是高速 `setImmediate`循环。该 PR不补 cron/goal/monitor/history mutation的 negotiated hold，因此是抑制和诊断，不是根因修复。
