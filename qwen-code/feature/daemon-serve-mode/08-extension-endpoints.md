...
| `/extensions/*` | GET/POST/PUT/DELETE | 读 bearer / 写 `mutate(strict)` | #6825 open：V2 user-level extension artifact 管理、transactional store 与 async operation status。 |
| `/workspaces/:workspace/extensions/*` | GET/PUT/DELETE/POST | 读 bearer / 写 `mutate(strict)` | #6825 open：workspace projection、activation override 与 runtime refresh；不拥有 artifact mutation。 |
...
| `/session/:id/shell` | POST | `mutate(strict)` | server-side shell 执行；真正的安全边界是 cwd 服务端固定 + bearer（见 shell 小节）。 |
...
所有端点都先过中间件链（Origin-strip → CORS → hostAllowlist → bearerAuth → json）。只读诊断面只要求全局 bearer；`recap` / `btw` 这类只花 token、不改 daemon/workspace 持久状态的控制面使用非 strict `mutate()`；`shell`、`rewind`、settings 写入、workspace-qualified mutation 和 extension V2 artifact/activation mutation 使用 `mutate(strict)`。
...
| #6798 | fix(serve): route session actions to the owning workspace | 2026-07-13 | metadata、recap、BTW、mid-turn、task cancel、goal clear legacy route 按 live owner runtime dispatch。 |
| #6825 | feat(serve): add extension management v2 | open | `extension_management_v2` 当前 open 实现：transactional user-level artifact store、workspace activation policy、operation queue、post-commit warnings、SDK/TUI/Web Shell surface。 |
...
| #6833 | fix(serve): Route session continue, language, and artifacts by owner | open | continue、language、artifact add/delete legacy route 按 owning runtime dispatch。 |
...
严格门控是必须的：rewind 不是 cosmetic control，它会改内存会话、恢复工作区文件，并可能让多个客户端看到历史倒退。成功后 bridge 发布 `session_rewound` SSE，reducer 记录 `rewindCount` / `lastRewind`，其它客户端据此刷新 transcript、diff 和文件视图。#6826 后，在 multi-workspace daemon 中这两条 singular route 先按 live session owner 解析 runtime，再调用 owning `runtime.bridge`；`rewindFiles` 只接受可选 boolean，SDK rewind 即使配置 ACP transport 也强制走 REST，避免绕开 strict mutation auth。
...
### extension management v2（#6825 open）

#6825 open diff 新增 `extension_management_v2`，并保留旧 `workspace_extensions` 作为 primary compatibility/diagnostic surface。V2 不采用早期 workspace-qualified extensions 方案名：核心模型是 user-level artifact + workspace activation policy，而不是每个 workspace 拥有自己的安装物；#6825 取代 #6638 的早期观察版，补齐 transaction journal/recovery、settings revision bundle、operation warnings 和 SDK/TUI/Web Shell surface。

资源拆分：

- user-level artifact：安装目录在 `QWEN_HOME/extensions`，由 `ExtensionStore` 唯一写入。
- activation policy：按 default activation、exact workspace override（enabled/disabled/inherit）、V1 path rule 和 global default 计算 default/workspace/effective/source 状态。
- store state：`~/.qwen/extension-store/state.json`、`state.previous.json`、staging、trash、rollback、transaction journal 和 recovery；支持 V1 import/projection/downgrade，并以单调 generation 驱动 watcher 收敛。
- sensitive settings：prepare 阶段写 revisioned secret bundle，staged artifact 只放 backend/revision selector；commit 原子激活完整 bundle，取消/替换负责清理未选中或旧 bundle。
...
慢操作返回 `202 Accepted` + `operationId`，operation history 只在 daemon 本地保留且 terminal records capped；store generation 才是权威状态。prep queue 并发 2，durable commit queue 单并发，且 artifact/state 持久化后立即释放 commit lane，runtime refresh/cleanup 不阻塞下一个 durable commit；prepared update 携带 artifact generation，stale same-artifact commit 返回 `extension_conflict`。workspace activation/refresh 仍要求 trusted workspace，read projection 可按既有 trust 策略降级。install 需要显式 consent 和初始 activation；GitHub/npm 下载接入 public network policy，拒绝 credential、非 HTTPS、private network 和 local link 等高风险来源，并使用 pinned lookup/curl resolve。post-commit refresh/settings/cleanup 失败不会回滚 committed generation，而是通过 operation status、legacy CLI command 和 TUI action 显示 warning；CLI enable/disable/install/link/uninstall/update 会打印 committed warning。
...
3. publish `user_shell_command` 事件（带 `originatorClientId`，供其他客户端在 SSE 上抑制自己动作回声）。#6826 后 multi-workspace daemon 会先按 live owner 选择 runtime，因此 secondary session 的 shell cwd 是 secondary workspace，而不是 primary workspace。
...
### shell 缺少 baseline 支持能力标签

值得一记：`capabilities.ts` 上有 `session_tasks`（L85）、`session_recap`（L181）、`session_btw`（L184），但没有表示「server-side shell baseline 支持」的 `session_shell` 类能力标签。#6826 新增的 `multi_workspace_session_shell` 只表示 multi-workspace daemon 中 singular shell route 已按 live owner runtime 分发，且要求 shell 已有效启用；它不是通用 shell 支持探针。客户端仍无法仅靠 `/capabilities.features` 判断单 workspace daemon 是否支持 server-side shell，只能盲发 `POST /shell` 看结果。
...
3. **shell 端点无 baseline 能力标签**。`capabilities.ts` 有 `session_tasks`/`session_recap`/`session_btw`，但缺少通用 `session_shell`。#6826 的 `multi_workspace_session_shell` 只表达 owner-routing 能力，不等同于 baseline shell support；客户端无法用 `/capabilities` 预探测单 workspace daemon 的 server-side shell 支持，只能盲发。与「gate on features 而非 mode」的总契约不一致，应补一个 `session_shell`（或同名）always-on 标签。
