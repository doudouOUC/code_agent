...
这些端点没有 SSE delta；它们是 snapshot/export surface。`session_stats` 已经是能力标签，早期 README 把 `stats/export` 作为未落地整体描述已经过期：stats 已落地，primary export 由 #6297 落地，workspace-qualified active export 由 #6844 落地，workspace-qualified archived export 由 #6911 落地。

#6911 的 archived export 是独立只读面：`GET /workspaces/:workspace/session/:id/archive/export` 只查 selected trusted workspace 的 archived JSONL，不看 active session、不 fallback primary、不启动 ACP child。它复用 full export formatter、attachment header 与 shared archive lease；同一个 session id 同时存在 active 和 archive 时，archive route 只导出 archive 版本，客户端必须 gate `workspace_archived_session_export`。
...
## daemon file logger（异步队列、降级、截断、stable rotation）
