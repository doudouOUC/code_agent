...
| #7603 | fix(sdk-java): Harden daemon transport reliability | open | 不新增 daemon tag；Java client 消费 #7458 event epoch，并补 terminal-before-202、SSE/JSON malformed path 与 teardown ordering 的可靠性边界。 |
...
#7458 在这个语义上做 additive hardening：daemon 给每条 session event stream 下发 `eventEpoch`，客户端把 cursor 保存为 `(eventEpoch,lastEventId)`；重连时 epoch mismatch 不再靠数值大小猜测，直接进入 `state_resync_required{reason:'epoch_reset', detail:'epoch_mismatch'}`。`detail` 是附加诊断字段，老 SDK 忽略后仍能按 `reason` 走 snapshot/resync 恢复。
