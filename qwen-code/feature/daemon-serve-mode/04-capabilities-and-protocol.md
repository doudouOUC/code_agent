...
| #8572 | feat(daemon): Add SSE stream and client observability | open draft | 不新增 daemon tag；REST SSE response additive 返回 `X-Qwen-SSE-Stream-Id`，客户端可传 `connectReason`/`previousStreamId` 作为诊断线索，旧 daemon/代理剥离 header 时保持兼容。 |
...
8. **#8572/#8588 仍是 draft open diff**。SSE stream id、connect reason、previous stream lineage 与 health `activeWork` 只记录当前方案；若最终合并时 header/query 名、reason 枚举、private active-work heartbeat meta 或 deep health payload 调整，本文需要按最终 diff 再同步。
...
### #8572 — SSE stream diagnostics（draft open）
