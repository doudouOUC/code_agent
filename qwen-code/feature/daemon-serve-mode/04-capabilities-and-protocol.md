...
| #6525 | feat(serve): Add cursor-paged transcript replay endpoint | 2026-07-10 | 新增 `session_transcript`：`GET /session/:id/transcript` 分页返回 active persisted transcript replay frames。 |
...
| #6911 | feat(cli): Add archived session export | 2026-07-15 | 新增 `workspace_archived_session_export`：selected trusted workspace archived transcript full export，不从 active export 或 core REST capability 推断。 |
...
| #8415 | fix(serve): Coordinate caller-supplied session IDs | merged | 新增 `session_id_override` capability；REST/ACP/SDK 只有在该 tag 存在时才能发送 caller-supplied UUID session id，并必须验证 response id 是否 honor 请求。 |
...
| #10294 | feat(sdk): Add standalone session APIs | open | 当前 diff 消费 `standalone_sessions_v1`，为完整 lifecycle 增加严格 validators、explicit restore strategy 与 outcome-unknown exact recovery。 |
...
| #6743 | fix: Make chat recording failures durable and visible | 2026-07-12 | 新增 known event `recording_stopped`：recording durable write failure 后停止 recorder 并通知客户端。 |
| #6745 | feat(serve): support runtime workspace removal | 2026-07-13 | 新增 `workspace_runtime_removal` 和 workspace row `removable`，支持 removable secondary runtime hot removal。 |
| #6769 | feat(serve): Bound persisted transcript pages | 2026-07-13 | 给 workspace transcript route 新增 `transcript_page_too_large` 错误和 source/response/cursor byte budgets。 |
...
`CONDITIONAL_SERVE_FEATURES: ReadonlyMap<ServeFeature, (toggles: AdvertiseFeatureToggles) => boolean>` 把"该 tag 在什么开关下广告"的判定与 tag key 收敛到一处。`origin/daemon_mode_b_main@18e848f32` 的早期基线有 7 个条目，后续 mainline 又追加了 workspace/channel/multi-workspace 条件能力：
...
| `workspace_qualified_rest_core` | workspace-qualified REST deps 可用 | `/workspaces/:workspace/...` core REST 已注册 |
| `channel_reload` / `channel_control` | channel worker reload / runtime manager wired | channel worker reload 或 runtime selection control 可用 |
| `workspace_qualified_acp` | ACP HTTP enabled 且 multi-workspace sessions enabled | `/workspaces/:workspace/acp` 可用 |
| `workspace_qualified_voice` | ACP HTTP enabled 且 multi-workspace sessions enabled | `/workspaces/:workspace/voice` 与 workspace Voice WS 可用 |
| `persistent_workspace_registration` / `workspace_runtime_removal` | 对应 store/controller 可用 | dynamic workspace 持久化或 removable runtime hot removal 可用 |
| `multi_workspace_session_rewind` / `multi_workspace_session_shell` | multi-workspace sessions enabled；shell 还要求 shell enabled | singular rewind/shell route 已按 live owner runtime 分发 |
| `workspace_trust_hot_reload` | trust monitor/reconciler 与 generation guard wired | workspace trust 变更可在 daemon 内热重载，v2 status 暴露 `transition/failed/blocked` 状态 |

`AdvertiseFeatureToggles` 随这些条件能力扩展；老字段包括 `requireAuth` / `mcpPoolActive` / `allowOriginActive` / `promptDeadlineMs` / `writerIdleTimeoutMs` / `persistSettingAvailable`，近期 multi-workspace/channel 能力会继续在同一 toggle 结构里添加开关。#7019 后，docs contract test 会从开发文档中读取 conditional capability tag 表，并与 `CONDITIONAL_SERVE_FEATURES` 的实际 key 集合对齐；当前文档明确覆盖 27 个条件标签，避免新增/删除 tag 后只改代码、不改 protocol docs。
...
|---|---|---|---|
| `health`, `capabilities` | `GET /health`, `GET /capabilities` | 无 | `DaemonClient.health()`, `capabilities()`；#8588 已合入，deep health payload additive 增加 `activeWork`，客户端应把它当 restart/idle guard，而不是 liveness proof；#8691 已合入后，capabilities additive 增加 `limits.sessionRestoreTimeoutMs`，用于 SDK/WebUI restore timeout derivation |
...
| `session_load`, `unstable_session_resume` | `POST /session/:id/load`, `POST /session/:id/resume` | load 会重放历史帧；resume 不重放 UI 历史；#8691 已合入后，restore public timeout 返回 retryable `restore_timeout`，late settlement 负责 cleanup/quarantine；#9055 selective restore runtime 保持 internal/additive projection，不改变旧 wire | `loadSession()`, `resumeSession()`；TS SDK 用 per-request timeout、explicit fetch timeout、server restore budget + 10s、默认 70s 的顺序派生客户端计时 |
...
| `session_rewind`, `multi_workspace_session_rewind` | `GET /session/:id/rewind/snapshots`, `POST /session/:id/rewind` | `session_rewound` | `getRewindSnapshots()`, `rewindSession()`；#6826 后 multi-workspace daemon 中按 live owner runtime 分发，SDK rewind 强制走 REST |
...
| `workspace_acp_status`, `workspace_acp_preheat` | `GET /workspace/acp/status`, `POST /workspace/acp/preheat?timeoutMs=N` | 无；status 不创建 channel，preheat 与 Session 创建共享初始化；`ready === channelLive`，timeout 只结束当前 HTTP waiter | `workspaceAcpStatus()`, `workspaceAcpPreheat(timeoutMs?)`，均强制 native REST；WebUI deferred connect 只在 selected workspace 精确等于 primary 时调用 |
...
| `workspace_trust_hot_reload` | `GET /workspace/trust?statusVersion=2`, `POST /workspace/trust/request`；workspace-qualified trust status/request 同步使用 selected runtime | trust generation change 触发 runtime close/drain/recreate；transition/failed/blocked fail-closed，不 fallback primary | SDK trust status v2 types/helper；Web Shell applying/failed polling 与 trust prompt gating |
...
| `workspace_archived_session_export` | `GET /workspaces/:workspace/session/:id/archive/export` | 无，直接返回 attachment；active-only id 返回 `session_not_archived` | `WorkspaceDaemonClient.exportArchivedSession()` |
| `workspace_persisted_transcript` | `GET /workspaces/:workspace/session/:id/transcript` | 无；返回 selected workspace active persisted transcript replay page；#6769 增加 `transcript_page_too_large` | `WorkspaceDaemonClient.getSessionTranscriptPage()` |
| `workspace_session_export` | `GET /workspaces/:workspace/session/:id/export` | 无；selected trusted workspace full export，不 fallback primary、不启动 ACP | `WorkspaceDaemonClient.exportSession()`，强制 native REST |
| `channel_reload` | `POST /workspace/channel/reload` | 无；HTTP 返回 post-reload worker snapshot | `reloadChannelWorker()` |
| `channel_control` | `GET/PUT/DELETE /workspace/channel`，reload 仍是 `POST /workspace/channel/reload`（#6741） | 无；HTTP 返回 runtime selection / worker snapshot | daemon channel control helpers；CLI `qwen channel set/status/stop` |
| `workspace_qualified_acp` | `POST/GET/DELETE /workspaces/:workspace/acp`，以及同 path WebSocket upgrade | ACP SSE/WS 按 selected runtime dispatcher 输出；legacy `/acp` 保持 primary | ACP transport client 使用 workspace-qualified URL |
| `workspace_qualified_voice` | `GET/POST /workspaces/:workspace/voice`、`POST /workspaces/:workspace/voice/transcribe` 与 workspace Voice WS | selected runtime Voice settings/env/transcription；进程级 admission lease 与 runtime removal activity | `WorkspaceDaemonClient` Voice status/update/transcribe helpers；streaming WS 按 selected runtime |
| `persistent_workspace_registration` | `POST /workspaces {persist:true}`、`GET /workspace-registrations`、`DELETE /workspace-registrations/:id` | dynamic workspace desired-state 写入 user-level store；delete 不卸载 active runtime | `addWorkspace(cwd, {persist:true})` |
| `workspace_runtime_removal` | `DELETE /workspaces/:workspace`（#6745） | runtime drain/remove side effects；不删除 workspace files/settings/transcripts | workspace removal SDK/WebUI helpers，基于 `workspaces[].removable` gate |
| `workspace_extensions`, `extension_management_v2` | legacy `GET /workspace/extensions`；V2 `GET/POST /extensions/*` 与 `GET/PUT/DELETE/POST /workspaces/:workspace/extensions/*`（#6825） | `extensions_changed` / workspace runtime refresh side effects；V2 operation status 走 `/extensions/operations/:operationId`，post-commit warning 可见 | legacy workspace extension diagnostics；V2 `extensionCatalog()`、install/check/update/delete/default activation、operation polling、workspace activation/refresh helpers |

注意：`prompt_absolute_deadline` / `writer_idle_timeout` / `workspace_settings` / `workspace_qualified_rest_core` / `channel_reload` / `channel_control` / `workspace_qualified_acp` / `workspace_qualified_voice` / `persistent_workspace_registration` / `workspace_runtime_removal` / `multi_workspace_session_rewind` / `multi_workspace_session_shell` / `workspace_trust_hot_reload` 是条件 tag。前两者只在配置了正数 deadline/idle timeout 时广告；#7400 后 `prompt_absolute_deadline` 的语义是 bridge 在 admission 点 arm deadline，覆盖 queue wait + execution，并在超时时发布 `turn_error{code:'prompt_deadline_exceeded'}`、释放 FIFO，而不是 HTTP route 返回 504。`workspace_settings` 与 `workspace_qualified_rest_core` 只在 host 注入 `persistSetting` 且对应路由注册时广告；`channel_reload` 只在 channel worker snapshot 与 reload deps 都被 wire 时广告；`channel_control` 只有 runtime manager wired 时广告；`workspace_qualified_acp` 与 `workspace_qualified_voice` 只有 ACP HTTP enabled 且 multi-workspace sessions enabled 时广告；`persistent_workspace_registration` 只有 workspace registration store 注入/可用时广告；`workspace_runtime_removal` 只有 removal controller 安装时广告；`multi_workspace_session_rewind` 随 multi-workspace sessions 启用广告，`multi_workspace_session_shell` 还要求 session shell 可用；`workspace_trust_hot_reload` 只有 trust monitor/reconciler/generation guard wired 时广告。`workspace_persisted_transcript` 和 `workspace_session_export` 是 workspace-qualified read/export tag，但实际读取仍按 workspace/trust policy 校验；`extension_management_v2` 是 #6825 的 additive tag，表示 user-level artifact + workspace activation policy 的 V2 管理面，具体 mutation 结果通过 operation status/warnings 表达；旧 `workspace_extensions` 仍只是 legacy 诊断/compatibility surface。

#7200 的 `workspace_acp_status` / `workspace_acp_preheat` 是 baseline v1 capability tag，不进入 `CONDITIONAL_SERVE_FEATURES`。它们只声明 primary workspace 的 legacy ACP status/preheat 路由可发现；workspace-qualified ACP preheat/status 不是该 PR 的契约，客户端不能把它们推广到 `/workspaces/:workspace/acp`。

`recording_stopped`（#6743）不是 capability，而是 daemon known event：当 chat recording durable append 失败并永久停止 recorder 时发出。payload 只暴露 failed session id 和通用停止语义，不泄漏本地 path/errno；SDK/WebUI normalizer 应把它渲染为 session status/warning，而不是未知 debug block。
...
- #7019 追加 docs contract test：文档中列出的 conditional capability tags 必须与 `CONDITIONAL_SERVE_FEATURES` key 集合一致，防止 protocol 文档继续停留在旧 7/13 项口径。它不替代代码级谓词测试，而是覆盖“代码已变、文档没变”的漂移面。
...
1. **epoch_reset**（L401，#7458 前的数值 heuristic）：`opts.lastEventId >= this.nextId`——游标来自已死 epoch（daemon 重启使 `nextId` 归 1、ring 清空）。`forcePush` 一个 `state_resync_required{ reason:'epoch_reset', lastDeliveredId, earliestAvailableId: ring[0]?.id ?? nextId }`，且 `replayFrom = 0`（全量重放当前 ring，否则用 stale `lastEventId=50` 过滤会把新 epoch 的低 id 帧 1,2,3 全丢）。
...
#7458 在这个语义上做 additive hardening：daemon 给每条 session event stream 下发 `eventEpoch`，客户端把 cursor 保存为 `(eventEpoch,lastEventId)`；重连时 epoch mismatch 不再靠数值大小猜测，直接进入 `state_resync_required{reason:'epoch_reset', detail:'epoch_mismatch'}`。`detail` 是附加诊断字段，老 SDK 忽略后仍能按 `reason` 走 snapshot/resync 恢复。#7619 补 route-level regression，确保 `session/load` response 不丢 `eventEpoch` / `replayDegraded`。#7622 继续沿用同一 additive frame type，在 replay byte budget 超限时发送 `reason:'replay_budget_exceeded'`，让客户端复用 snapshot/resync 恢复路径。
...
5. **`protocolVersions` 仅 `'v1'`，多版本路径未经实战**。`isFeatureAvailableInProtocol` 的版本裁剪逻辑当前恒真，`'v2'` 引入前其正确性只有单测覆盖、无线上验证。

6. **W23/W24 新增诊断与控制能力继续扩大列表**。#4820 新增 `session_rewind`，#4822 新增 `workspace_hooks` / `session_hooks`，#4832 新增 `workspace_extensions`，#4816 新增条件能力 `workspace_settings`（仅 `persistSettingAvailable` 时广告），#4812 新增 `session_branch`（always-on），#4861 新增条件能力 `rate_limit`（仅 `toggles.rateLimit === true` 时广告）。集成测试 baseline 需同步。
...
8. **#9513/#9665/#9687/#9763/#9819/#9820/#9838/#9933/#9976/#9978/#10179 已合入；#9626/#10294 是 open**。standalone public capability 已由 #10179 合入；storage conflict repair 与 standalone TypeScript SDK 仍只能记录当前方案。#9978 本身明确没有 `standalone_sessions_v1`，#10179 新增该 capability，#10294(open) 仅作为 capability-gated consumer；#8691/#9042/#9055/#9134/#9181/#9261/#9362/#9380/#9396 已按 merged diff 更新。
...
| `acp-bridge/src/eventBus.test.ts` | `state_resync_required`（epoch_reset/ring_evicted）+ `replay_complete` 顺序（与 `qwen-serve-baseline.test.ts` 的 SSE 背压帧序镜像，#4245 一并修过）；#7458 继续覆盖 eventEpoch mismatch 与 degraded replay。 |
...
- `eventBus.ts:subscribe`：`state_resync_required`——`epoch_reset`（#7458 前用 `lastEventId >= nextId`，#7458 后优先用 `eventEpoch` mismatch；`replayFrom=0`）与 `ring_evicted`（`earliestInRing > lastEventId+1`）两条 gap 检测。
...
### #8691 — restore timeout limits and errors（merged）
...
### #8743 / #9055 — selective restore design to runtime

- #8743 不新增 public feature tag、capabilities field、REST/SDK request/response 字段或 SSE event type；#9055 runtime 实现仍保持 additive compatibility。
- recent/all/none replay、anchor、`hasMore`、`partial` 和 bounded replay error 先保持 daemon 内部 projection 语义；只有未来需要对外暴露时才作为 additive field 单独审计。
...
### #9181 — Conversations runtime boundary（merged）

- #9181 不新增 standalone route、public capability、SDK surface 或 event type；能力面仍通过现有 Live/owner-routed compatibility path 暴露。
...
### #9341 / #9362 / #9512 / #9513 / #9626 / #9665 / #9687 / #9763 — standalone/runtime safety primitives
...
### #9978 / #10179 / #10294 — standalone internal core、public capability 与 SDK

#9978 已合入 internal `StandaloneSessionService`，但刻意没有 public route、`standalone_sessions_v1`、SDK 或 UI。#10179 已在完整 standalone service/runtime dependency graph 安装时条件广告 `standalone_sessions_v1`，并注册 `/standalone/sessions` lifecycle route；partial bridge、embedded runtime 或 service 缺失时不暴露半套能力。#10294 当前 open diff 才提供 TypeScript SDK consumer；旧 daemon 缺 capability 时必须在发 route 前失败，不能试探调用或 fallback workspace。
