...
| #6638 | feat(serve): add extension management v2 | open | 当前 open diff 新增 additive `extension_management_v2`；不采用早期 workspace-qualified extensions 方案名，legacy `workspace_extensions` 保持 primary compatibility。 |
...
| `workspace_extensions`, `extension_management_v2` | legacy `GET /workspace/extensions`；V2 `GET/POST /extensions/*` 与 `GET/PUT/DELETE/POST /workspaces/:workspace/extensions/*`（#6638 open） | `extensions_changed` / workspace runtime refresh side effects；V2 operation status 走 `/extensions/operations/:operationId` | legacy workspace extension diagnostics；V2 `extensionCatalog()`、install/check/update/delete/default activation、workspace activation/refresh helpers |

注意：`prompt_absolute_deadline` / `writer_idle_timeout` / `workspace_settings` / `workspace_qualified_rest_core` / `channel_reload` / `channel_control` / `workspace_qualified_acp` / `persistent_workspace_registration` / `workspace_runtime_removal` 是条件 tag。前两者只在配置了正数 deadline/idle timeout 时广告；`workspace_settings` 与 `workspace_qualified_rest_core` 只在 host 注入 `persistSetting` 且对应路由注册时广告；`channel_reload` 只在 channel worker snapshot 与 reload deps 都被 wire 时广告；`channel_control` 只有 #6741 open diff 的 runtime manager wired 时广告；`workspace_qualified_acp` 只有 ACP HTTP enabled 且 multi-workspace sessions enabled 时广告；`persistent_workspace_registration` 只有 workspace registration store 注入/可用时广告；`workspace_runtime_removal` 只有 #6745 open diff 的 removal controller 安装时广告。`workspace_persisted_transcript` 是 #6740 的 unconditional read-only tag，但实际读取仍按 workspace/trust policy 校验；`extension_management_v2` 是 #6638 open diff 的 additive tag，表示 user-level artifact + workspace activation policy 的 V2 管理面；旧 `workspace_extensions` 仍只是 legacy 诊断/compatibility surface。
