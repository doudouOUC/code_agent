...
`AdvertiseFeatureToggles` 随这些条件能力扩展；老字段包括 `requireAuth` / `mcpPoolActive` / `allowOriginActive` / `promptDeadlineMs` / `writerIdleTimeoutMs` / `persistSettingAvailable`，近期 multi-workspace/channel 能力会继续在同一 toggle 结构里添加开关。
