...
| #6741 | feat(cli): Add runtime daemon channel control | open | 当前 open diff 新增条件能力 `channel_control`：daemon runtime channel selection 查询/设置/停止。 |
...
| #6745 | feat(serve): support runtime workspace removal | open | 当前 open diff 新增 `workspace_runtime_removal` 和 workspace row `removable`，支持 removable secondary runtime hot removal。 |
| #6769 | feat(serve): Bound persisted transcript pages | open | 当前 open diff 给 workspace transcript route 新增 `transcript_page_too_large` 错误和 source/response/cursor byte budgets。 |
...
| `workspace_persisted_transcript` | `GET /workspaces/:workspace/session/:id/transcript` | 无；返回 selected workspace active persisted transcript replay page；#6769 open 增加 `transcript_page_too_large` | `WorkspaceDaemonClient.getSessionTranscriptPage()` |
...
| `channel_control` | `GET/PUT/DELETE /workspace/channel`，reload 仍是 `POST /workspace/channel/reload`（#6741 open） | 无；HTTP 返回 runtime selection / worker snapshot | daemon channel control helpers；CLI `qwen channel set/status/stop` |
...
| `workspace_runtime_removal` | `DELETE /workspaces/:workspace`（#6745 open） | runtime drain/remove side effects；不删除 workspace files/settings/transcripts | workspace removal SDK/WebUI helpers，基于 `workspaces[].removable` gate |
