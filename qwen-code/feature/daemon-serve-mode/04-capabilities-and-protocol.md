...
| #7458 | fix(serve): detect stale SSE cursors via epoch token | open | 不新增 frame `v`；在 REST/SSE metadata 下发 `eventEpoch`，客户端重连回传后以 `state_resync_required{detail:'epoch_mismatch'}` 表达 daemon restart stale cursor。 |
| #7463 | feat(sdk-java): Add daemon transport | open | 不新增 daemon tag；Java daemon transport alpha 消费现有 HTTP/SSE 协议，并把 admission watermark、event cursor、terminal correlation 和 fail-closed exception taxonomy 固化为 JVM client surface。 |
...
#7458 当前 open diff 在这个语义上做 additive hardening：daemon 给每条 session event stream 下发 `eventEpoch`，客户端把 cursor 保存为 `(eventEpoch,lastEventId)`；重连时 epoch mismatch 不再靠数值大小猜测，直接进入 `state_resync_required{reason:'epoch_reset', detail:'epoch_mismatch'}`。`detail` 是附加诊断字段，老 SDK 忽略后仍能按 `reason` 走 snapshot/resync 恢复。
...
| `acp-bridge/src/eventBus.test.ts` | `state_resync_required`（epoch_reset/ring_evicted）+ `replay_complete` 顺序（与 `qwen-serve-baseline.test.ts` 的 SSE 背压帧序镜像，#4245 一并修过）；#7458 当前 open diff 继续覆盖 eventEpoch mismatch 与 degraded replay。 |
...
- `eventBus.ts:subscribe`：`state_resync_required`——`epoch_reset`（#7458 前用 `lastEventId >= nextId`，#7458 当前 open diff 优先用 `eventEpoch` mismatch；`replayFrom=0`）与 `ring_evicted`（`earliestInRing > lastEventId+1`）两条 gap 检测。
