...
| #5035 | fix(daemon): push session title updates | 2026-06-09 | 不新增 tag；ACP child 自动标题经 side channel 转成既有 `session_metadata_updated`。 |
| #5098 | feat(web-shell): persist `/goal` status via daemon transcript events | 2026-06-11 | 不新增 tag；goal 状态进入 transcript block `_meta.goalStatus`，刷新/多 tab 可恢复。 |
| #5175/#5266 | feat/fix(web-shell): mid-turn message injection | 2026-06-16/18 | 不新增 tag；新增 `mid_turn_message_injected` 事件与 `POST /session/:id/mid-turn-message` 语义，#5266 集中常量并补 drain timeout recovery。 |
| #5174 | feat(cli): Add daemon status API | 2026-06-16 | `daemon_status` baseline capability + `GET /daemon/status` summary/full JSON 诊断面。 |
| #5211 | test(serve): update daemon status integration baseline | 2026-06-17 | 把 `daemon_status` 纳入集成能力基线，并让 E2E 在 PR 上运行，降低 registry/E2E 镜像漂移。 |
...
| #5398 | feat(web-shell): manage extensions from web shell | 2026-06-20 | 扩展 `workspace_extensions` 从诊断快照到管理面；新增 extension mutation endpoints、`extensions_changed` 事件和 SDK/UI 消费。 |
| #5504 | feat(acp): support model-invocable commands in daemon sessions | 2026-06-21 | 不新增 serve tag；ACP `available_commands_update` 驱动 daemon session 注册 model-invocable command provider/executor。 |
| #5544 | feat(mcp): support MCP resources and prompts | 2026-06-22 | 不新增 serve tag；MCP resource / prompt discovery 进入 pooled/ACP session 能力面。 |
...
| `session_context`, `session_context_usage`, `session_supported_commands`, `session_tasks`, `session_stats` | `GET /session/:id/context`, `/context-usage`, `/supported-commands`, `/tasks`, `/stats` | 无，均为 snapshot | `context()`, `contextUsage()`, `supportedCommands()`, `tasks()`, `stats()` |
...
| `workspace_hooks`, `session_hooks` | `GET /workspace/hooks`, `GET /session/:id/hooks` | 无，诊断 snapshot；#4834 让 webui 聚焦消费 hooks 状态 | `workspaceHooks()`, `sessionHooks()` |
| `workspace_extensions` | `GET /workspace/extensions`；#5398 后 extension mutation endpoints | `extensions_changed`；诊断 snapshot 仍可只读消费 | `workspaceExtensions()` + extension management helpers |
...
注意：`prompt_absolute_deadline` / `writer_idle_timeout` / `workspace_settings` 是条件 tag。前两者只在配置了正数 deadline/idle timeout 时广告；`workspace_settings` 只在 host 注入 `persistSetting` 且路由注册时广告。
...
#5035 复用既有 `session_metadata_updated`，没有新增事件类型或能力 tag。协议上真正新增的是 ACP child → bridge 的项目私有通知 `qwen/notify/session/title-update`；REST/SSE 客户端只看到标准 daemon metadata event，所以老客户端最多“不知道这是自动标题”，不会丢渲染能力。

#5098 把 `/goal` 状态放进 daemon transcript block 的 `_meta.goalStatus`，也没有新增 `/capabilities` tag。它属于 transcript payload 形状的 additive metadata：旧 SDK 忽略 `_meta`，新 web-shell 可在刷新、多 tab、replay prompt-status 恢复时重建 goal 面板。

#5175/#5266 的 mid-turn 注入同样未加 feature tag。客户端兼容策略是：支持该 UI 的 web-shell 调 `POST /session/:id/mid-turn-message`，老 daemon 返回 404/不支持时回退到下一轮普通 prompt；收到 `mid_turn_message_injected` 后再把输入标为已注入。#5266 把事件常量集中，减少 SDK/web-shell/bridge 的字符串漂移。

#5398 把 `workspace_extensions` 从只读诊断拓展成“诊断 + 管理”。这里的兼容性风险不是 tag 缺失，而是 tag 名没有区分 read-only 与 mutation；客户端需要对 mutation endpoint 做错误降级，并监听 `extensions_changed` 刷新 workspace/session 资源视图。

#5504 属于 ACP session 行为补齐，而非 serve capability registry 变更：daemon session 在收到 `available_commands_update` 后，如果 settings 可用，会注册 model-invocable command provider/executor，避免模型可调用命令在 daemon/ACP 路径缺失。它不应被写进 REST feature tag 矩阵。
...
| #4284 | E2E baseline | #4249（`workspace_memory`/`agents`）+ #4269（`workspace_file_read`）；并提及更早的 #4268（`mcp_guardrails`） | +3 |
...
5. **`serverTimestamp` 落点与注释不一致**。`server.ts:~3664` 注释称 SDK 三处回落探测"planned in #4353 (not yet merged into `daemon_mode_b_main`)"，但本分支 `normalizer.ts:extractServerTimestamp` 已实现该三处探测——daemon 侧注释滞后于 SDK 侧实现（跨 PR 时序错位）。同时 daemon top-level `_meta` merge 在生产中恒为 no-op（无 producer 设顶层 `_meta`），真实落点是嵌套 `data._meta`。
...
### #5035 — title update over existing metadata event

- ACP child 发 `qwen/notify/session/title-update` side channel，bridge 转成 `session_metadata_updated`。
- 不新增 capability tag，也不扩 ACP 标准 `SessionUpdate` union；REST/SSE 客户端按既有 metadata event 处理。

### #5098 — goal status transcript metadata

- `/goal` 状态写入 daemon transcript block `_meta.goalStatus`。
- 不新增 event type/tag；旧客户端忽略 `_meta`，新 web-shell 从 replay/live transcript 恢复 goal 状态。
...
- #5211 把 `daemon_status` 加进集成 E2E baseline，并让相关 E2E 在 PR 上运行，降低 registry 与外部 wire-shape 镜像漂移。

### #5175/#5266 — mid-turn injected event

- #5175 新增 `mid_turn_message_injected` daemon event，作为 mid-turn queue 被 ACP child drain 后的确认帧。
- #5266 将事件名集中到 `daemonEventTypes.ts` 并由 SDK re-export，避免 bridge/web-shell/SDK 多处手写字符串。
- 当前未注册 capability tag；客户端以 endpoint 404/事件缺失做降级。

### #5398 — extensions_changed

- `workspace_extensions` surface 从只读扩展为管理面，新增 `extensions_changed` workspace mutation event。
- SDK/web-shell 监听该事件后刷新 extension snapshot、commands/hooks/settings 等派生资源。
- tag 未区分 read-only 与 mutation，客户端必须对 mutation endpoint 做错误降级。

### #5504 — ACP model-invocable commands

- ACP daemon session 消费 `available_commands_update`，在 settings 可用时注册 model-invocable command provider/executor。
- 属于 ACP session 行为补齐，不进入 REST `/capabilities.features`。
