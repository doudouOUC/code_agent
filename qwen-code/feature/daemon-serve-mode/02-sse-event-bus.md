...
**replay 回放**（L443-456）：遍历整个 `ring`，对 `e.id > replayFrom` 的帧 `forcePush`（绕过 cap），累计 `replayedCount` 与 `lastReplayedId`。然后 `forcePush` 一个 `replay_complete` sentinel（L475），让消费者**确定性**地把"追赶中"指示器切到"live"（无论实际回放了几帧，甚至 0 帧也发，给出 catching-up→live 的正向信号）。字段 `lastReplayedEventId` 是 #4484 D4 重命名后的规范名（旧 `lastEventId` 在裸 trace 里与 SSE 协议的 `Last-Event-ID` 语义撞车），同时保留 `lastEventId` 别名向后兼容；`replayedCount` 取循环实计数而非 `lastId - lastEventId`（ring 有洞时后者会高估）。
...
2. **EventBus 仍私有于 SSE 路由**（`eventBus.ts:150` 的 `FIXME(stage-1.5)`，chiga0 finding 2）。计划抬成顶层 `packages/event-bus`，让 `channels/`/`dualOutput/`/`remoteInput/`/未来 TUI co-host 与 WebSocket transport 共享同一总线，而非各跑并行事件流。`BridgeEvent` 形状已接近所需，缺的是 bus 可被公开寻址。

3. **serverTimestamp 的 SDK 读取端未落地**（`server.ts:3664` doc）。3 处回落探测计划在 chiga0 的独立 PR #4353，尚未并入 `daemon_mode_b_main`；当前该戳仅 daemon 侧可见，SDK 需 `as any` 或等 #4353。
