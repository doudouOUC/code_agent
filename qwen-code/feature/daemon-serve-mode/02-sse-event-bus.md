...
| [#7458](https://github.com/QwenLM/qwen-code/pull/7458) | W30 open | 为每个 EventBus mint 显式 `eventEpoch`，通过 REST/SSE headers 与客户端重连回传检测 daemon restart 后的 stale cursor；compaction replay 保留 attribution，并在 ingest failure 后暴露 degraded snapshot。 |
...
| `eventEpoch` | #7458 open | EventBus 构造时生成的不可复用 stream epoch token；不进入每个 `BridgeEvent` payload，而是经 REST response 与 SSE header 下发，供客户端把 cursor 解释为 `(epoch,lastEventId)` |
...
### 显式 eventEpoch（#7458 当前 open）

原始 `epoch_reset` 只能用 `lastEventId >= nextId` 猜测 daemon restart。如果新 daemon 已经产生足够多事件，让旧 cursor 小于新的 `nextId`，数值检查会误判为同一 epoch 内的合法续传。#7458 的当前实现给每个 EventBus 创建一个不可复用 `eventEpoch`，并在以下边界下发：
