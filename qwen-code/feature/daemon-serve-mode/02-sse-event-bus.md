...
| [#8572](https://github.com/QwenLM/qwen-code/pull/8572) | W32 draft open | 为每条 REST SSE stream 增加 daemon 分配的 stream UUID、客户端 connect reason / previous stream lineage、EventBus subscriber diagnostics callback，以及 open/resync/slow-client/eviction/close telemetry。 |
...
原始 `epoch_reset` 只能用 `lastEventId >= nextId` 猜测 daemon restart。如果新 daemon 已经产生足够多事件，让旧 cursor 小于新的 `nextId`，数值检查会误判为同一 epoch 内的合法续传。#7458 的最终实现给每个 EventBus 创建一个不可复用 `eventEpoch`，并在以下边界下发：

- create/load/resume response；
- non-blocking prompt 202 envelope；
- `GET /session/:id/events` 的 `X-Qwen-Event-Epoch` response header。
...
#8572 draft open diff 把这条观测升级为 stream-scoped telemetry：SSE route 先给 accepted stream 生成 `X-Qwen-SSE-Stream-Id`，再把 resync reason/detail/gap、slow-client diagnostics、EventBus eviction、writer idle、socket error 和最终 close reason 与同一 stream id 关联。close finalizer 还记录 settled frame count、last written event id、backpressure count、max drain wait 和 live publish-to-write-settled lag。它只在 REST SSE route 层处理，EventBus 仍不持有 client identity、HTTP headers 或 daemon logger。
...
3. **顶层 `_meta` merge 当前是 no-op**。无 producer 在 envelope 顶层设 `_meta`（ToolCallEmitter 的 `_meta` 嵌在 `data._meta`），`formatSseFrame` 的顶层 spread 是 forward-compat 逃逸口而非现网生效路径。

4. **`maxQueued` 上界 2048 与 ring 8000 不对称**。一次跨度 >2048 的非 forced backlog 不可能（forced replay 绕过 cap），但若 live 帧产出持续快于慢消费者排空，仍会在打满 2048 时被 evict——这是设计意图（保护 daemon），但 operator 需理解 `?maxQueued` 调高只是推迟而非消除 evict。
...
- #8572 draft open diff 另覆盖 `onSubscriberDiagnostic` 被调用、handled 时 suppress legacy log、throw/false 时保留 fallback，以及 trigger event type/bytes 不污染 EventBus frame payload。
...
- #8572 draft open diff 补 stream id header、connect reason/previous stream id parsing、opened/closed telemetry、slow-client diagnostic、EventBus eviction、socket error、writer idle 与 close reason attribution 的 focused SSE route tests。
...
- `eventBus.ts:subscribe`：`state_resync_required` 帧——`epoch_reset`（#7458 前为 `lastEventId >= nextId` heuristic，#7458 后优先用 `eventEpoch` mismatch）与 `ring_evicted`（`earliestInRing > lastEventId + 1`）两条检测路径。
...
- `routes/sse-events.ts`：accepted stream 生成 UUID header，关联 connect reason、previous stream id、client id、resync gap、slow warning、eviction 与 close telemetry。
- `RestSseTransport` / `DaemonSessionClient` / WebUI provider：传播 stream id lineage 与有限 connect reason，旧 daemon 或 stripped header 继续按原 SSE cursor 工作。
