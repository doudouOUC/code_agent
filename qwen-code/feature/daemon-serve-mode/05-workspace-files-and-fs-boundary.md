...
能力分三档（`/capabilities` 的 `features[]`）：`workspace_file_read`（`GET /file|/list|/glob|/stat`，#4269）、`workspace_file_bytes`（`GET /file/bytes`，#4280）、`workspace_file_write`（`POST /file/write|/file/edit`，strict 门控，#4280）。#8002 当前 open diff 另以 additive `workspace_file_read_cursor` 标记 text cursor paging，旧客户端不传 `cursor` 时仍走原窗口读取。
...
| #8002 | byte-cursor paging（open） | 当前 open diff 为 Serve `/file` 增加 `hasMore`/`nextCursor`/`cursor`，用 snapshot-bound byte cursor 支持大文本连续翻页。 |
...
| `GET /file` | `cursor` | opaque string | #8002 当前 open diff；与 `line` 互斥，畸形值返回 `parse_error` |
...
4. **#7967/#8002 仍为 open**。handle-bound range refactor 与 byte-cursor paging 只记录当前 open diff；尚不能视为 `main` 已落地能力。#7947 已合入，但仍只放行 UTF-8 bounded line-window，full snapshot、edit、hash 和 optimistic locking 继续保留 256 KiB 门。
...
| #8002 current open focused suites | 177 CLI + 95 Core + 405 TS SDK | Cursor parse/encode、append-only continuation、replace/truncate mismatch、BOM/CRLF metadata、long-line cap、SDK/MCP additive fields。 |
