...
F2（#4336）的核心是 `McpTransportPool`（`mcp-transport-pool.ts:104`），挂在 `QwenAgent.mcpPool`（`acpAgent.ts:523`）——早期单 workspace daemon 中等价于 per-workspace 单例；multi-workspace 后，每个 isolated workspace runtime / ACP 子进程拥有自己的 pool 与 budget scope。session 不再各自 `new McpClient`，而是通过 `McpClientManager.discoverAllMcpToolsViaPool`（`mcp-client-manager.ts:1425`）向池 `acquire`。
