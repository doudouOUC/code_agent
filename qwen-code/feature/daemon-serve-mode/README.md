...
### W28 2026-07-06 ~ 2026-07-09 daemon / serve follow-up
...
| #6525 | cursor-paged transcript（open） | `GET /session/:id/transcript` 从 active persisted JSONL 冻结快照分页返回 id-less replay frames，不改 live EventBus；cursor HMAC 防篡改，超大 snapshot 结构化拒绝。 |
...
| #6558 | persisted secondary session list | trusted non-primary session list 合并 active persisted sessions 与 live summaries；unsupported archived/organized/grouped view 仍拒绝。 |
...
| #6621 | workspace-qualified ACP（open） | `/workspaces/:workspace/acp` open 方案：per-runtime ACP dispatcher/connection registry/device-flow/client-MCP 绑定，legacy `/acp` 保持 primary。 |
...
| #6525 | cursor-paged transcript（open） | active persisted transcript 分页 replay endpoint open 方案，避免 `/load` 一次性返回全历史。 |
| #6540 | session owner index | registry owner index、trusted non-primary load/resume 与 route owner dispatch。 |
| #6558 | persisted secondary session list | trusted non-primary workspace 的 active persisted session list，仍不开放 archived/organized/grouped view。 |
...
| #6621 | workspace-qualified ACP（open） | `/workspaces/:workspace/acp` ACP transport open 方案，尚未合入。 |
