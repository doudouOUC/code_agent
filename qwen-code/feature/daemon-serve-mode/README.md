...
| #6825 | extension management v2（open） | 新增 `extension_management_v2`：user-level extension artifact + workspace activation policy，全局 `/extensions/*` 与 workspace projection routes 分离；`ExtensionStore` journal/recovery、operation queue、post-commit warning、revisioned settings bundle 和 SDK/TUI/Web Shell surface。 |
...
| #6825 | extension management v2（open） | user-level extension store、workspace activation policy、transaction journal/recovery、revisioned settings bundle、async operation queue 与 post-commit warning。 |
...
| #6833 | continue/language/artifact owner routing（open） | continue、language、artifact add/delete legacy route 按 owning runtime dispatch，保留既有 auth 和 response shape。 |
