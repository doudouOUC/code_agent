...
### W28 2026-07-06 ~ 2026-07-08 daemon / serve follow-up
...
| #6525 | cursor-paged transcript（open） | `GET /session/:id/transcript` 从 active persisted JSONL 冻结快照分页返回 id-less replay frames，不改 live EventBus。 |
| #6540 | session owner index（open） | registry-owned owner index、trusted non-primary load/resume、`session_workspace_conflict` 和 `/workspaces/:workspace/sessions` 复数别名；未合入。 |
...
| #6540 | session owner index（open） | registry owner index 与 trusted non-primary load/resume open 方案，尚未合入。 |
