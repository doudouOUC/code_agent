...
| 13 | [资源预算与公平调度](13-resource-budgeting.md) | #8093 closed draft 的 process-wide `ResourceBudget`、completion reserve、emergency pool、bulk/spawn/process fair scheduler、buffered process runner foundations，#8245 已合入的 daemon memory budget reporting，#8423 已合入的 memory pressure observe mode，#8462 已合入的 active ACP child RSS aggregate，#8508/#9380 已合入的 child heap partition/status measurement model，#8911 已合入的 daemon ACP NDJSON buffers、#8947 已合入的 ACP transport resource guard、#9007 已合入的 ACP HTTP pre-attach buffer byte budget、#11428 已合入的容量owner解耦、#11515 已合入的默认256注册/800 session/25 Channel owner边界、#11653已合入的无限cgroup哨兵修复、#11911/#11940/#12008已合入的count准入/自动回收/用户stop，以及#12265 open的fixed-heap校准证据 |
| 14 | [Linux 4c8g 容量实测](14-capacity-validation-linux-4c8g.md) | #11428/#11515 合入后在 4 vCPU / 8 GB Linux + 257 个真实 Git 仓库上的部署侧实测：注册规模阶梯、真实模型并发、重启/churn/空闲与cgroup授权；#11653/#11911/#11940/#12008已修复哨兵并补count恢复链，#12265 open补Node 24 heap证据，但per-child enforcement仍未启用 |
...
- **资源预算、数量准入与恢复分层（#8093 closed draft，#8245/#8423/#8462/#8508/#9380/#11428/#11515/#11653/#11911/#11940/#12008 merged，#12265 open）**：#8093 关闭前 draft 只提供 `ResourceBudget`、fair schedulers 与 buffered process runner primitives；#8245/#8423/#8462/#8508/#9380 建立 memory denominator、pressure、RSS 与 child heap 观测；#11428/#11515 分离并扩展 workspace/session/Channel owner，#11653拒绝无限或超宿主cgroup值。#11911把modeled child count接成显式`admit`准入，#11940在首次拒绝后最多回收一个零session、零activity warm child，#12008让用户确认停止loaded runtime并在registry证明释放后单次继续。三者仍不读取实时RAM或应用per-child heap ceiling；#12265只补fixed-heap校准证据，生产enforcement仍关闭。
...
| #12265 | ACP child fixed-heap calibration evidence | 当前open docs；发布8 runs/184 turns/360 tools的Node 24接受集与历史/失败证据，不改变production mode或heap argv。 |
...
_生成于 2026-06-03；按个人 PR 口径更新于 2026-09-20_
