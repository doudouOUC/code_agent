...
| 07 | [acp-bridge 抽包与多客户端权限协调](07-acp-bridge-and-permission.md) | 抽包 seam、四策略权限仲裁、并发不变量、same-host read/write delegation 能力分离（#8620 merged）、daemon ACP/HTTP resource guard（#8911/#8947/#9007 merged）、ACP channel liveness（#9976 merged）、process-tree ownership（#10142 merged）、new-session deadline/late cleanup（#10268 merged）、standalone private service/guard/public API（#9978/#10179 merged）、active-work close refusal/hold 上限（#9134/#9820 merged）、private current-session task creator/empty persistence（#9838/#10144 merged）、permission/AUQ timeout 默认 disabled（#9933 merged）、load/resume AUQ restore/hardening（#9665/#9763 merged）与 sessionless user-language fan-out（#10554 merged） |
...
| #10751 | session turn navigation protocol | 已合入 Phase 1 sparse turn index、HMAC snapshot、snapshot-bound transcript anchor、ACP vendor method 与 TS SDK；浏览器 rail尚未实现。 |
...
| #10828 | relaxed standalone daemon ownership design | 已合入 docs-only 设计：支持拓扑限定为同机同用户多个更新后 daemon，以 mandatory per-session lease、bridge attestation、Live publisher admission、controller binding 与 journal contention 约束替代未来的 global owner；runtime 尚未完整实现。 |
| #10924 | mandatory Conversations writer fences | 已合入私有 marker/forwarding attestation、强制 lease、identity-qualified local reclaim、batch writer error、generation-scoped pin与 unbound-task eligibility；外层 global owner仍保留。 |
...
| #11054 | separate-store headless turn navigation | 已合入 Phase 2A：历史 pages隔离到 external `HistoricalTranscriptPageTable`，导航 store统一 index、provisional、locator、anchored load、gap recovery与 degraded fallback；Phase 2B/3未交付。 |
...
8. **open/draft diff 不能写成 main 已落地能力**。#8093/#8824 为 closed draft，#9048/#10286/#10786/#11053/#11143 为 closed，只能作为历史前身或替代方案记录。#11015 仍为 open，只能作为当前 diff记录。#10828/#11020 已合入 docs-only design，但设计中的 runtime不能写成已落地；#11054/#11120及其他 merged PR按最终 diff更新。#11053/#11143的 flat-store ledger不能叠加到 #11054已合入的 separate-store架构。#8743 已由 #9055 runtime PR承接。
