...
| # | 子文档 | 覆盖 |
|---|---|---|
| 01 | [HTTP 服务 / 路由 / 中间件链](01-http-server-and-middleware.md) | 中间件链顺序、路由表、bearer / --require-auth / mutate / CORS / host allowlist 五道闸、prompt route 202 + bridge-owned deadline / 权限响应超时 / access log、REST SSE stream lifecycle diagnostics（#8572 merged）、restore timeout 504/retryable error mapping（#8691 merged）、`/health?deep=1.activeWork`（#8588/#9042 merged）与 `serve --open` ephemeral auth 设计（#9738 open docs-only） |
...
| #9738(open docs-only) | `serve --open` ephemeral auth | 当前仅设计 loopback interactive `--open` 的进程级 bearer 与 URL fragment→tab sessionStorage 交付，runtime 未实现。 |
...
| #9738(open docs-only) | `serve --open` ephemeral auth | 当前仅设计 generated bearer 与 fragment/sessionStorage handoff；未修改 runtime。 |
...
8. **open/draft diff 不能写成 main 已落地能力**。#8093/#8824 为 closed draft，#9048 为 closed，只能作为历史前身记录。#9626 为 open，#9738 为 open docs-only，只能作为当前方案记录。#9513/#9665/#9687 已按 merged diff 更新；#8743 已由 #9055 runtime PR 承接，不再按未落地 draft 追踪。其余 merged PR 继续以最终 diff 为准。
