...
| #6716 | persistent workspace registration（open） | dynamic workspace registration 可 opt-in 写入 user-level store，daemon 重启后恢复 secondary runtime，并提供 registration list/forget API。 |
...
| #6716 | persistent workspace registration（open） | dynamic workspace desired-state store、启动恢复与 registration list/forget API。 |
