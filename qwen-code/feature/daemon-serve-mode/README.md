...
| 08 | [扩展端点 recap/btw/tasks/shell/rewind/hooks/extensions/settings/logger](08-extension-endpoints.md) | 控制面端点、诊断端点、workspace skill status read model（#8080）、sessionless `POST /language`（#10554 open）、绕过 prompt FIFO、shell `this`-binding 隐患 |
...
| #10514(open) | standalone WebShell PR6 UI plan | docs-only 计划；定义全局/项目入口、Recents 生命周期、context deep link、项目控件隔离与 typed recovery，尚无运行时代码。 |
