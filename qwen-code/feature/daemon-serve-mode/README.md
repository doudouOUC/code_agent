...
| 13 | [资源预算与公平调度](13-resource-budgeting.md) | #8093 closed draft 的 process-wide `ResourceBudget`、completion reserve、emergency pool、bulk/spawn/process fair scheduler、buffered process runner foundations，#8245 已合入的 daemon memory budget reporting，#8423 已合入的 memory pressure observe mode，#8462 已合入的 active ACP child RSS aggregate，#8508/#9380 已合入的 child heap partition/status measurement model，#8911 已合入的 daemon ACP NDJSON buffers、#8947 已合入的 ACP transport resource guard、#9007 已合入的 ACP HTTP pre-attach buffer byte budget、#11428 已合入的容量owner解耦、#11515 已合入的默认256注册/800 session/25 Channel owner边界，以及#11653已合入的无限cgroup哨兵修复 |
| 14 | [Linux 4c8g 容量实测](14-capacity-validation-linux-4c8g.md) | #11428/#11515 合入后在 4 vCPU / 8 GB Linux + 257 个真实 Git 仓库上的部署侧实测：注册规模阶梯、真实模型并发、重启/churn/空闲与cgroup授权；发现#11591 Git状态开销及#8182 child授权分歧，其中无限哨兵一半已由#11653修复，raise-only与准入分歧仍保留 |
...
- **资源预算 foundation 与容量owner分层（#8093 closed draft，#8245/#8423/#8462/#8508/#9380/#11428/#11515/#11653 merged）**：#8093 关闭前 draft 只提供 `ResourceBudget`、fair schedulers 与 buffered process runner primitives，不接生产路由、不广告 capability，也不改变 daemon 行为；#8245/#8423 只补 status denominator/pressure observation；#8462 已把 active ACP child RSS aggregate 接入 status 观测；#8508 已发布 observe-only child heap partition model，#9380 已补 daemon-owned ACP child old-generation peak measurement。#11428先把workspace注册、child建模与Channel事务预算拆成独立常量owner；#11515随后把默认注册policy提升到256，并独立限制total session为800、Channel owner为25；#11653让spawn复用可用内存探测，拒绝无限/超宿主cgroup值，但仍不实现LRU、raise-only缺口或内存enforcement。
...
_生成于 2026-06-03；按个人 PR 口径更新于 2026-09-15_
