...
| # | 子文档 | 覆盖 |
|---|---|---|
| 01 | [HTTP 服务 / 路由 / 中间件链](01-http-server-and-middleware.md) | 中间件链顺序、路由表、bearer / --require-auth / mutate / CORS / host allowlist 五道闸、prompt route 202 + bridge-owned deadline / 权限响应超时 / access log、REST SSE stream lifecycle diagnostics（#8572 merged）、restore timeout 504/retryable error mapping（#8691 merged）与 `/health?deep=1.activeWork`（#8588 merged，#9042 merged 补 background shell hold） |
...
| 03 | [会话生命周期](03-session-lifecycle.md) | spawn/attach/close/delete、sessionScope single/thread、heartbeat、load/resume、session archive/unarchive、session organization、batch load replay、attach-ref ledger、prompt terminal exactly-once 与 follow-up hardening、session writer lease opt-in（#7894）、managed writer shutdown（#7812）、timestamp drift reconciliation（#7886）、Todo Stop Guard continuation hardening（#7821）、maintenance writer isolation（#7975）、certified writer handoff（#7976）、caller-supplied session id admission（#8415 merged）、restore timeout / late cleanup quarantine（#8691 merged）、selective session restore runtime（#9055 merged）、Conversations runtime foundation（#8890 merged）、Conversations hidden runtime ownership/lifecycle boundary（#9181 merged）、persisted session catalog cache 与 cancellation（#8892/#8954 merged）、workspace session live-state version/route（#9261 merged）、standalone source/identity primitives 与 hardening（#9341/#9512 merged）、PR2A restore regressions（#9513 open）、persisted storage lifecycle/conflict repair（#9626 open draft）、AUQ restore（#9665 open）、runtime record I/O retryability（#9362 merged）、live-state activity watermark 与 WebShell consumer（#9396/#9476 merged）、continuation admission logs（#8932 merged）、restore request shape fencing（#8933 merged）、repeated ACP tool failure guard（#8469 merged）、activeWork lifecycle gate（#8588 merged）、background shell activeWork hold（#9042 merged）与 active-work close authorization / close refusal follow-up（#9134 merged） |
| 04 | [能力注册表与协议](04-capabilities-and-protocol.md) | SERVE_CAPABILITY_REGISTRY、协议版本、typed event schema、协议补全、能力覆盖矩阵、workspace trust hot reload capability（#7268）、session_id_override capability（#8415 merged）、SSE stream diagnostics、session restore timeout limits/error taxonomy、selective restore pagination metadata（#9055 merged）、Conversations internal runtime visibility guard（#9181 merged）、standalone/runtime safety primitives（#9341/#9512/#9362 merged）与 PR2A restore regressions（#9513 open）、storage conflict repair capability（#9626 open draft）、AUQ restore flag/no-capability boundary（#9665 open）、workspace session live-state capability/route（#9261 merged）、ACP child heap status field（#9380 merged）、live-state activity watermark（#9396 merged）与 activeWork health additive wire 字段（#8572 merged / #8691 merged / #8588 merged / #9042 merged） |
...
| 11 | [WebUI 库与 ACP 传输层](11-webui-and-transport.md) | @qwen-code/webui、context-usage API、ACP Streamable HTTP、WebSocket transport、trust hot reload applying/failed UI state、workspace-scoped Web Shell Voice（#7754）、live journal repair（#8414 merged）、ACP textual tool-result projection（#8450 merged）、WebUI SSE reconnect reason（#8572 merged）、restore request/watchdog timeout（#8691 merged）、same-id attachment stale-work fencing（#8833 merged）、shared session catalog scheduling（#8891 merged）、prompt-safe navigation（#8931 merged）、restore request shape fencing（#8933 merged）、transactional cross/same-session switching（#8882/#8939 merged）、prompt admission ownership hardening（#8955 merged）、same-session refresh race closeout（#8990 merged）、transactional resync/live-journal repair（#9048 closed observation）、selective restore runtime（#9055 merged）、Web Shell text file attachments（#9180 merged）、workspace session live-state route/handshake/consumer（#9261/#9366 merged）、activity timestamp consumer（#9396/#9476 merged）、internal Conversations runtime visibility guard（#9181 merged）与 ACP HTTP pre-attach budget（#9007 merged） |
...
| #9513(open) | PR2A restore regressions | 当前 open diff 只让同拼写 active+archived 双副本 load/resume 选 active，并修复 metadata race、legacy parent、目录消失和重复 catalog scan；异拼写 conflict 与 mutation 继续 fail-closed。 |
| #9563 | WebShell title refresh guard | 已合入 effective title gate，catalog fallback title 可见后不再每 turn 重复刷新完整 session catalog。 |
| #9626(open draft) | persisted storage lifecycle | 当前 draft 统一 logical/storage identity、maintainable transcript ownership 与 capability-gated archive conflict repair。 |
| #9665(open) | restore ask_user_question HITL | 当前 open diff 通过默认关闭的 serve flag，在 load/resume 后重新挂起尾部纯 AUQ batch；v1 不新增 capability tag。 |
...
| #9626(open draft) | persisted storage lifecycle | 当前 draft 统一 logical/storage identity、maintenance fencing 与 capability-gated archive conflict repair。 |
| #9665(open) | restore ask_user_question HITL | 当前 open diff 提供默认关闭的 restore flag；仅恢复尾部纯 AUQ batch，且不广告 capability。 |
...
8. **open/draft diff 不能写成 main 已落地能力**。#8093/#8824 为 closed draft，#9048 为 closed，这些只能作为历史前身记录。#9513/#9665 为 open，#9626 为 open draft，只能作为当前方案记录。#8743 已由 #9055 runtime PR 承接，不再按未落地 draft 追踪。#7812/#7821/#7886/#7894/#7947/#7967/#7975/#7976/#7994/#8002/#8080/#8245/#8383/#8387/#8414/#8415/#8423/#8450/#8462/#8464/#8469/#8508/#8572/#8588/#8620/#8691/#8833/#8852/#8882/#8890/#8891/#8892/#8893/#8911/#8931/#8932/#8933/#8939/#8947/#8954/#8955/#8990/#9007/#9042/#9055/#9134/#9180/#9181/#9261/#9341/#9362/#9380/#9396/#9476/#9512/#9563 已按 merged diff 更新。
