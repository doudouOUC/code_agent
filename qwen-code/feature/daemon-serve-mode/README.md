...
| 13 | [资源预算与公平调度](13-resource-budgeting.md) | #8093 closed draft 的 process-wide `ResourceBudget`、completion reserve、emergency pool、bulk/spawn/process fair scheduler、buffered process runner foundations，#8245 已合入的 daemon memory budget reporting，#8423 已合入的 memory pressure observe mode，#8462 已合入的 active ACP child RSS aggregate，#8508/#9380 已合入的 child heap partition/status measurement model，#8911 已合入的 daemon ACP NDJSON buffers、#8947 已合入的 ACP transport resource guard、#9007 已合入的 ACP HTTP pre-attach buffer byte budget、#11428 已合入的容量owner解耦、#11515 已合入的默认256注册/800 session/25 Channel owner边界、#11653已合入的无限cgroup哨兵修复、#11911已合入的opt-in child数量准入、#11940已合入的零session warm child回收，以及#12008 open的用户确认runtime stop |
| 14 | [Linux 4c8g 容量实测](14-capacity-validation-linux-4c8g.md) | #11428/#11515 合入后在 4 vCPU / 8 GB Linux + 257 个真实 Git 仓库上的部署侧实测：注册规模阶梯、真实模型并发、重启/churn/空闲与cgroup授权；无限哨兵已由#11653修复，#11911/#11940已补显式数量准入和安全warm-child回收，但raise-only/per-child enforcement仍保留，#12008用户确认stop仍为open |
...
- **资源预算、数量准入与恢复分层（#8093 closed draft，#8245/#8423/#8462/#8508/#9380/#11428/#11515/#11653/#11911/#11940 merged，#12008 open）**：#8093 关闭前 draft 只提供 `ResourceBudget`、fair schedulers 与 buffered process runner primitives；#8245/#8423/#8462/#8508/#9380 建立 memory denominator、pressure、RSS 与 child heap 观测；#11428/#11515 分离并扩展 workspace/session/Channel owner，#11653拒绝无限或超宿主cgroup值。#11911把modeled child count接成显式`admit`准入，#11940在首次拒绝后最多回收一个零session、零activity warm child；两者仍不读取实时RAM或应用per-child heap ceiling。loaded session占满时的用户确认stop仅存在于#12008 open diff，不是当前`main`能力。
