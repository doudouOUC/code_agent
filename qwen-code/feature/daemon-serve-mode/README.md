...
| # | 子文档 | 覆盖 |
|---|---|---|
| 01 | [HTTP 服务 / 路由 / 中间件链](01-http-server-and-middleware.md) | 中间件链顺序、路由表、bearer / --require-auth / mutate / CORS / host allowlist 五道闸、prompt route 202 + bridge-owned deadline / 权限响应超时 / access log、REST SSE stream lifecycle diagnostics（#8572 merged）、restore timeout 504/retryable error mapping（#8691 merged）、`/health?deep=1.activeWork`（#8588/#9042 merged）、permission/AUQ timeout 默认 disabled（#9933 merged）、current-session scheduled-task runtime wiring（#9838 merged）、standalone public API（#10179 merged）、`serve --open-with-auth`（#9738 merged）、trusted-loopback operator authority（#10403 merged）与 sessionless user-language route（#10554 merged） |
...
| 03 | [会话生命周期](03-session-lifecycle.md) | spawn/attach/close/delete、sessionScope single/thread、heartbeat、load/resume、session archive/unarchive、session organization、batch load replay、attach-ref ledger、prompt terminal exactly-once 与 follow-up hardening、session writer lease opt-in（#7894）、managed writer shutdown（#7812）、timestamp drift reconciliation（#7886）、Todo Stop Guard continuation hardening（#7821）、maintenance writer isolation（#7975）、certified writer handoff（#7976）、caller-supplied session id admission（#8415 merged）、restore timeout / late cleanup quarantine（#8691 merged）、new-session initialization deadline/late cleanup（#10268 merged）、selective session restore runtime（#9055 merged）、Conversations runtime foundation/boundary（#8890/#9181 merged）、standalone primitives/internal core/public API（#9341/#9512/#9978/#10179 merged）、archive race recovery与 persisted maintenance/conflict repair（#9513/#9626 merged）、post-commit writer cleanup（#10286 closed→#10300 merged）、AUQ restore/hardening（#9665/#9763 merged）、per-session model restore（#9687 merged）、provisional reasoning延后（#10706 merged）、relaxed Conversations ownership设计/fence/runtime cutover（#10828/#10924/#11207 merged）、worktree persistence/reset/route/delete lifecycle（#10643/#11308/#11309 merged，#11015 open）、housekeeping bind-mount fail-fast（#11285 merged）、failed close-probe backoff（#11120 merged）、runtime record I/O retryability（#9362 merged）、live-state activity watermark/WebShell consumer（#9396/#9476 merged）、ACP channel transport liveness（#9976 merged）、ACP process-tree reaping（#10142 merged）、Live task canonical bridge ID（#9819 merged）、current-session scheduled task/empty persistence（#9838/#10144 merged）、activeWork lifecycle/close/refusal-hold guard（#8588/#9042/#9134/#9820 merged）与用户确认workspace runtime stop生命周期（#12008 merged） |
| 04 | [能力注册表与协议](04-capabilities-and-protocol.md) | SERVE_CAPABILITY_REGISTRY、协议版本、typed event schema、协议补全、能力覆盖矩阵、workspace trust hot reload capability（#7268）、session_id_override capability（#8415 merged）、SSE stream diagnostics、session restore timeout/error taxonomy、selective restore metadata（#9055 merged）、Conversations runtime guard（#9181 merged）、standalone/runtime safety、archive recovery 与 internal core（#9341/#9512/#9362/#9513/#9978 merged）、`standalone_sessions_v1`（#10179 merged；#10294 merged SDK consumer）、storage conflict repair capability（#9626 merged）、AUQ restore flag/no-capability 与 suppress-meta 边界（#9665/#9763 merged）、per-session model internal projection（#9687 merged）、workspace live-state capability/route与poll interval（#9261/#11339 merged）、ACP child heap/live-state fields（#9380/#9396 merged）、private channel-liveness negotiation（#9976 merged）、`scheduled_task_session_reuse` 条件能力（#9838 merged）、permission timeout operator-only 默认（#9933 merged）、trusted-loopback session-shell 广告（#10403 merged）、条件 `user_language_sync`（#10554 merged）、`standalone_session_options_v1`（#10719 merged）、`session_turn_navigation`（#10751 merged）、`session_worktree_persistence_v1`（#10643 merged）、`session_worktree_reset_v1`（#11015 open）、ACP child count-admission additive status/error（#11911/#11940 merged）与 `workspace_runtime_stop`（#12008 merged） |
| 05 | [工作区文件路由与 FS 边界](05-workspace-files-and-fs-boundary.md) | resolveWithinWorkspace 防穿越、editAtomic hash CAS、原子写、Serve large-text bounded read（#7947）、handle-bound range reader（#7967 merged）、byte-cursor paging（#8002）、lineEnding metadata consistency（#8383 merged）、same-host daemon text read delegation（#8620 merged）与 approved external built-in text writes（#8852 merged） |
| 06 | [MCP 守卫与共享传输池](06-mcp-guardrails-and-pool.md) | per-session 预算 → workspace 共享池、引用计数、env 隔离、unsafe replay guard（#8387） |
| 07 | [acp-bridge 抽包与多客户端权限协调](07-acp-bridge-and-permission.md) | 抽包seam、四策略权限仲裁、并发不变量、same-host read/write delegation能力分离（#8620 merged）、daemon ACP/HTTP resource guard（#8911/#8947/#9007 merged）、ACP channel liveness（#9976 merged）、process-tree ownership（#10142 merged）、new-session deadline/late cleanup（#10268 merged）、standalone private service/guard/public API（#9978/#10179 merged）、active-work close refusal/hold上限（#9134/#9820 merged）、private current-session task creator/empty persistence（#9838/#10144 merged）、permission/AUQ timeout默认disabled（#9933 merged）、load/resume AUQ restore/hardening（#9665/#9763 merged）、sessionless user-language fan-out（#10554 merged），以及mandatory lease后的relaxed Conversations runtime cutover（#11207 merged） |
...
| 13 | [资源预算与公平调度](13-resource-budgeting.md) | #8093 closed draft 的 process-wide `ResourceBudget`、completion reserve、emergency pool、bulk/spawn/process fair scheduler、buffered process runner foundations，#8245 已合入的 daemon memory budget reporting，#8423 已合入的 memory pressure observe mode，#8462 已合入的 active ACP child RSS aggregate，#8508/#9380 已合入的 child heap partition/status measurement model，#8911 已合入的 daemon ACP NDJSON buffers、#8947 已合入的 ACP transport resource guard、#9007 已合入的 ACP HTTP pre-attach buffer byte budget、#11428 已合入的容量owner解耦、#11515 已合入的默认256注册/800 session/25 Channel owner边界、#11653已合入的无限cgroup哨兵修复、#11911/#11940/#12008已合入的count准入/自动回收/用户stop、#12265已合入的fixed-heap校准证据，以及#12353 open的opt-in enforcement draft |
| 14 | [Linux 4c8g 容量实测](14-capacity-validation-linux-4c8g.md) | #11428/#11515 合入后在 4 vCPU / 8 GB Linux + 257 个真实 Git 仓库上的部署侧实测：注册规模阶梯、真实模型并发、重启/churn/空闲与cgroup授权；#11653/#11911/#11940/#12008已修复哨兵并补count恢复链，#12265已合入Node 24 heap证据，#12353 open提议显式`enforce`，但per-child enforcement仍未进入`main` |
...
- **workspace trust 可以热重载（#7268）**：最终实现将 trust policy loader、monitor、reconciler 与 runtime generation guard 串起来，trust grant/revoke 后关闭旧 generation、drain 并重建 runtime；过渡、失败或 blocked 状态通过 v2 trust status 暴露，route commit 边界 fail-closed。
- **Web Shell Voice 必须跟随 composer workspace（#7754）**：最终实现让 primary/locked/split-view composer 解析唯一 Voice owner；trusted secondary 走 workspace-qualified Voice status/settings/model/stream routes，未知、歧义、不可信、draining/removed 状态 fail closed，不 fallback primary。
...
- **资源预算、数量准入与恢复分层（#8093 closed draft，#8245/#8423/#8462/#8508/#9380/#11428/#11515/#11653/#11911/#11940/#12008/#12265 merged，#12353 open）**：#8093 关闭前 draft 只提供 `ResourceBudget`、fair schedulers 与 buffered process runner primitives；#8245/#8423/#8462/#8508/#9380 建立 memory denominator、pressure、RSS 与 child heap 观测；#11428/#11515 分离并扩展 workspace/session/Channel owner，#11653拒绝无限或超宿主cgroup值。#11911把modeled child count接成显式`admit`准入，#11940在首次拒绝后最多回收一个零session、零activity warm child，#12008让用户确认停止loaded runtime并在registry证明释放后单次继续，#12265合入fixed-heap校准证据。当前`main`仍不应用per-child ceiling；#12353仅以open draft增加opt-in`enforce`。
...
本节保留 daemon serve mode 基线能力的 PR 归因入口，后续章节按子系统拆解 HTTP 服务、session lifecycle、协议、extension surface、SDK 与 Web Shell 的具体实现。
...
**SSE 写侧背压（`server.ts` SSE 路由 L2653-3059）**：`res.write` 返回 false（内核发送缓冲满）时 `await drain`，避免用户态无界堆积；所有写（含心跳）经 `writeChain` 单飞串行化，防止心跳与主循环交错写半个 SSE 帧。15s 心跳保活（`: heartbeat`）。`--writer-idle-timeout-ms`（T2.9/#4530）加一层应用级 idle 守卫：若"最近一次成功 flush"早于预算则直写 `client_evicted{writer_idle_timeout}` 并清理（绕过可能已卡死在 drain 的 chain）。#8572 的 close finalizer 不改变这套背压语义，只把每次 false write、drain 等待、live publish 到 write settled 的最大延迟和最终 close reason 归因到具体 stream。
...
**close / detach / delete**（#4240 / Wave 2.5 PR 11；#7386/#7400/#7453 reliability closeout）：`DELETE /session/:id`（`closeSession`，attach 不计入 `--max-sessions` cap）、`POST /session/:id/detach`（`detachClient`，按 per-client `attachRefs` 释放真实 attach 引用，重复/未知/匿名/owner detach 不偷减 `attachCount`）、`POST /sessions/delete`（批量）、`PATCH /session/:id/metadata`（重命名等）。`closeSession`/`killSession` 标 `isDying` 同步、`kill()` channel、由 `channel.exited` handler 在 OS reap 后做 alive-set 清理；这期间并发 `spawnOrAttach` 能立刻 spawn 一个全新 channel 并重指 `channelInfo`（不必等 OS reap）。所有 teardown 路径会在 `session_closed`/`session_died` 和 `events.close()` 前先 `flushPromptTerminals()`，为 active 与 queued prompt 发布 exactly-once formal terminal。#7453 在 #7400 自审后继续收口：running prompt 从 visible list 移除时先隐藏但保留 pending entry，等 settle/teardown 后再清；queued terminal 只发事件、不污染 session `turnError`/retry 状态；queued deadline 保留 `PromptDeadlineExceededError` 类型，并让 deadline helper 回到纯叶子模块。
...
**status / perf / health / log surface + dashboard 方案记录**（#6263/#6270/#6296/#6325/#6416/#6961/#6969；#6253 closed）：`/daemon/status` 继续作为 daemon 诊断聚合端点扩展。#6263 增加 optional `runtime.perf`，只暴露 daemon 进程 event-loop lag snapshot 与 daemon-child pipe inbound/outbound byte 统计；ACP child lag 留在 OTel gauge 与 forwarded stderr stall warning，避免把 child-only 观测塞进 status JSON。#6270 在 `runtime.activity` 增加 `activePrompts`、`lastActivityAt`、`idleSinceMs`，bootstrap status 返回稳定 `0/null/null`，并给 full workspace MCP section summary 补 `serversConnected`、`serversErrored`、`serversDisabled`。#6325 继续补齐 prompt FIFO 可观测性：`runtime.activity.pendingPrompts` 统计所有已接受但未 settle 的 prompts，`queuedPrompts` 统计已接受但尚未 dispatch 的 FIFO 等待项；`runtime.perf.promptQueueWait` 和 metrics ring 同步保留 queue wait count/mean/max/last 与窗口 p95。#7400 后 `pendingPromptCount` 还成为 close-on-last-detach、attach rollback 与 idle reaper 的生命周期门控，覆盖 active + queued 和 FIFO hand-off gap；queued prompts 仍只是 status backpressure signal，不升级为 status issue。多客户端要避免共享 FIFO 时应在 `POST /session` 显式使用 `sessionScope:"thread"`。#6296 修正 preflight auth 的 false warning：env var 未命中时读取 live `Config` 的 resolved `generationConfig.apiKey`，使 settings apiKey / provider envKey / CLI flag 与实际 session 启动口径一致。#6253 的 closed diff 只作为未合入 dashboard 方案记录。#6416 给 status 增加 daemon-wide total admission 的 additive 字段。#6961 把 `GET /health?deep=1` 从 primary-only counter 改为聚合所有 managed runtimes（含 draining），新增 `workspaceCount`，但仍保持 informational counter snapshot。#8588 已合入并在 deep health 里 additive 增加 `activeWork`：只要任一 managed/draining runtime 存在 accepted-but-unsettled prompt、运行中的 background Agent，或 Agent terminal notification 正在接受/排队/持久化/父 continuation 处理，就返回 true；#9042 已合入，把 Session-managed background shell running/terminal-notification/parent-continuation 也纳入 bounded `shell` hold，并通过 active-work v1 类别协商区分 `agent`、`notification`、`shell`；#9134 已合入，让 automatic close 在拒绝前不破坏 queued work，deferred spawn-owner kill 遇到 close/probe in-flight 时等待 settle，child 明确拒绝 close 时保留 entry 而不杀同 channel sibling sessions。ACP child 通过 private initialize `_meta` 协商 active-work heartbeat，按 session ownership + 单调 seq 上报 transition，45s 缺预期 heartbeat 时回收 owning channel。#6969 把 daemon log 收敛为 stable `debug/daemon/daemon.log` + bounded archives/fallback families，并在 status 中暴露 `runId`、log mode/health/issues 和 dropped counters。
...
**session export / organization / load replay / settings isolation/cache / bounded transcript / recording durability**（#6292/#6297/#6305/#6309/#6310/#6407/#6482/#6525/#6740/#6743/#6769/#6844/#6911）：#6292 先修 ACP session 创建/加载/恢复入口的 settings 隔离：`newSession`、`loadSession`、`unstable_resumeSession` 在请求开始时各自加载 workspace settings，并显式传入 `newSessionConfig()` 与 `createAndStoreSession()`，避免并发请求通过共享 `this.settings` 串 workspace 或 runtime output dir。#6297 新增 `session_export` capability 和 `GET /session/:id/export`，复用 CLI export formatter 返回 HTML/Markdown/JSON/JSONL attachment，并在 SDK 与 Web Shell 侧栏增加 capability-gated download。#6305 新增 `session_organization` capability，把 group catalog 与 per-session pin/group 状态存在 project sidecar `session-organization.v1.json`。#6309 给 daemon-owned `session/load` 增加 response-mode replay。#6310 引入 workspace LRU `loadSettingsCached()`。#6407 把 `settings_reloaded` 改成 SDK `workspace.settings.changed` signal。#6482 把 `/load` replay snapshot 变成 byte-capped window。#6525 新增 `GET /session/:id/transcript`。#6740 新增 workspace-qualified persisted-only transcript reader。#6844 新增 workspace-qualified full export：`GET /workspaces/:workspace/session/:id/export` 只读 selected trusted workspace 的 active JSONL。#6911 再新增独立 `workspace_archived_session_export` 与 `GET /workspaces/:workspace/session/:id/archive/export`，只读 selected trusted workspace archived JSONL，不查 active、不 fallback primary、不启动 ACP。#6769 给 workspace transcript route 增加 byte bounds。#6743 让 chat recording 首次 JSONL append 失败后永久停止 recorder。

**multi-workspace Phase 1/2a/2b/3/4 + hardening closeout**（#6394/#6410/#6416/#6511/#6540/#6558/#6567/#6621/#6625/#6631/#6635/#6716/#6717/#6724/#6740/#6745/#6769/#6798/#6825/#6826/#6833/#6839/#6844/#6911/#6912/#6961/#7003/#7005/#7019/#7754）：为 daemon multi-workspace 建立内部 runtime registry，并逐步从 sessions-only 扩展到 workspace-qualified surface。#6394 先落 Phase 1 single-runtime registry。#6410 补 Phase 2a foundation。#6416 落地 runtime-local env snapshot 与 total admission。#6511 打开 sessions-only live closed loop。#6540 抽出 owner index。#6558 合并 trusted non-primary persisted sessions 与 live summaries。#6567 增加 `/workspaces/:workspace/...` core REST 和 SDK `WorkspaceDaemonClient`。#6621 把 ACP transport 扩展到 `/workspaces/:workspace/acp`。#6625 支持运行时动态注册 workspace；#6716 增加 persistent workspace registration。#6631 补齐 trusted non-primary archived/organized/group list；#6717 允许 untrusted secondary 读取 persisted-only catalog；#6724 给 trusted secondary 补 organization mutation。#6740 增加 workspace persisted transcript route；#6769 给 workspace transcript route 加 byte bounds；#6745 新增 runtime hot removal；#6798/#6826/#6833 把 legacy session actions、rewind/shell、continue/language/artifact 改为 owner-aware；#6839 新增 workspace-qualified Voice；#7754 让 Web Shell Voice 跟随 composer owner，trusted secondary 走 workspace-qualified Voice routes 且 unsafe owner fail closed；#6844 新增 workspace-qualified active export；#6911 新增 workspace-qualified archived export；#6912 加固 Web Shell non-primary archive/unarchive row identity 与 reconciliation；#6961 让 deep health 聚合所有 managed runtimes。#6635 把 channel workers 按 workspace 分组；#6825 新增 `extension_management_v2`。#7003 给 legacy `/session`、`/sessions`、`/permission` routes 建立 telemetry ownership catalog，并在 handler 解析 runtime 后 late-bind workspace hash，不能确认 owner 时不 fallback primary；#7005 将 branch/fork/cd 明确保留为 primary-only live-session routes，对 secondary owner fail-closed，并加固测试 env/setup-github workspace ownership；#7019 增加 hardening baseline 文档，把 process-global、legacy-primary、workspace-qualified、live-session-owner、persisted-workspace 五类 ownership 变成后续开发口径。
...
| `GET /session/:id/stats` | #4515 follow-up | 会话统计快照：模型请求/token 累计、工具调用/决策累计、文件增删行。 |
| `GET /session/:id/export` | #6297 | primary workspace active transcript 全量导出 HTML/Markdown/JSON/JSONL attachment。 |
| `GET /workspaces/:workspace/session/:id/export` | #6844 | selected trusted workspace active transcript 全量导出；不 fallback primary、不启动 ACP。 |
...
| `/export` | #6297/#6844 | primary export 已落地；workspace-qualified export 通过独立 `workspace_session_export` capability gate。 |
...
`recap`/`btw` 用 `mutate()`（非 strict，与 `/prompt` 同 posture：token 成本而非状态变更）；shell 执行本地命令，当前走 strict mutation gate，并在 #6826 后按 live owner runtime 分发。`DaemonWorkspaceService`（#4563）从 `AcpSessionBridge` 抽出 workspace 级状态读取（方案 C）。
...
| #6825 | extension management v2 | 新增 `extension_management_v2`：user-level extension artifact + workspace activation policy，全局 `/extensions/*` 与 workspace projection routes 分离；`ExtensionStore` journal/recovery、operation queue、post-commit warning、revisioned settings bundle 和 SDK/TUI/Web Shell surface。 |
| #6839 | workspace-qualified Voice | trusted selected runtime 支持 `/workspaces/:workspace/voice`、batch transcription 和 Voice WS；进程级 admission 上限 8，并接入 runtime removal activity。 |
| #7754 | Web Shell Voice composer workspace scoping | 让 primary/locked/split-view composer 解析 fail-closed Voice owner；trusted secondary 使用 workspace-qualified Voice status/settings/model/stream routes，owner 变化清理 capture generation。 |
| #6844 | workspace session export | selected trusted workspace 支持 full transcript export，复用 legacy formatter/header/archive lock，不 fallback primary。 |
...
| #6741 | runtime channel control | `channel_control` + `GET/PUT/DELETE /workspace/channel`，daemon 运行时启用/替换/查询/停止 channel worker selection。 |
...
| #6745 | runtime workspace removal | `DELETE /workspaces/:workspace` hot remove removable secondary runtime，drain sessions/ACP/memory/channel 并忘记 persistent aliases。 |
| #6769 | workspace transcript page bounds | workspace transcript route 增加 4 MiB source、32 MiB response 与 64 KiB cursor 边界。 |

### W29 2026-07-13 ~ 2026-07-16 daemon / serve follow-up

| PR | 子主题 | 一句话作用 |
| --- | --- | --- |
| #6798 | legacy session owner routing | metadata、recap、BTW、mid-turn message、task cancel、goal clear 改按 live owner runtime 分发，secondary trusted session 不再被 primary-only guard 误拦。 |
| #6825 | extension management v2 | user-level extension artifact 与 workspace activation policy 分离，`ExtensionStore`/operation queue/journal/recovery 串行化 artifact 与 activation mutation。 |
| #6826 | multi-workspace rewind/shell | rewind snapshot、rewind、shell singular routes 按 live session owner runtime 分发，并新增 `multi_workspace_session_rewind` / `multi_workspace_session_shell`。 |
| #6833 | continue/language/artifact owner routing | legacy continue、language、artifact add/delete 改为 owner-aware mutation，复用原 strict auth、client id 和错误映射。 |
| #6839 | workspace-qualified Voice | trusted selected runtime 支持 workspace Voice REST/WS、batch transcription、settings/env 隔离与 runtime removal activity guard。 |
| #7754 | Web Shell Voice composer workspace scoping | 让每个 composer 绑定唯一 Voice owner；unsafe secondary fail closed，不回退 primary，capture generation 随 owner 变化清理。 |
| #6844 | workspace session export | selected trusted workspace full transcript export，不 fallback primary、不启动 ACP，复用 legacy formatter/header/archive lock。 |
| #6907 | cold first-session tracing | deferred runtime wait、channel wait 与 ACP `session_start` 分阶段 span 串成 cold `POST /session` trace，定位首会话慢点。 |
...
### W31 2026-07-27 ~ 2026-07-29 daemon / serve follow-up
...
| #7886 | transcript timestamp drift tolerance | 将 transcript timestamps 降为 advisory，timestamp-only drift 走 SHA-256 full reconciliation，identity/owner/tail 等硬状态仍 fail-closed。 |
| #7947 | Serve large text bounded reads | 允许 Serve workspace `/file` 对超过 256 KiB 的 UTF-8 文本返回 bounded line window，full snapshot/edit/hash 仍保留旧大小门。 |
| #7967 | handle-bound text range reader | 已合入，将 path reader 与 caller-owned handle reader 分离，删除混合 flag/dead fields，为 cursor paging 复用同一 descriptor 铺路。 |
| #7975 | daemon maintenance writer isolation | 让 archive/delete/unarchive/scheduled cleanup/ACP orphan cleanup 在 selected runtime 内执行，并为会写 transcript 的维护任务获取 daemon writer lease。 |
...
| #8383 | Serve text lineEnding metadata | 已合入，让 workspace text read 的 `meta.lineEnding` 从完整 decoded file 检测，避免 CRLF cursor paging 前后页不一致。 |

### W32 2026-08-03 ~ 2026-08-09 daemon / serve follow-up
...
| #8415 | caller-supplied session id admission | REST/ACP/SDK 支持请求指定 UUID session id，并通过 daemon-wide admission 防 live/pending/history/archive/workspace 冲突。 |
| #8423 | daemon memory pressure observe mode | status 用 daemon root RSS/heap 对真实 cgroup/host/heap denominator 计算 pressure ratio，默认 observe-only warning。 |
...
| #8572 | REST SSE stream observability | 每条 REST SSE stream 分配 UUID，SDK/WebUI 报告 connect reason 与 previous stream lineage，daemon log/telemetry 关联 open/resync/slow-client/evict/close。 |
| #8588 | activeWork deep health | `GET /health?deep=1` additive 暴露 activeWork，并通过 ACP child heartbeat 把 background Agent 和 Agent terminal notification 纳入 idle/restart guard。 |
| #8620 | same-host text read delegation | daemon-owned same-host bridge 关闭 delegated text read、保留 delegated text write，让批准后的 direct `read_file` 可读 workspace 外普通文本，而最终写入仍走 WorkspaceFileSystem 边界。 |
...
| #8743(closed by #9055) | selective session restore design | docs-only draft 的 selective restore projection 已由 #9055 runtime PR 承接；历史设计保留为实现前身。 |

### W33 2026-08-10 ~ 2026-08-16 daemon / serve follow-up
...
| #8882 | transactional cross-session switching | 已合入，现代 WebUI cross-session load/resume 在 target restore/replay staging 完成并赢得仲裁前保留 source visible owner，legacy daemon 继续 detach-first。 |
| #8890 | Conversations runtime foundation | 已合入，抽出 Conversations runtime / store / registry 基础层，为 daemon-managed standalone session 铺底；不再是纯 docs-only draft。 |
| #8891 | shared WebShell session catalog scheduling | 已合入，WebShell session catalog 通过 `SessionCatalogStore` 共享 cached page、in-flight request 和 client-wide scheduler，mutation 后按 owning workspace invalidate/patch。 |
| #8892 | persisted session catalog cache | 已合入，daemon session list 对 persisted JSONL/worktree sidecar catalog 做 2 秒 single-flight cache，Live/organization/filter/sort/page 仍按请求计算。 |
| #8893 | non-interactive OpenAI log housekeeping | 已合入，headless CLI、stream-json 与 ACP lifecycle 注册 OpenAI API log retention 队列，退出时 best-effort abort/drain。 |
| #8911 | bounded daemon ACP NDJSON buffers | 已合入，`qwen serve` ACP child 的 NDJSON frame 上限 64 MiB，decoded inbound queue 上限 256 条/64 MiB，并在超限时终止精确 child。 |
| #8931 | prompt-safe session navigation | 已合入，prompt submission 在 async host admission hook 返回后重新检查 session write gate，navigation preparing 时保留 source draft。 |
| #8932 | continuation admission logs | 已合入，accepted continuation 写低敏 `continuation enqueued` 结构化日志，只含 sessionId/promptId/clientId。 |
| #8933 | restore request shape fencing | 已合入，按 target + resume/load/all/recent(N) 区分 restore coalescing，避免不同 replay 语义互相满足。 |
| #8939 | transactional same-session refresh | 已合入，same logical session load/reload/resume/clientId rebind 先 staging candidate，commit 前保留 source attachment/transcript/event stream。 |
| #8947 | ACP transport resource guard | 已合入，在 #8911 raw stream bounds 之外，限制 ACP handler/prepared response/outbound/outstanding request 队列并让 fatal failure 立即隔离 channel generation。 |
| #8954 | session list cancellation propagation | 已合入，WebShell/SDK/daemon list 请求共享 `AbortSignal`，过期 list 不再占用 scheduler 或写回 stale catalog page。 |
| #8955 | prompt admission ownership hardening | 已合入，prompt submit/queued/follow-up/background catch-up 统一按 owner token gate，restore pending 时旧 owner 不能清空 draft 或向新 attachment 发 prompt。 |
| #8990 | same-session refresh race closeout | 已合入，补齐 same-session refresh 的 late restore、metadata、stream restart、candidate retirement 和 replay suffix ownership gate。 |
| #9007 | ACP HTTP pre-attach byte budgets | 已合入，给 pre-attach buffered JSON-RPC reply 增加 serialized-byte budget 和 delivery receipt，超限时关闭精确 logical connection。 |
| #9042 | background shell activeWork hold | 已合入，让 Session-managed background shell running、terminal notification queued 与 parent continuation processing 上报 bounded `shell` activeWork hold，避免 idle cleanup/restart 误清理。 |
| #9048(closed) | transactional resync / live-journal repair | closed diff 只作为观察：曾把 authoritative resync 与 live-journal repair 纳入 provider-local restore coordinator，但未合入 `main`。 |
| #9055 | selective session restore runtime | 已合入，cold load/resume 构建一次 transcript index，只读取 runtime state 与目标 replay projection 所需 records；explicit `historyPageSize` 在 payload read 前选择 bounded recent page，并返回 pagination metadata。 |
| #9134 | active-work close authorization | 已合入，让 conditional close 在拒绝前不取消 queued work，deferred kill 避开 close/probe in-flight，并把 child definitive close refusal 处理为可重试保留。 |
| #9180 | Web Shell text file attachments | 已合入，让 composer paste/drop 接收文本文件，渲染 file chips，并把内容作为 ACP `resource` blocks 随 prompt 投递。 |
| #9181 | Conversations runtime boundary isolation | 已合入，增加 Conversations runtime owner record、serve lifecycle release proof、activity gate 和 ordinary runtime visibility guard，隐藏 internal `live-conversation` runtime。 |
| #9261 | workspace session live-state protocol | 已合入 selected-runtime/trusted-only `/workspaces/:workspace/sessions/live-state`、catalog version token、cache invalidation 顺序和 Web Shell/SDK refresh handshake。 |

### W34 2026-08-17 ~ 2026-08-23 daemon / serve follow-up

| PR | 子主题 | 一句话作用 |
| --- | --- | --- |
| #9341 | standalone conversation isolation primitives | 已合入 explicit standalone source、loadable metadata reader、case-insensitive session id conflict、JSONL integrity-aware parsing 和 standalone directory identity/admission guard；仍不开放 public standalone route、capability、SDK/UI。 |
| #9362 | transient runtime record I/O retryability | 已合入 Conversations owner/discovery record 的 transient `open`/`readFile` 错误分类，普通 EMFILE/EIO 继续走 retryable runtime unavailable，ELOOP/malformed/unsafe state 才 permanent compromised。 |
| #9380 | ACP child peak old-generation heap | 已合入 daemon-owned ACP child old-generation peak measurement，通过 `/daemon/status` observe-only 暴露 per-child maxima，不改 spawn/enforcement。 |
| #9396 | live-state session activity watermark | 已合入 live-state optional `updatedAt`，普通 turn activity 不推进 catalog version。 |
| #9476 | WebShell activity timestamp consumer | 已合入 post-completion live-state settle、当前 active page recency patch/reorder 与 full catalog fallback。 |
...
| #9626 | persisted maintenance | 已合入；维护 empty/malformed/orphan owned regular transcript，对 unknown/mixed ownership fail closed，并提供 identity/generation-fenced、capability-gated archive conflict repair 与跟随 sidecar cleanup。 |
...
| #9738 | `serve --open-with-auth` | 已合入；独立 opt-in flag 在 listen 前校验 loopback/Web Shell/assets，复用或生成 bearer，并走既有 URL fragment→tab sessionStorage 交付。 |
| #9763 | restore ask_user_question hardening | 已合入普通发送 history 合法化、replay/re-hang suppress 锁步、post-answer notice 与 unattended batch persistence 修复。 |

### W37 2026-09-07 daemon / serve follow-up

| PR | 子主题 | 一句话作用 |
| --- | --- | --- |
| #11207 | relaxed Conversations ownership runtime | 已合入pre-publication legacy owner check、mandatory per-session lease、independent deletion journal、exact Live publisher admission与WebShell writer-blocked局部恢复，使不同新版daemon可共享root并服务不同session。 |
| #11208 | continuous history and compact turn rail | 已合入frozen historical viewport、reading-anchor pagination、viewport pins、bounded全量turn ticks与按需远端定位，将#11054数据层接入可见WebShell。 |
| #11285 | deleted bind-mount housekeeping fail-fast | 已合入单层0700 lock目录创建，异常bind mount立即失败并跳过housekeeping，不再让recursive mkdir永久悬挂或放大CPU。 |
| #11308 | worktree route managed cold restore | 已合入route isolation/workspace root持久化、managed load+attestation、superseded redirect持久化、失效binding回滚与旧route metadata迁移。 |
| #11309 | worktree delete cleanup | 已合入runtime/sidecar/marker/slug/sharing与用户工作核验，确认删除且generation仍有效后保守回收checkout，branch不force删除。 |
| #11311 | generic MCP approval argument preview | 已合入WebShell generic MCP完整参数预览；独立Mem0 writer复用既有daemon权限，不新增route。 |
...
| #11447 | browser notification details/navigation | 已合入有界title/prompt/reply、图标、instance-scoped目标导航和embed branding；内置standalone仅在明确无保存偏好时默认开启。 |
| #11455 | ACP submitted prompt provenance | 已合入WebShell逐请求public声明、daemon/bridge private trust转发及fresh非channel Hook发布；缺失时不从request/display推断。 |
| #11515 | configurable 256-workspace registration | 已合入默认256及1–256 operator配置，扩容模式默认800 total sessions，store支持255条secondary/8 MiB，Channel owner独立限25；runtime仍eager且无LRU。 |
| #11653 | ACP child heap cgroup sentinel | 已合入统一可用内存探测，拒绝v1/v2无限哨兵和超宿主constraint；50%、16 GiB、raise-only与cache策略不变。 |

### W38 2026-09-14 ~ 2026-09-20 daemon / serve follow-up
...
| #12008 | user-directed runtime stop | 已合入；让用户查看并确认停止loaded workspace runtime，区分in-flight失败与close间预算耗尽，并在`stopped+released`后只允许一次guarded continuation。 |
| #12265 | ACP child fixed-heap calibration evidence | 已合入docs-only证据；发布8 runs/184 turns/360 tools的Node 24接受集与历史/失败证据，不改变production mode或heap argv。 |
| #12353 | opt-in ACP child heap enforcement | 当前open draft；新增`enforce`模式，用boot-resolved fixed ceiling替换managed child旧heap参数，并与count admission/status统一；默认仍为`observe`。 |
...
| #8572 | REST SSE stream/client observability | 每条 REST SSE stream 由 daemon 分配 UUID，TS SDK 传 `connectReason`/`previousStreamId`，WebUI 仅在可判定 prompt restart、normal end、transport error 或 state resync 时填 reason。 |
...
| #4530 / #7400 / #7453 | 超时 | prompt 绝对 deadline + SSE writer idle timeout（T2.9）；#7400 将 prompt deadline 从 route-side abort timer 迁入 bridge dispatch race，保证 202 后 formal terminal 与 FIFO 释放；#7453 补 queued deadline typed error 与 prompt-terminal 清理顺序。 |
...
| #5260 / #9933 | timeout flag/default | `qwen serve --permission-response-timeout-ms` 暴露 ACP 权限/`ask_user_question` 单次响应超时；#9933 后省略/0 无限等待，显式正数启用 deadline，非法值启动失败，超大值 clamp 到 Node timer 上限。 |
...
| #6844 | workspace session export | selected trusted workspace active transcript full export；SDK workspace client 强制 native REST。 |
...
| #6798 | legacy session action owner routing | metadata、recap、BTW、mid-turn、task cancel、goal clear 按 live session owning runtime dispatch。 |
| #6825 | extension management v2 | user-level extension store、workspace activation policy、transaction journal/recovery、revisioned settings bundle、async operation queue 与 post-commit warning。 |
| #6826 | multi-workspace rewind/shell | rewind snapshots、rewind、shell 按 live owner runtime dispatch；SDK rewind 强制走 REST，新增 multi-workspace rewind/shell 条件能力。 |
| #6833 | continue/language/artifact owner routing | continue、language、artifact add/delete legacy route 按 owning runtime dispatch，保留既有 auth 和 response shape。 |
| #6839 | workspace-qualified Voice | selected runtime Voice settings/transcribe/stream，进程级 Voice admission 与 runtime removal activity。 |
| #7754 | Web Shell Voice composer workspace scoping | 将 Web Shell Voice status/settings/model/stream 绑定到 composer owning workspace；owner 不安全时 fail closed，不 fallback primary。 |
| #8080 | workspace skill status read model | workspace skills status 读取不再触发 extension/skill 重扫描；daemon 侧 5s TTL、generation guard、single-flight、ETag 与 mutation invalidation，ACP child 只返回 committed cache。 |
| #8093(closed draft) | daemon resource budgeting foundations | 关闭前 draft diff 新增 process-wide `ResourceBudget`、completion reserve、emergency pool、bulk/spawn/process fair scheduler 和 buffered process runner，但未接生产 route。 |
| #8245 | daemon memory budget reporting | 已合入，在 daemon boot 时解析 configured/effective/modeled memory budget，通过 `/daemon/status`、协议、SDK 类型和 docs 报告；wire 上 `enforced:false`，不改变 admission 或 ACP child spawn。 |
...
| #8415 | caller-supplied session id admission | 通过 `session_id_override` capability、UUID validator 与 daemon-wide admission 协调 requested session id，防 live/pending/history/archive/workspace 冲突。 |
| #8423 | daemon memory pressure observe mode | status 用 daemon root RSS/heap 对真实 cgroup/host/heap denominator 计算 pressure ratio，默认 observe-only warning，不做 enforcement。 |
...
| #8588 | activeWork deep health | `GET /health?deep=1.activeWork` 作为 restart guard，覆盖 prompt、background Agent 与 Agent terminal notification 的 unsettled 状态。 |
| #8620 | same-host text read delegation | same-host daemon-owned bridge 广告 `readTextFile:false/writeTextFile:true`，让 direct read/pre-read 回到子进程本地 CLI 权限，最终 text write 继续委派 WorkspaceFileSystem。 |
| #8691 | safe session restore timeout | session load/resume restore 专用 deadline、structured retryable timeout、late result cleanup、quarantine gate 与 SDK/WebUI timeout budget。 |
| #8743(closed by #9055) | selective session restore design | docs-only draft 设计的 projection / replay page 已由 #9055 runtime PR 承接，不再作为未落地能力单独追踪。 |
| #9042 | background shell activeWork hold | 已合入，把后台 shell 与 terminal notification 纳入 activeWork 类别协商和 idle/restart guard。 |
| #9048(closed) | transactional WebUI resync/repair | closed diff 只保留为未合入观察；WebUI authoritative resync / live-journal repair 的事务化不能写成 main 能力。 |
| #9134 | active-work close authorization | 已合入，修复 `onlyIfUnheld` close 的非破坏性授权、共享 drain deadline、deferred spawn-owner kill 顺序和 definitive close refusal 下的 sibling-safe 保留。 |
| #9180 | Web Shell text file attachments | 已合入，支持 paste/drop 文本文件、composer file chip、queue/retry/restore 生命周期和 daemon prompt `resource` block 投递。 |
| #9181 | Conversations runtime boundary isolation | 已合入，增加 Conversations runtime owner record、ServeAppLifecycle release proof、shutdown activity gate 和 ordinary workspace visibility guard。 |
| #9261 | workspace session live-state protocol | 已合入纯内存 live-state 探针、daemon-local catalog version、cache invalidation 和客户端双读握手。 |
| #9341 | standalone conversation isolation primitives | 已合入 standalone source classification、loadable metadata reader、session id case conflict、JSONL integrity 和 standalone directory identity guard；public standalone API 仍未开放。 |
| #9362 | runtime record I/O retryability | 已合入 Conversations runtime owner/discovery record 的 transient open/read failures 保持 retryable，避免一次 I/O 故障永久 compromised。 |
| #9380 | ACP child peak old-generation heap | 已合入 daemon-owned ACP child old-generation peak measurement，通过 `/daemon/status` observe-only 暴露 per-child maxima，不改 spawn/enforcement。 |
| #9396 | live-state session activity watermark | 已合入 live-state optional `updatedAt`，普通 turn activity 不推进 catalog version。 |
| #9476 | WebShell activity timestamp consumer | 已合入 post-completion live-state settle、当前 active page recency patch/reorder 与 full catalog fallback。 |
...
| #9626 | persisted maintenance | 已合入；只维护 owned regular transcript，并以 identity/generation fencing、retained PR-binding merge 与 capability-gated option 修复 archive conflict。 |
...
| #9738 | `serve --open-with-auth` | 已合入独立 opt-in flag，复用既有 bearer 与 fragment/sessionStorage handoff。 |
| #9763 | restore ask_user_question hardening | 已合入 restore replay/persistence/notice follow-up，并保持 flag default-off。 |
| #9819 | Live task canonical bridge identity | 已合入；persisted/wire spelling 保留，owner/resident/resume/events/prompt dispatch 使用 canonical bridge key，歧义继续 fail closed。 |
| #9820 | bounded conditional-close refusal holds | 已合入；拒绝继续保留 session，只采纳最多 1024 条 holds，超限保留最后合法 cache。 |
| #9838 | current-session scheduled task | 已合入；用 Core prompt context、private ACP identity、Serve owner/lifecycle admission、generation rollback 与条件 capability 允许 durable task 显式复用当前普通 session。 |
| #9933 | permission timeout default disabled | 已合入；普通权限/AUQ 未配置时无限等待，显式正数仍可设置 deadline，wire/policy 不变。 |
| #9976 | ACP channel transport liveness | 已合入；协商后按 shared channel 做 nonce ping，连续两次按时 timeout 才复用 transport-failure teardown，parent timer 延迟不归咎 child。 |
| #9978 | standalone session PR2B internal core | 已合入 fail-closed `StandaloneSessionService`、私有目录 containment、owner routing 与 source-aware guards；该 PR 本身不开放 public route/capability/SDK/UI。 |
| #10142 | ACP process-tree reaping | 已合入；标准 ACP spawn 增加 tree ownership、bounded process snapshot、TERM→KILL 与 root-exit cleanup；legacy attach 保持 direct-child。 |
| #10144 | empty-session scheduled-task persistence | 已合入；REST existing-session task commit 前写入隐藏 default source anchor，能力/不存在/失败分别稳定映射。 |
| #10179 | standalone public daemon API | 已合入；条件广告 `standalone_sessions_v1`，增加 `/standalone/sessions` route family、exact owner admission 与 journaled delete recovery。 |
| #10294 | standalone TypeScript SDK | 已合入；capability-gated 完整 lifecycle、strict runtime validators、explicit restore strategy 与 outcome-unknown exact recovery，不自动重试 create。 |
| #10268 | new-session initialization deadline | 已合入；绝对 deadline/AbortSignal 贯穿 child 初始化，旧 child 由迟到 exact close、ID fence 与 fresh-admission quarantine 兼容收敛。 |
| #10269 | model provider runtime sync | 已合入 provider mutation 后 parent env、active ACP child 与 live Session registry 同步，并以 optional `runtimeSync` 报告结果。 |
| #10286(closed) / #10300 | post-commit session cleanup ownership | #10286 已关闭；#10300 已基于 merged standalone lifecycle，以 exact writer lock ownership 保护 transcript commit 后的 sidecar cleanup。 |
| #10403 | trusted-loopback operator authority | 已合入实际 primary loopback、无 token、未启用 `--require-auth` 时的完整 operator authority，统一 strict mutation、显式 session shell、Local Control pairing 与 Web Shell Channel controls。 |
...
| #10514 | standalone WebShell chats | 已合入；增加显式 global/workspace/Live 新会话意图、standalone Recents 与生命周期、pre-provider exact deep-link gate、非 workspace 项目功能隔离、composer/split ingress guard 和 typed recovery。 |
| #10554 | sessionless user-level language sync | 已合入；条件广告 `user_language_sync`，由 daemon 唯一持久化 user/global output language，再向 trusted runtimes fan-out 并通过 SDK 暴露聚合结果。 |
...
| #10643 | worktree-isolated named tasks | 已合入；用 `session_worktree_persistence_v1`、canonical relocate、排他 marker+原子 sidecar 和 strict restore attestation 建立 persisted-v1 worktree session。 |
| #10706 | provisional standalone reasoning options | 已合入；content-generator config 尚不可用时只返回 mode/model，managed activation 后再由 session-context refresh 发布完整 reasoning options。 |
| #10719 | fresh standalone model options | 已合入只读 `standalone_session_options_v1`、strict SDK validator 和 WebShell 零 session hydration，并把 model 原子放入首次 create。 |
| #10751 | session turn navigation protocol | 已合入 Phase 1 sparse turn index、HMAC snapshot、snapshot-bound transcript anchor、ACP vendor method 与 TS SDK；其合入时浏览器 rail 尚未实现，后由 #11054/#11208 承接。 |
| #10824 | repeated standalone New task reload | 已合入；standalone clear 后递增 restore nonce，让同一 deferred context 重读模型选项而不提前创建 session。 |
| #10828 | relaxed standalone daemon ownership design | 已合入 docs-only 设计：支持拓扑限定为同机同用户多个更新后 daemon，以 mandatory per-session lease、bridge attestation、Live publisher admission、controller binding 与 journal contention 约束替代 global owner；runtime 后由 #10924/#11207 分阶段交付。 |
| #10924 | mandatory Conversations writer fences | 已合入私有 marker/forwarding attestation、强制 lease、identity-qualified local reclaim、batch writer error、generation-scoped pin 与 unbound-task eligibility；该 PR 合入时仍有外层 global owner，后由 #11207 移除。 |
| #11015(open) | worktree reset ownership transfer | 当前 diff新增 marker-last CAS、双向 supersession、typed crash recovery、Channel registry self-heal和 `session_worktree_reset_v1`；尚未合入。 |
| #11020 | turn navigation Phase 2 design | 已合入 docs-only客户端 page/gap/index设计，不代表 runtime已实现。 |
| #11053(closed) / #11143(closed) | flat-store ledger turn navigation | 两个关闭未合入方案把 anchored pages按 ledger位置纳入 flat render store，并增加 gaps、turn index和锚定打开；只能作为历史观察。 |
| #11054 | separate-store headless turn navigation | 已合入 Phase 2A：历史 pages 隔离到 external `HistoricalTranscriptPageTable`，导航 store 统一 index、provisional、locator、anchored load、gap recovery 与 degraded fallback；可见连续历史与 rail 后由 #11208 交付。 |
| #11120 | failed close-probe suppression | 已合入 unanswered conditional close的指数退避、detail日志、10ms settle poll及 condemned/no-capability本地收敛；active-work hold根因未修复。 |
| #11207 | relaxed Conversations runtime cutover | 已合入legacy owner兼容检查、不同session跨daemon并发、mandatory session lease、独立deletion journal、exact Live publisher准入与WebShell writer冲突恢复。 |
| #11208 | continuous history and compact turn rail | 已合入bounded historical viewport、双向分页阅读锚点、return-to-live与64px全量turn ticks；#11054的Phase 2A已推进为可见Phase 2B/3。 |
| #11285 | housekeeping bind-mount fail-fast | 已合入单层lock目录创建，deleted bind mount上的ENOENT快速降级，避免recursive mkdir永久pending/CPU循环。 |
| #11308 | worktree route managed restore | 已合入route metadata、managed load attestation、superseded durable redirect、失效load回滚与旧route migration。 |
| #11309 | owned worktree delete cleanup | 已合入delete前ownership/runtime/containment/sharing核验，确认删除后只回收无用户工作的checkout且不force删branch。 |
| #11311 | generic MCP permission argument preview | 已合入无专用preview时的完整literal args；Mem0 writer复用既有daemon权限，不新增route。 |
...
| #11447 | browser notification details/navigation | 已合入turn内容摘要、Qwen Code图标、context-safe点击导航与可选嵌入品牌。 |
| #11515 | configurable 256-workspace registration | 已合入默认256注册、operator覆盖、800 global session默认、扩展store与25 Channel owner准入；不含dormant runtime/LRU。 |
| #11653 | ACP child heap cgroup sentinel | 已合入spawn/status可用内存口径统一；无限/超宿主cgroup值回退host total，raise-only缺口仍保留。 |
| #11812 | standalone HTTP UUID fallback | 已合入；普通HTTP非回环浏览器缺少native `randomUUID()`时用安全随机字节生成UUID v4，caller ID与exact recovery不变。 |
...
| #6741 | runtime channel control | daemon runtime channel selection manager 与 `channel_control`。 |
...
8. **open/draft diff 不能写成 main 已落地能力**。#8093/#8824 为 closed draft，#9048/#10286/#10786/#11053/#11143 为 closed，只能作为历史前身或替代方案记录。#11015仍为open，只能作为当前diff记录。#10828/#11020虽为docs-only design，但#10828核心runtime已由#11207承接，#11020 Phase 2数据/UI已由#11054/#11208承接；#11308/#11309/#11311/#11322/#11323/#11337/#11339/#11398/#11428/#11447/#11515/#11653已合入，需按最终diff记录。#11053/#11143的flat-store ledger不能叠加到已合入的separate-store架构。#8743已由#9055 runtime PR承接。
...
#11322/#11323已合入#11208可见历史交互的两个后续。`MessageList`把150ms locate cooldown timer绑定到组件生命周期，新定位替换旧timer且unmount清理；`GlobalTurnNavigation`在layout phase完成session tail定位后才加载visible metadata，避免tail已缓存时仍请求最旧index page。真实缺页与用户滚动/键盘定位仍按需加载。
...
#11447已合入增强方案：按`promptId`从投影后transcript提取有界会话标题、本轮提问/回复，加入图标与workspace/standalone/Live目标；点击在所属实例验证context后打开原会话并清理split状态。`WebShellWithProviders`可配置默认值/品牌/图标，内置standalone只在storage确认没有保存偏好时默认开启，storage不可读保持关闭且不自动申请权限；文本清理保留泛型、比较符与围栏HTML。通知正文会进入系统通知中心/锁屏，页面仍须运行且权限仍需用户授予。

#11428已合入行为保持型容量常量解耦。该阶段CLI注册仍限25，ACP child partition仍以私有25做observe-only建模，Channel控制默认deadline仍为2,130,000ms；deprecated `MAX_DAEMON_WORKSPACES`只保留导入兼容。随附256测量和LRU设计本身没有实现注册扩容、runtime休眠、eviction或heap enforcement。

#11515随后已交付P1注册扩容：默认256，并支持operator-only `QWEN_SERVE_MAX_WORKSPACES=1..256`或embedded option；启动、完整持久化恢复、dynamic/scratch admission与store write共享同一boot-time上限。容量大于25时标准daemon默认总session限800；schema v1 store支持255条secondary与8 MiB，Channel控制独立限制25个含recovery保留项的owner并在capabilities/status公布。runtime仍eager构建，dormant/LRU、child内存enforcement与256 active workspace资源保证均未提供。

#11653已修复ACP child spawn误接受无限cgroup哨兵。`getAcpMemoryArgs()`现在复用daemon状态模型的`detectAvailableMemoryMb()`，只有正数且严格低于宿主的constraint才生效；v1/v2无限哨兵、等于或高于宿主的值回退host total。50%、16 GiB封顶、一次性cache、raise-only与`--expose-gc`不变，因此#8182的低内存授权缺口和完整enforcement仍未解决。

#11455也已把WebShell原始composer文本作为逐请求public声明送入daemon。REST/ACP route和bridge剥离可伪造metadata，只沿trusted context重建private声明；Session仅对fresh、非channel、非空白声明发布`UserPromptSubmit.submitted_prompt`，不会从request text或display projection推断。generic action、manual scheduled run、retry与server-restored queue继续省略。

## 2026-09-15 follow-up：HTTP非回环standalone session创建

#11812已合入TypeScript daemon SDK的standalone UUID兼容修复。普通HTTP非回环WebShell没有secure-context-only的`crypto.randomUUID()`时，SDK改用仍可用的`crypto.getRandomValues()`生成16个随机字节，设置UUID v4 version/variant位并格式化为小写ID；native API可用时保持原路径，caller显式ID仍优先且不读取browser crypto。
...
_生成于 2026-06-03；按个人 PR 口径更新于 2026-09-21_
