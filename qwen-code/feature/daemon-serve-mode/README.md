...
| # | 子文档 | 覆盖 |
|---|---|---|
| 01 | [HTTP 服务 / 路由 / 中间件链](01-http-server-and-middleware.md) | 中间件链顺序、路由表、bearer / --require-auth / mutate / CORS / host allowlist 五道闸、prompt route 202 + bridge-owned deadline / 权限响应超时 / access log、REST SSE stream lifecycle diagnostics（#8572 merged）、restore timeout 504/retryable error mapping（#8691 merged）与 `/health?deep=1.activeWork`（#8588 merged，#9042 open 补 background shell hold） |
...
### W33 2026-08-10 ~ 2026-08-13 daemon / serve follow-up
...
| #8890(open) | Conversations runtime foundation | 当前 open diff 抽出 Conversations runtime / store / registry 基础层，为 daemon-managed standalone session 铺底；不再是纯 docs-only draft。 |
...
| #9042(open) | background shell activeWork hold | 当前 open diff 让 Session-managed background shell running、terminal notification queued 与 parent continuation processing 上报 bounded `shell` activeWork hold，避免 idle cleanup/restart 误清理。 |
| #9048(open) | transactional resync / live-journal repair | 当前 open diff 把 authoritative resync 与 live-journal repair 纳入 provider-local restore coordinator：source read-only visible，candidate 离屏恢复并经 epoch/watermark/replay/ownership 校验后原子 commit。 |
...
| #9042(open) | background shell activeWork hold | 当前 open diff 把后台 shell 与 terminal notification 纳入 activeWork 类别协商和 idle/restart guard。 |
| #9048(open) | transactional WebUI resync/repair | 当前 open diff 让 WebUI authoritative resync 与 live-journal repair 走 source-retained / candidate-staged / atomic-commit 流程。 |
...
7. **`/health` deep 探针非真实 liveness**。`?deep=1` 只读 Map-size getter（`sessionCount`/`pendingPermissionCount`）和 #8588 已合入的 `activeWork` 状态；#9042 当前 open diff 会把 background shell hold 纳入同一 guard，但仍不 ping 各子进程。它能阻止“仍有后台 Agent/terminal notification/shell 时被当作空闲重启”，但检测不出"wedged 但仍计数/仍 active"的会话，真实 liveness 仍应靠 TCP/进程/trace/log 等观测。

8. **open/draft diff 不能写成 main 已落地能力**。#7967/#8093/#8245/#8383/#8890/#9007/#9042/#9048 仍为 open，#8824 为 closed draft；这些只能作为当前方案或历史前身记录。#8743 已由 #9055 runtime PR 承接，不再按未落地 draft 追踪。#7812/#7821/#7886/#7894/#7947/#7975/#7976/#7994/#8002/#8080/#8387/#8414/#8415/#8423/#8450/#8462/#8464/#8469/#8508/#8572/#8588/#8620/#8691/#8833/#8852/#8882/#8891/#8892/#8893/#8911/#8931/#8932/#8933/#8939/#8947/#8954/#8955/#8990/#9055 已按 merged diff 更新。
