...
> **状态标注**：本文凡涉及 **#4432（Phase 4b retry，MERGED）** 的符号均显式标注 `【#4432 已合入】`；#7667/#8150/#8176/#8180/#10016 已按 merged diff 更新。
...
| #10016 | MERGED（2026-08-26） | private LLM context usage attribute | 给每个非 internal physical `qwen-code.llm_request` attempt 增加 bounded/versioned JSON 分类，并用 provider input total 归一化。 |
...
注意 idle 超时路径（L497–509）走的是另一个 `endLLMRequestSpan`（`success:false, error:'Stream span timed out (idle)'`），**不带 ttftMs**——因为超时意味着消费者放弃迭代，首 token 时延已无意义。`spanEndedByTimeout` 闸门（L556/L586/L615）确保 finally 不会对已超时结束的 span 重复打点。【#4432 已合入】会把 `retrySnapshot` 也灌进这条超时路径（见后文「重试可见性」R2#8）。
...
> 本节全部代码来自 **PR #4432（Phase 4b，MERGED）**，`gh pr diff 4432 --repo QwenLM/qwen-code`。行号以 PR diff 为准，合入后漂移。
...
【#4432 已合入】`LoggingContentGenerator` 侧新增 `snapshotRetryMetadata()`：
...
【#4432 已合入】`telemetry/types.ts:ApiRetryEvent` 字段：`event.name:'api_retry'`、`model`、`prompt_id?`、`attempt_number`（1-based 单调，对齐 ALS `attempt`）、`error_type?`、`error_message`、`status_code?`、`retry_delay_ms`、`duration_ms`、`subagent_name?`。
...
【#4432 已合入】`telemetry/loggers.ts:logApiRetry`：
...
【#4432 已合入】`telemetry/session-tracing.ts:endLLMRequestSpan` 把公式从
...
【#4432 已合入】`session-tracing.ts:LLMRequestMetadata`（L46）的三个字段在 4b 中**重新定义了语义**（接口本身在 main 已前向声明，恒 `undefined`）：
...
## `qwen-code.context.usage`（#10016 merged）

最终实现为每个非 internal physical LLM attempt 在同步 start prelude 中构造 request-local `ContextUsageV1`。分类来源是已经交给 `LoggingContentGenerator` 的 provider-neutral logical request：effective system instruction、实际发送的 tool declarations、memory、已加载 skill body 和 structured messages。它只读 committed in-memory cache，不调用 `SkillManager.listSkills()`，tracing disabled 或 internal prompt 时不做 snapshot。
...
属性是最长 1024 字符的 private JSON string，只含 version、数字 aggregate 和 `estimated:true`；不记录内容、路径、工具名、模型/session/user id，也不新增 metric/log/session attribute。snapshot、validation 或 serialization 失败均 best-effort 省略。#10016 已合入，真实 OTLP backend 与生产开销尚未验证。
...
- interaction span 作为 GenAI Agent invocation：operation=`invoke_agent`，`gen_ai.agent.name=qwen-code`，并带 conversation/session identity 与 prompt-scoped owner。
...
- headless JSON Schema verdict 只绑定 UserQuery/Retry owner 并跨 tool continuation 继承；Cron、Notification、Teammate 和 runtime Goal drain 的纯文本输出不再误报 `structured_output_missing`。
- Goal finalization 与 headless terminal failure 保留真实 bounded error message，而不是泛化为 hook/phase label。
...
> 【#4432 已合入】标注的实现和测试已进入 `main`；本文未独立复跑该历史测试集。

### #10016 — LLM context usage span attribute（merged）
