...
| [#9121](https://github.com/QwenLM/qwen-code/pull/9121) | MERGED | main agent tracing edge cases | 已合入，修正 budget abort、provider 吞 abort、TUI deferred tool batch owner、structured-output owner 与 Goal/headless diagnostic message |
...
3. **`tool.blocked_on_user` / `hook` 仍恒在主线但 subagent span 缺位**：Phase 2 的两类 span 已合入；但 Phase 3 的 `qwen-code.subagent`（#4410）已合入，并发子 agent 的 llm_request/tool 子 span 会平铺挂在共享 interaction 下，无法区分归属。
