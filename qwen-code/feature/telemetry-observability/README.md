...
| #8743 | selective session restore stage telemetry（draft open） | 设计 future stages：`transcript_index`、`resume_state_select`、`selected_record_read`、`history_replay`、`runtime_initialize`、`post_replay_services`；只允许 bounded counts/bytes/booleans/durations/cache state。 | Daemon restore design |
...
| #8743 | selective restore stage telemetry（draft open） | 设计把 restore telemetry stage 细分为 transcript index、resume state selection、selected record read、history replay、runtime initialize 和 post-replay services，便于后续证明 selective restore 不再重复 full materialization。 | Daemon restore design |

#8691/#8743 的 restore telemetry 保持低敏：只记录 session id、restore action、channel id、timeout、public/late/cleanup/quarantine 结果、阶段耗时、bounded counts/bytes 和 cache state；不记录 transcript 内容、用户 prompt、record id、cursor、文件路径正文或 auth data。telemetry/logger 失败不能改变 restore timeout、cleanup、quarantine 或 selective projection 行为。

### 6.11 OpenAI API log retention
