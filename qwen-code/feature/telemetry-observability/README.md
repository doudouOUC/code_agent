...
13. **#8469/#8743 仍是 draft open，#8691 仍是 open**：repeated tool execution failure guard telemetry、session restore timeout telemetry 与 selective restore stage telemetry 只记录当前 diff/设计方案。若后续阈值、mode、LoopType、execution outcome、restore timeout/error code、cleanup/quarantine、stage timing 或 selective restore stage 名调整，本文需要按最终 merged diff 再同步；当前不能把 `qwen-code.repeated_tool_failure_guard`、#8691 的 restore telemetry 或 #8743 的 selective restore stages 视为 `main` 已落地事件。#8572 的 `qwen-code.daemon.sse.*` 已按 merged diff 更新。
