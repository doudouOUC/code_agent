...
> 关联 PR：**#3879**（反应式压缩）、**#3985**（反应式压缩加固）、**#3872**（会话记录瘦身，修复 #3822）。
...
| 维度 | 主动阈值压缩（已有） | 反应式溢出压缩（#3879/#3985） | 持久层记录瘦身（#3872） |
|------|------|------|------|
| 触发时机 | 发送**前**（预防） | provider 拒绝**后**（恢复） | 工具结果**落盘时** |
| 压缩对象 | 对话历史（live history） | 对话历史（live history） | JSONL 存档里的 `FileDiff` 字段 |
| 触发依据 | token 估算越过 `auto` 阈值 | 报错文案被分类器判定为溢出 | 字段字符数越过 char limit |
| 是否影响发给模型的内容 | 是 | 是 | 否（只动存档，不动 live） |
| 关键符号 | `ChatCompressionService.compress` cheap-gate | `getContextLengthExceededInfo` + reactive 分支 | `sanitizeToolCallResultForRecording` |
