...
> 关联 PR：**#3879**（反应式压缩）、**#3985**（反应式压缩加固）、**#3872**（会话记录瘦身，修复 #3822）、**#5042**（超大工具结果外置）、**#5111**（active tool result history 累计预算）、**#5865**（compression summary side-query streaming）、**#5868**（configurable auto-compact threshold + Stop hook context usage）。
...
| 维度 | 主动阈值压缩（已有 + #5868） | 反应式溢出压缩（#3879/#3985） | 持久层记录瘦身（#3872） | 超大工具结果外置（#5042） | active tool result history 预算（#5111） |
|------|------|------|------|------|------|
| 触发时机 | 发送**前**（预防） | provider 拒绝**后**（恢复） | 工具结果**落盘时** | 工具结果进入上下文前 | provider request 前 |
| 压缩对象 | 对话历史（live history） | 对话历史（live history） | JSONL 存档里的 `FileDiff` 字段 | 单个超大 tool result 文本 | 多轮累计 compactable tool result history |
| 触发依据 | token 估算越过 `auto` 阈值；#5868 后比例项可由 `context.autoCompactThreshold` 配置 | 报错文案被分类器判定为溢出 | 字段字符数越过 char limit | 单条结果超过 28K 字符 | `toolResultsTotalCharsThreshold` 累计字符预算 |
| 是否影响发给模型的内容 | 是 | 是 | 否（只动存档，不动 live） | 是（上下文只留 stub + preview + 文件指针） | 是（旧 compactable result 变 cleared placeholder） |
| 关键符号 | `ChatCompressionService.compress` cheap-gate / `computeThresholds(window, pct?)` | `getContextLengthExceededInfo` + reactive 分支 | `sanitizeToolCallResultForRecording` | persisted tool-result stub / `tool-results/<callId>.txt` | microcompaction size trigger |
...
### 3.2.2 可配置 auto-compact 阈值与 Stop hook context usage（#5868）

#5868 补的是主动阈值压缩的可运维面，而不是新增一种压缩算法。

**配置项**：新增 `context.autoCompactThreshold`，值域为 `(0, 1]`，非法值回落默认 `0.7`。`computeThresholds(window, pct?)` 接收该比例，并被四个生产调用点复用：

- `chatCompressionService.ts`：auto-compaction cheap-gate；
- `geminiChat.ts`：发送前 hard-cap rescue 的阈值计算；
- `contextCommand.ts`：`/context` 展示；
- `useContextualTips.ts`：上下文提示阈值。

qwen-code 的阈值不是单纯 `window * pct`，而是三层阈值里取 `max(proportional, absolute)`。absolute branch 形如 `effectiveWindow - 13K`，所以大窗口模型上低于约 0.7 的自定义比例可能没有可见效果；该配置主要影响 32K-128K 这类小窗口模型，或用户把阈值设得比 absolute branch 更高的场景。

**Stop hook stdin**：Stop hook payload 新增三个可选字段：
...
`packages/core/src/hooks/context-usage.ts:buildContextUsage(windowSize, inputTokens)` 负责构造数据；`Session.ts`、`client.ts` 和 messageBus Stop handler 都接入该 helper。hook 脚本因此能观察 context fullness，例如在超过自定义阈值时提示用户 `/compact`。

边界：Stop hook **只能观察和追加上下文**，不能直接触发 `/compact`；#5868 也不改变三层阈值公式，且不修正 `cxt` 百分比显示的 API 限制。
...
| **#5868** | configurable threshold + hook context usage | 新增 `context.autoCompactThreshold` 并透传到 `computeThresholds(window,pct?)` 的四个调用点；Stop hook stdin 增加 `context_usage`、`context_limit`、`input_tokens`，让外部脚本能观察上下文占用。 |
...
#5868 的 `context.autoCompactThreshold` 只配置 proportional branch。对大 context window，`effectiveWindow - 13K` 的 absolute branch 往往更高，因此把阈值设成 0.5/0.6 可能不会让 `/context` 的 Auto threshold 明显提前。这是当前三层阈值公式的设计结果，不是配置没有生效。若要彻底控制大窗口触发点，需要后续重新设计 absolute branch。
...
### PR #5868 — configurable auto-compact threshold and hook context usage

- `config.ts` / settings schema / docs：新增 `context.autoCompactThreshold`，值域 `(0,1]`，非法值回落默认 0.7。
- `chatCompressionService.ts` / `geminiChat.ts` / `/context` command / contextual tips：`computeThresholds(window,pct?)` 统一接入自定义比例，避免展示和实际压缩门槛不一致。
- `hooks/context-usage.ts`：新增 `buildContextUsage(windowSize,inputTokens)`，无效输入返回 `undefined`。
- Stop hook callers：`Session.ts`、`client.ts`、messageBus Stop handler 把 `context_usage`、`context_limit`、`input_tokens` 透传到 hook stdin；hook 只能观察，不能直接触发 compact。
