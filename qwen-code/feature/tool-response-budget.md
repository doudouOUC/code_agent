...
> 当前记录：#7323 已合入；#7470 已合入并补 Shell 无 artifact truncation 回归测试；#8450 已合入，只在 ACP transport 边界裁剪 textual tool-result projection，不改变 canonical transcript 或 model-facing response。
...
- #8450 已合入，且它是 transport-display projection，不是模型上下文预算；后续如字段集合、预算或 marker 调整，需要按新 diff 再更新。
