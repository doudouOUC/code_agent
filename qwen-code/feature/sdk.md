...
- **REST stream lineage（#8572 merged）**：`DaemonSessionClient` 维护 `lastAcceptedRestStreamId` 和 `hasAcceptedRestStream`；REST 订阅默认首连 `initial`、后续 `resume`，WebUI 等调用方可传白名单 `sseConnectReason`。SDK 会在运行时删除 untyped caller 传入的 `clientId`、`previousSseStreamId` 和 `onSseStreamAccepted`，避免外部覆盖 session-owned stream identity。
...
#### 4.3.1 standalone session lifecycle（#10179 merged / #10294 merged）
...
7. **caller-supplied session ID override 已合入**。#8415 让 TS/Java/daemon MCP 在 request 中带 `sessionId`，并要求 daemon capability gate 与 response verification；旧 daemon 或未广告 capability 的 daemon 必须走自动生成 session id，不能把本地 requested id 当作已创建事实。
...
### #8415 — caller-supplied session ID override（merged）
...
### #8572 — REST SSE stream observability（已合入）
...
### #8691 — restore timeout derivation（merged）

- `DaemonClient.capabilities()`：缓存 `/capabilities.limits.sessionRestoreTimeoutMs`，供后续 load/resume 派生 timeout。
...
### #12008 — workspace runtime stop SDK（merged）
...
### #12301 / #12390 / #12391 — Java Runtime Broker state（foundation merged, Tool Execution open）
...
- #12391 仍为 open：当前 diff 增加 Tool Execution identity、dispatch claim、UNKNOWN no-replay 与 open cancellation path 的内存参考实现；没有 JDBC、真实 dispatch 或 public API，调用方不能把它视为已发布 SDK surface。
