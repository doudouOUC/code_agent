...
2. **远程守护进程式（TS daemon SDK，#4226 / #4360）**：调用方通过 HTTP + SSE 连接一个常驻的 `qwen serve` 守护进程（见 daemon/serve 技术方案）。一个 daemon 绑定一个 workspace，多个客户端（Web UI、IDE、TUI、第三方适配器）可同时 attach 到同一会话，共享 MCP 连接池、跨客户端权限协商、断线重连续传事件。#8002 还把 workspace text file read 的 `cursor`、`hasMore` 与 `nextCursor` 暴露为 additive 字段；#8572 当前 open diff 让 REST SSE transport 学习 accepted stream id，并向 daemon 报告可解释的 reconnect reason 和 previous stream lineage。适合「agent 即服务」的远程、多端、协作场景。
...
这些 SDK 共享「把 agent 当作可编程对象」的目标，但落在不同的进程边界与协议上。
...
| 维度 | Python SDK | TS daemon SDK | Java daemon SDK alpha |
| --- | --- | --- | --- |
| 进程边界 | 本机 fork 一个 CLI 子进程 | 通过网络连远端常驻 daemon | 通过网络连远端常驻 daemon |
| 传输协议 | stdio 上的 stream-json（JSON Lines） | HTTP（请求/响应）+ SSE（事件流） | HTTP admission + resumable SSE |
| 入口符号 | `query()` / `query_sync()` | `DaemonClient` / `DaemonSessionClient` | `DaemonClient` / `DaemonSessionClient` / `DaemonSession` |
| 多客户端 | 否（1 调用方 = 1 子进程） | 是（多端 attach 同一会话） | 是（daemon thread-scoped session） |
| 包名/版本 | PyPI `qwen-code-sdk` (`pyproject.toml`) | npm `@qwen-code/sdk` (`package.json` v0.1.8) | Maven `com.alibaba:qwencode-sdk`，#7463 准备 `0.1.0-alpha` |
| 权限模型 | `can_use_tool` 异步回调（拉模式） | SSE `permission_request` 事件 + `respondToPermission`（推模式 + 投票） | typed events + permission response API |
...
目录 `packages/sdk-typescript/src/daemon/`：`DaemonClient.ts`（HTTP 客户端）、`DaemonSessionClient.ts`（会话作用域包装）、`events.ts`（类型化事件 schema + 归一/reducer）、`types.ts`（wire 类型）、`sse.ts`（SSE 解析）、`DaemonAuthFlow.ts`（设备码登录助手）、`ui/normalizer.ts`（UI 事件归一）。包名 `@qwen-code/sdk`。
...
- **SSE 订阅** `subscribeEvents`（`DaemonClient.ts:1564`）：仅对**连接阶段**（请求→收到响应头）套 `fetchTimeoutMs`，body 流本身长寿不计时；校验 `content-type` 必须含 `text/event-stream`（防代理把 SSE 替换成 JSON/HTML 导致「静默零事件」）；用 `Last-Event-ID` 头实现断点续传，可选 `?maxQueued=N`；最终 `yield* parseSseStream(res.body, signal)` 产出 `DaemonEvent` 帧。#7269 修复 REST SSE request cleanup：每条订阅登记 request-local `AbortController`，iterator early return/consumer throw/stream error/connect error/dispose 都会 abort 底层 fetch 并 cancel body stream，防 EventBus subscriber 与 pending read 泄漏。#8572 open diff 额外发送 `X-Qwen-Client-Id`、`connectReason`、`previousStreamId`，并从 `X-Qwen-SSE-Stream-Id` response header 学习 daemon accepted stream id；这些字段仅用于诊断，旧 daemon 或剥离 header 时继续按原 cursor 工作。

### 4.2 workspace file read cursor fields（#8002）

#8002 扩展 `DaemonClient.readWorkspaceFile()` 与 `WorkspaceDaemonClient.readFile()` 的请求/响应类型。请求可带 opaque `cursor`，并要求调用方不要同时传 `line`；响应可包含 `hasMore` 与 `nextCursor`。客户端应先 gate `workspace_file_read_cursor` capability，再展示“下一页”按钮或自动继续读取。

这些字段只是 HTTP `/file` 的 additive protocol mirror：cursor 绑定服务端 snapshot/descriptor，不能被当成长期 bookmark；遇到 `hash_mismatch` 应提示文件已替换或截断并重新从 `line` 读取。旧 daemon 不返回这些字段时，SDK 保持原有一次性 line-window 行为。

### 4.3 `DaemonSessionClient`：会话作用域包装
...
- **REST stream lineage（#8572 open）**：`DaemonSessionClient` 维护 `lastAcceptedRestStreamId` 和 `hasAcceptedRestStream`；REST 订阅默认首连 `initial`、后续 `resume`，WebUI 等调用方可传白名单 `sseConnectReason`。SDK 会在运行时删除 untyped caller 传入的 `clientId`、`previousSseStreamId` 和 `onSseStreamAccepted`，避免外部覆盖 session-owned stream identity。
...
- `DaemonClient`: daemon HTTP client，负责 create/load/resume、prompt admission、capabilities、permission response、cancel、heartbeat、close/detach 等路由。
- `DaemonSessionClient`: 绑定一个 session/clientId，维护 SSE cursor 和 prompt pending map。
- `DaemonEvent` / `PromptTerminal` / `PromptAcceptance` / `PromptTextResult`: typed event、admission 和 terminal result。
- `SseReader`: strict SSE parser，校验 UTF-8、JSON、frame shape、event id gap 与 compressed stream。
- `DaemonHttpException`、`PromptAdmissionUnknownException`、`PromptOutcomeIndeterminateException`、`MutationOutcomeUnknownException`、`DetachOutcomeUnknownException` 等 exception 类型：把“无法证明成功”的路径 fail closed。

传输策略是 **non-retried POST admission + resumable SSE**。prompt admission 成功后，从 daemon 返回的 watermark 开始订阅 SSE；客户端发送 `Accept-Encoding: identity`，用 `Last-Event-ID` 续传，只在 observer 成功处理事件后推进 cursor，并对 replayed events 去重。`promptText()` 只是 bounded convenience API，底层仍依赖 typed terminal；如果 terminal 缺失、事件 gap、压缩流、observer failure 或 mutation response 丢失，API 不返回部分成功。
...
#7603 在此基础上消费 #7458 的 restart-safe cursor。`PromptAcceptance` 保存 optional `eventEpoch`，Java client 在初始 SSE request 和 reconnect 上发送 `X-Qwen-Event-Epoch`，并从 validated SSE response 学习 epoch；response epoch mismatch 会在消费事件前 fail closed。该 follow-up 还收紧 truncated JSON、null data、SSE gap、terminal-before-202 buffering 和 teardown ordering，并通过 `scripts/run-java-daemon-sdk-e2e.ts` 扩展 real daemon E2E。

### 4.5 协议补全的 SDK 消费（#4360：serverTimestamp / provenance / errorKind / state_resync_required）
...
| #8572 | OPEN | REST SSE stream observability | TS daemon SDK / WebUI 传递 connect reason、previous stream lineage，并从 `X-Qwen-SSE-Stream-Id` 学习 accepted stream id；字段仅用于诊断，兼容旧 daemon。 |
...
8. **REST SSE stream observability 仍是 open diff**。#8572 的 stream id、connect reason 和 previous stream lineage 只是诊断面，不能替代 `eventEpoch` / `Last-Event-ID` cursor、session id 或 client id；header 被代理剥离时客户端必须继续正常工作。
...
### #8572 — REST SSE stream observability（当前 open）
