...
- **超时模型** `fetchWithTimeout`（`DaemonClient.ts:334`）：短命方法套 `fetchTimeoutMs`，且把 body 读取也纳入计时窗口（避免代理 mid-body 卡死）；`prompt()` 与 SSE 订阅**绕过**该超时（模型+工具回合可达数分钟），靠 `AbortSignal` 取消。`restartMcpServer`/runtime MCP 增删用更宽的 `MCP_RESTART_DEFAULT_TIMEOUT_MS = 330_000`（比 daemon 侧 300s 上限多 30s headroom，`DaemonClient.ts:122`）。多信号用 `composeAbortSignals`（`AbortSignal.any` + polyfill，`DaemonClient.ts:2002`）合并。
