...
> 涉及 PR：#5231（Workflow tool token budget + per-run UI surfacing）、#5600（Dynamic Workflows port completion）、#5679（agent/workflow integer env strict parse）、#5740（workflow snapshot prune path-traversal guard）、#5930（workflow stall env decimal seconds parse）。
...
- `QWEN_CODE_WORKFLOW_STALL_SECONDS=<int>`：stall watchdog 秒数。#5930 后只接受普通十进制整秒；精确 `0` 表示禁用 watchdog，`0x10`、`1e3`、`1.0`、`2.5` 等写法回落默认值而不是被 `Number()` 宽松解析。
...
#5930 收紧了 stall watchdog env 的解析：`QWEN_CODE_WORKFLOW_STALL_SECONDS` 只接受 `/^\d+$/` 这类十进制整数字符串，且 `0` 是唯一的禁用值。此前 `Number(raw)` 会接受 `0x10`、`1e3`、`1.0`、`2.5`，这些写法在 shell/env 里容易被误以为是十进制秒数；现在非法写法直接忽略并使用默认 watchdog 配置，避免部署环境里一个非规范字符串改变 workflow 卡死检测行为。
...
| #5930 | merged | `QWEN_CODE_WORKFLOW_STALL_SECONDS` 改为十进制整秒解析；`0` 禁用，其它非规范 numeric-looking 字符串回落默认值。 |
