...
| Broker 持久化与恢复 | Binding、Runtime Session 和 Tool Execution 共享持久事实源，支持 CAS、租约接管与原执行查询 | upstream 已合入 binding/session JDBC 与 Broker service core；#12445 以 merge commit `d2e4cc74d5` 合入 Tool Execution JDBC（评审 head `15d395bc40`），并通过 H2 与真实 MySQL 大小写不敏感排序规则验证；该 head 还修复了 `$ref`/`@type` 被 fastjson2 解释以及正指数 BigDecimal 被收窄的问题，平行 #12458 已关闭并由它取代。#12477 当前 head `64a68f2279`（实现 commit `b29426173c`）已在物理执行前补 owner/generation 最终复核，#12478 当前 head `ea6c0d9739`（实现 commit `ddb65ba448`）已消除 MySQL session time zone 与服务时钟的租约偏差并通过三种 offset 的 H2/真实 MySQL 验证；两者仍为 open。`e666150153` 已接 Spring JDBC/Flyway V3、AES-GCM seed 和 reconcile/attest gate，并修复真实 outer gate 404。Broker 启用时缺数据库或密钥配置会失败，不回退内存；持久 `READY` 仍须 reconcile + attest + CAS 后才能打开本 JVM gate；#12447 已合入 route 单源和共享 conformance 基础，具体 Java transport、required 跨语言 CI 与生产挂载尚待完成 |
