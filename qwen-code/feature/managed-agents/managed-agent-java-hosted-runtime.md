...
| Broker 持久化与恢复 | Binding、Runtime Session 和 Tool Execution 共享持久事实源，支持 CAS、租约接管与原执行查询 | upstream 已合入 binding/session JDBC 与 Broker service core；#12445 head `15d395bc40` 提交 Tool Execution JDBC，并通过 H2 与真实 MySQL 大小写不敏感排序规则验证；最新 head 还修复了 `$ref`/`@type` 被 fastjson2 解释以及正指数 BigDecimal 被收窄的问题。`e666150153` 已接 Spring JDBC/Flyway V3、AES-GCM seed 和 reconcile/attest gate，并修复真实 outer gate 404。Broker 启用时缺数据库或密钥配置会失败，不回退内存；持久 `READY` 仍须 reconcile + attest + CAS 后才能打开本 JVM gate；#12447 已合入 route 单源和共享 conformance 基础，具体 Java transport、required 跨语言 CI 与生产挂载尚待完成；接线前还需消除 MySQL 会话时区与服务 UTC 时钟的租约判断偏差，并在执行前复核 dispatch owner/generation。第三轮验证已将当前 head 标记为 merge-ready，一名 human collaborator 已 approve 且代码 CI 全绿；PR 仍受此前 `CHANGES_REQUESTED` 与待完成自动评审阻塞 |
