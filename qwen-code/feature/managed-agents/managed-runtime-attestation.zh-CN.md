...
| A1：路由单源 | route manifest、精确 allowlist、16 KiB 限制、no-store | upstream #12447 已合入；#12506 已提交 attestation-only 真实进程挂载，尚待评审合入，生产 Hosted profile 仍待后续 |
| A2：契约门禁 | 语言无关 fixtures/schema、TS/Java 消费、required CI | #12447 已提供共享文件、TypeScript 行为测试与 Java fixture consumer；#12506 补 TypeScript 子进程 boot/attest/shutdown 验证，具体 Java client、跨语言进程 E2E 与 required CI lane 仍未完成 |
