...
2026-09-10 文档核对：M2 的 PDF 物理取消已有限定验收，Gateway 模型转写仍仅设计，M3 客户端展示与产物访问待实现；不得将整个 M2 标为完成。当前按[首阶段计划](managed-agent-daemon-default-plan.md)优先接默认引擎和普通 Web Shell/SDK，保留已验证媒体路径，完整媒体扩展后置。
