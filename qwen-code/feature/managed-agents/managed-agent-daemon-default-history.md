...
本文归档代码基线 `a836081466` 时的原总方案全文，保留历史切片、失败、验收范围和当时判断。下文的“当前”“下一步”、三处入口及先完成全部能力再切换等表述只描述记录当时的状态，不作为现行实施指令。当前以 [HTML v1.2](managed-agent-dual-path-architecture.html) 和[双链路方案](managed-agent-java-hosted-runtime.md)为准：执行引擎设计是 B 的接缝，R1～R5 保留为历史切片；架构目标与实现完成状态分别核验。归档日期：2026-09-10；状态校正日期：2026-09-18。
...
现有工具执行链中的 Hook 回执仍须准确保留，不把错误写成成功；这一回归要求不等于重新开展完整 Hook 迁移。阶段任务与尚未完成的证据见 [首阶段实施计划](managed-agent-daemon-default-plan.md)。
