...
本文归档代码基线 `a836081466` 时的原总方案全文，保留历史切片、失败、验收范围和当时判断。下文的“当前”“下一步”、三处入口及先完成全部能力再切换等表述只描述记录当时的状态，不作为现行实施指令。当前范围、四处入口、完成状态与验收要求以[默认替换总方案](managed-agent-daemon-default.md)、[执行引擎详细设计](managed-session-execution-engine.md)和[首阶段计划](managed-agent-daemon-default-plan.md)为准。归档日期：2026-09-10。
