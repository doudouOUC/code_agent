...
> **2026-09-18 最新产品架构：** 请先阅读 [Java、Hosted Harness 与 Tool Runtime](managed-agent-java-hosted-runtime.md)。当前冻结链路为 `WebShell -> JavaAgentProvider -> Java Session/Prompt/SSE API -> Hosted Harness -> Java Runtime Broker -> Tool Runtime`。Java 是 Session/Turn/Event 权威层，Hosted Harness 负责模型与 Agent Loop，Tool Runtime 按需承担本地副作用；不实现 Java daemon-compatible 网关。P0～P9a 是历史实验与能力演进编号，不再直接代表最终产品部署拓扑。
>
> 最新实现状态：Hosted Harness、Java Managed 主链路、Runtime Broker durable 核心和本地完整进程 E2E 已完成；首个模型事件约 495 ms，Runtime ready 约 15.916 s，同一 Tool 物理执行一次且一次响应丢失后可恢复。`JavaAgentProvider`、真实 DataAgent/ACS 联合 E2E、双 JVM + MySQL 故障注入和公共 Agent API Adapter 仍待完成。

> 历史实验状态：P0～P8、Managed 会话展示与控制、P9a 本地 Runtime 自动激活实验曾推送到 [doudouOUC/qwen-code 的 feature/managed-agents-p0-p8 分支](https://github.com/doudouOUC/qwen-code/tree/feature/managed-agents-p0-p8)，代码锚点为 [a836081466](https://github.com/doudouOUC/qwen-code/commit/a8360814668b3dfdff72ad3d99cbcaf26dd009a9)。这些记录不代表当前产品部署状态，也不代表 [QwenLM/qwen-code](https://github.com/QwenLM/qwen-code) `main` 已具备相同能力。
> 历史快照日期：2026-09-11；现行产品方案更新日期：2026-09-18。
>
> 以下至“当前方案入口”之间保留 2026-09-11 及之前的专项实现与验收摘要，只用于追溯证据；其中的“本轮”“下一步”“尚未接线”等状态不覆盖上面的 2026-09-18 产品状态。
...
>
> 历史 daemon 路线目标：曾计划按 Session/Harness/Runtime 三层拆分后，让 Managed Agent 替换 daemon 默认执行实现。[首阶段实施计划](managed-agent-daemon-default-plan.md)和[默认替换方案](managed-agent-daemon-default.md)保留该路线的范围与证据；它们不再决定产品部署拓扑或当前实施顺序。[Runtime invocation v2](managed-agent-runtime-invocations.md)与[子任务及持久文件历史](managed-agent-child-scopes.md)中的 Tool Runtime 语义和专项验证仍可复用。
>
...
>
> 此前完成 [Grep 原生后端迁移](managed-agent-grep-tools.md)：Gateway 使用公共声明，Runtime 按绑定配置执行原生 rg/git grep/grep，等待所属进程组退出后结算取消与释放。517 项定向测试、16 组隔离验收和两轮人工自审通过，57 个不同构建文件摘要与当前产物一致；独立审查 Agent 因额度不可用，本次采用手工复审，不记为独立审查通过。后端损坏回退、权限差异、worktree 隔离、输出限制、部分读取后写入均有实际证据。
>
> 上一阶段完成 [NotebookEdit 原生迁移与审批修改](managed-agent-notebook-tools.md)：与 Read 使用所属 Runtime 的完整读取记录，完整 notebook 用户修改经私有 prepare 引用重新准备和审批，保留原生写入与父备份。689 项去重定向测试、真实完整 host 的读取/编辑/最终回复、两轮私有 wire 修改及实体父备份核验通过；55 项构建产物（含 bundle）摘要稳定，测试进程/端口/临时目录已清理。build/bundle/typecheck/lint/格式和两轮人工自审通过；当时独立 Agent 受额度限制，仅做手工复审。该阶段未覆盖用户端编辑器、取消时序及多媒体，取消时序后续结果见下文。
>
> 此前补齐 NotebookEdit 取消时序与 v2 执行回执：读取/备份期间取消不再继续写入；已写入后关闭 Session 保留成功回执并等待排空，新执行仍被拒绝。151 项去重定向测试、三组真实 worker 受控延迟验收、完整 build/bundle/typecheck、两轮自审与独立源码审查通过；56 项产物摘要一致，进程、端口及临时根已清理。详见 [Notebook 专项复验](managed-agent-notebook-tools.md)。
>
...
>
> 历史 daemon 路线未切换普通默认入口，也未触碰当时的 4170 预览或用户数据。该路线中的 cwd、信任、模型权限、MCP/Hooks/Channels 依赖识别、旧会话 sticky owner、失败不隐式降级以及专项工具验收边界仍是可复用约束；其“四处普通 factory 接线”与 daemon 默认切换顺序不再是当前产品计划。

## 当前方案入口

最新产品落地以 [Java、Hosted Harness 与 Tool Runtime](managed-agent-java-hosted-runtime.md) 为 canonical 文档。它定义 Java 权威层、常驻多租户 Hosted Harness、Java 内嵌 Runtime Broker、按需 Tool Runtime、`JavaAgentProvider`、首轮并行 warm、durable execution/`UNKNOWN`、Legacy 并存和公共 Agent API 演进顺序。下列全量设计、R1～R5 和 P0～P9a 文档继续保留详细契约与历史证据；发生部署拓扑或产品边界冲突时，以最新 canonical 文档为准。

### 文档状态与阅读规则

| 类别 | 如何使用 | 代表文档 |
| --- | --- | --- |
| 当前产品方案 | 决定组件所有权、部署拓扑、实现状态和 M0～M8 顺序 | [Java、Hosted Harness 与 Tool Runtime](managed-agent-java-hosted-runtime.md) |
| 可复用技术契约 | 复用 fencing、幂等、权限、工具、checkpoint、取消和恢复语义；部署位置以当前产品方案重新映射 | [私有协议](managed-agent-control-protocol.md)、[Harness](managed-agent-harness.md)、[Runtime invocation v2](managed-agent-runtime-invocations.md)及各专项文档 |
| 历史实施路线 | 保存 P0～P9a、D1～D5、R1～R5 的实验代码、失败、测试和当时决策；不用于指挥当前开发 | [daemon 默认替换](managed-agent-daemon-default.md)、[首阶段计划](managed-agent-daemon-default-plan.md)、各 P 阶段文档 |

旧文档中的“当前”“下一步”“产品目标”均按该文档的历史日期解释；只有标注为当前产品方案的文档可以决定现在的实施顺序。

如需核对完整能力面，可读[全量设计与交付覆盖表](managed-agent-full-design.md)：18 类能力均有专项契约和验收索引。其 R2/R5/F1～F8 顺序属于历史 daemon 路线；能力清单和失败语义可复用，产品优先级由最新 M0～M8 重新排序。

[Session / Harness / Runtime 全局架构](managed-agent-session-harness-runtime.md)保留 daemon 内三层拆分的完整契约：三类 lease、Session 逻辑接口、单一 transcript、checkpoint、事件投影、工具回执及恢复。产品部署已将 Session/Event 权威移到 Java，将完整 Agent 放到 Hosted Harness；旧 R2.S1～R2.S3 不再作为产品实施顺序。
...
[首阶段计划](managed-agent-daemon-default-plan.md)中的 R1～R5 是历史 daemon 迁移顺序，不再指挥产品开发；可复用的能力边界已映射到最新方案的 M0～M8。[历史调查与验收记录](managed-agent-daemon-default-history.md)继续原样保存失败、测试和当时结论，不用于判断当前完成度。

## 分阶段设计文档

P0～P9a、D1～D5 和 R1～R5 均保留为历史专题编号。当前产品实施顺序只以最新产品架构的 M0～M8 为准。

| 阶段                     | 文档                                                                                 | 主题                                                                                                        |
| ------------------------ | ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- |
| 最新产品架构             | [Java、Hosted Harness 与 Tool Runtime](managed-agent-java-hosted-runtime.md)          | 2026-09-18 canonical：Java 权威层、Hosted Harness、内嵌 Broker、按需 Runtime、WebShell 与 Agent API 演进    |
...
| R2.S1～R2.S3 全局架构    | [Session / Harness / Runtime 三层拆分](managed-agent-session-harness-runtime.md)     | 历史 daemon 内三层设计；职责契约可复用，产品部署映射见最新架构                                               |
| 全量设计                 | [C01～C18 全量设计与交付覆盖](managed-agent-full-design.md)                          | 所有能力的详细设计、决定清单和 F1～F8 顺序；设计完成不等同实现完成                                          |
| 存储                     | [记录格式、提交与限额](managed-agent-session-storage.md)                             | 历史 daemon JSONL authority；产品 Java DB/Event Store 映射见最新架构                                      |
...
| 恢复与运行               | [持久回执、平台与性能](managed-agent-recovery-operations.md)                         | 物理回执与故障门槛；产品使用 durable Broker、generation fencing 和 UNKNOWN                                |
| 历史总方案               | [daemon 默认执行替换](managed-agent-daemon-default.md)                               | 历史本地 daemon 默认替换范围；不再决定产品拓扑或实施顺序                                                    |
| 历史 R1～R5 计划        | [默认替换首阶段计划](managed-agent-daemon-default-plan.md)                           | 历史 daemon 迁移顺序；能力边界映射到最新 M0～M8                                                            |
...
| Harness                  | [完整 Harness 的装配与可恢复执行](managed-agent-harness.md)                          | Hosted 在线主链路与私有协议 v1 已实现；通用 checkpoint、安全点及完整可恢复替换仍待完成                     |
| 私有协议                 | [Session / Harness / Runtime 私有协议](managed-agent-control-protocol.md)            | fencing/接管语义继续复用；产品分别落在 Java↔Harness 与 Broker↔Runtime 私有协议                              |
| 调度与装配               | [Managed coordinator](managed-agent-coordinator.md)                                  | Java admission/事件/恢复与 durable Broker 核心已实现；真实产品 E2E 和分布式门禁待完成                      |

> 以下第 1 节起保留早期 Gateway/Runtime 两层方案作为历史背景；其中的部署和实施顺序不覆盖最新 Java/Hosted Harness/Runtime 产品架构及 M0～M8 计划。
...
历史计划曾优先推进 D1～D5 的 daemon 默认执行替换，并把 P9b Kubernetes adapter 作为后续工作。当前产品不以 Kubernetes 为协议前提，优先完成真实 Java/Hosted Harness/DataAgent 网络与恢复门禁；本地 IPC 和进程回收仍不能作为生产隔离或容量证据。
...
历史结论保留为：常驻模型执行层与按需 Tool-only Runtime 的拆分方向成立，P9a 证明了本地进程激活链路。现行产品结论进一步调整为：**模型与 Agent Loop 位于 Hosted Harness，Session/Turn/Public Event 与 Runtime binding 位于 Java，Tool Runtime 按需创建；下一步按 M0～M5 完成真实产品 E2E、分布式故障注入和 `JavaAgentProvider`，不再先切换 daemon 默认实现。**
