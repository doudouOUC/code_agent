...
更新日期：2026-09-10；代码核对基线 `a836081466`。完整范围与 C01～C18 能力/入口/状态/验收总表见[默认替换总方案](managed-agent-daemon-default.md)，历史 D1～D5 不再单独决定当前执行顺序。本计划落实用户最新范围和优先级调整。在保留当前 4170 预览、用户数据和已验证能力的前提下，先完成创建时的执行引擎选择，再让兼容范围明确的新 daemon 会话使用 Managed 默认实现。保持现有 Agent 行为，不把局部验收扩大为全部默认替换完成。
...
下一步完成 extension 的严格只读输入、MCP/Hooks 全来源依赖判定、用途兼容准入和全部普通 factory 接线；三个普通 workspace factory 及直接嵌入入口尚未切换。MCP 包括 extension、runtime 和 Session 注入，Hooks 受 extension、trust 与 disable 设置影响；Channels 入口保持原执行路径，热 attach 也须校验用途。[执行引擎详细设计](managed-session-execution-engine.md)列明消费范围和实施顺序。Gateway 转写及完整工作区/后台/历史能力不阻挡这一步。
