...
> **普通工具接线（2026-09-19，HTML v1.4）：** 普通工具首版同 Workspace 的工具轮次从起始快照至 history 持久提交串行占用；新 Turn 与 idle 回收按 binding 状态串行决策。具体准备、结果结算及释放次序见[接线设计](managed-agent-ordinary-tools-integration.md)，不因文件历史 checkpoint RPC 成功提前释放轮次。
