...
本项完整范围见[默认替换总方案](managed-agent-daemon-default.md)，优先级遵循[首阶段计划](managed-agent-daemon-default-plan.md)。先让同一工作区的普通 daemon 会话按创建时确定的执行引擎运行，再扩大 Managed 默认适用范围。完整媒体展示、Skills、后台任务和历史迁移继续延期，已验证能力保留。
