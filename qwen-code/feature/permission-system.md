...
> 代码基线：规则解析器 / 权限管理器位于 `main`；多客户端权限协调器（mediator，PR #4335）早期位于 `daemon_mode_b_main`，已随 #4490 进入 `main`；subagent plan lifecycle 阻断（#6087）、plan-required teammate leader approval（#6138）、main-session `exit_plan_mode` 显式用户批准（#6967）与 shell safety 三态事实层（#7053）已在 `main`；Plan-mode shell safety routing（#7172）当前为未合入 diff。
...
围绕这套三态决策，演进出九个相互独立又彼此咬合的子问题，正是本方案要解决的：
...
9. **Plan mode 下 shell/monitor 需要按三态 facts 路由（#7172 open）**：模型在 Plan mode 发起的 shell/monitor 不能和普通执行态一样处理。read-only 仍可用于调查，明确 write 必须在审批前拒绝，unknown 只能走一次性精确审批，并在执行前校验 Plan revision、cwd、permission policy 和 raw invocation 未漂移。

前三点、第八点和第九点是**单进程内**的规则/事实/Plan shell 策略（`packages/core`），第四点是**跨进程多客户端**的协调层（`packages/acp-bridge`，早期落在 `daemon_mode_b_main`，现随 #4490 进入 `main`），第五点把取消/超时结果接回 ACP turn loop，确保权限层的"取消"不会被后续工具继续执行稀释，第六点限制 subagent/team agent 对主会话 plan lifecycle 的控制权，第七点把 main-session Plan mode 退出从自动权限放行面里拆出来。本方案逐层展开。
...
### 3.3.1 Plan mode shell routing（#7172 open）

#7172 open diff 在 `packages/core/src/core/plan-mode-shell-policy.ts` 上消费 #7053 的三态 facts，只作用于 Plan mode 中模型发起的 `run_shell_command` / `monitor`。它不改变用户手动 `!command`、普通执行态和持久 permission rule：
...
6. **#7172 仍是 open diff。** 当前文档记录的是该 PR 的当前实现；合入前仍需以后续 review/CI 结果为准。它不处理用户手动 `!command`，也不把 unknown approval 持久化成规则，speculative interactive approval 仍保持 fail-closed。
...
### #7172 — Plan-mode shell safety routing（open）
