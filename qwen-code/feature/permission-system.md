...
> 代码基线：规则解析器 / 权限管理器位于 `main`；多客户端权限协调器（mediator，PR #4335）早期位于 `daemon_mode_b_main`，已随 #4490 进入 `main`；subagent plan lifecycle 阻断（#6087）与 plan-required teammate leader approval（#6138）已在 `main`；main-session `exit_plan_mode` 显式用户批准（#6967）当前为 open diff。
...
围绕这套三态决策，演进出七个相互独立又彼此咬合的子问题，正是本方案要解决的：
...
7. **main-session Plan mode 退出必须由人显式批准（#6967 open）**：`exit_plan_mode` 不能被 YOLO/AUTO、allow rule、permission hook 或 LLM plan approval gate 隐式批准。它需要一个独立的 `requiresUserInteraction()` 权限维度，让 host/user 的显式 allow 成为唯一退出 Plan mode 的信号，并用 plan revision guard 防止审批和执行之间的计划漂移。

前三点是**单进程内**的纯函数式规则引擎（`packages/core`），第四点是**跨进程多客户端**的协调层（`packages/acp-bridge`，早期落在 `daemon_mode_b_main`，现随 #4490 进入 `main`），第五点把取消/超时结果接回 ACP turn loop，确保权限层的"取消"不会被后续工具继续执行稀释，第六点限制 subagent/team agent 对主会话 plan lifecycle 的控制权，第七点把 main-session Plan mode 退出从自动权限放行面里拆出来。本方案逐层展开。
...
### #6967 — explicit user approval to exit Plan mode（open）
