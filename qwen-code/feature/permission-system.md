...
围绕这套三态决策，演进出六个相互独立又彼此咬合的子问题，正是本方案要解决的：
...
6. **shell 自杀命令前置拦截（#5409）**：用户或模型可能发出 `pkill` / `killall` / `taskkill` 这类过宽命令，把 qwen-code 自身或同一工作流依赖的进程一起杀掉。它必须在 permission allow、YOLO/AUTO、LLM classifier 之前被确定性拦住，否则一条宽泛 `Bash(*)` allow rule 就能绕过。

7. **远程持久规则管理与 auto 模式硬拦截（#5743/#5754）**：daemon/ACP 客户端需要读写 persistent allow/ask/deny 规则，但不能暴露交互式 `/permissions` dialog；AUTO 模式也不能只依赖 LLM classifier 判断破坏性命令。#5743 增加 workspace permissions API，#5754 在 `evaluateAutoMode()` 前加 deterministic destructive-command pre-filter。

前三点是**单进程内**的纯函数式规则引擎（`packages/core`），第四点是**跨进程多客户端**的协调层（`packages/acp-bridge`，早期落在 `daemon_mode_b_main`，现随 #4490 进入 `main`），第五点把取消/超时结果接回 ACP turn loop，确保权限层的"取消"不会被后续工具继续执行稀释，第六点补上 shell 自保层，第七点补齐远程持久规则管理和 AUTO 模式的 deterministic guard。本方案逐层展开。
...
### 3.6 broad shell self-kill guard（#5409）

#5409 在 shell 工具执行前增加 broad self-kill 命令拦截，覆盖会大范围终止进程的 `pkill` / `killall` / Windows `taskkill` 形态。它的定位不是“危险命令需要 ask”，而是“这类过宽命令不应该进入权限放行路径”：即便用户处于 YOLO/AUTO、已有 `Bash(*)` allow rule，或 LLM classifier 出错，运行时也不能让模型一把杀掉 qwen-code 所在进程族。

边界保持保守：

| 拦截 | 放行到后续权限判断 |
|---|---|
| 过宽、按进程名或全局模式匹配，容易杀掉 qwen-code / node / shell 会话的 `pkill`、`killall`、`taskkill`。 | 精确 PID、明确作用在无关目标且不符合 broad self-kill pattern 的命令。 |

它与 #5754 的 AUTO destructive-command pre-filter 是两层不同保护：#5409 是 shell validation/self-protection，所有 approval mode 都生效；#5754 是 AUTO 模式下针对 git/IaC 破坏性操作的用户意图 gate，只影响 auto classifier 前的决策。
...
| **#5196** | `/dev/tcp`/`/dev/udp` redirect 边界 | shell AST 权限解析不再把 Bash `/dev/tcp/host/port`、`/dev/udp/host/port` 重定向当作普通文件 I/O 建模，避免错误套用文件路径权限语义。 |
...
6. **shell AST 里的特殊 device path 不能按普通文件权限理解（#5196）。** Bash 的 `/dev/tcp` / `/dev/udp` 是网络重定向语法，不是真正的可写文件路径。权限解析和只读/写入判断需要保留这类 shell 语义差异，不能把它们下沉到 path rule 或 file I/O 模型里。

7. **self-kill guard 不替代完整命令沙箱（#5409）。** 它只拦截已知会大范围杀进程、且容易命中 qwen-code 自身的 shell 形态；它不尝试证明任意 shell 命令安全，也不处理所有可能导致会话中断的进程管理命令。其它危险写操作仍由权限规则、AUTO destructive pre-filter 和用户确认承担。
...
### #5196 — `/dev/tcp` and `/dev/udp` are not file I/O

- shell permission parsing 对 Bash `/dev/tcp/...`、`/dev/udp/...` 重定向做特殊处理，不把它们当成普通文件 read/write target。
- 这样路径权限规则不会误判网络重定向为本地文件操作，默认 shell permission 仍按 shell 语义和安全策略收敛。
- tests 覆盖 TCP/UDP device redirection 的权限建模回归。
...
### #5409 — broad shell self-kill commands

- shell command validation：新增 broad self-kill pattern，识别 `pkill`、`killall`、Windows `taskkill` 中会过宽匹配 qwen-code / node / shell 会话的形态。
- permission boundary：拦截发生在 allow rules、YOLO/AUTO 和 LLM classifier 之前，不能被 `Bash(*)` 或 auto 模式绕过。
- false-positive boundary：精确 PID kill、窄目标且不属于 self-kill pattern 的命令继续进入普通 permission evaluation。
- tests：覆盖 broad self-kill 正例，以及 targeted PID / unrelated process pattern 的放行回归。

### #5622 — validate ask_user_question answer indexes
...
### #5754 — destructive commands in AUTO mode

- `destructive-commands.ts`：维护 destructive git/IaC pattern 与 shell-indirection unwrap，匹配 raw command string。
- `autoMode.ts`：在 L5.3 classifier 前执行 Layer 0 pre-filter，返回 `blocked:destructive-command`，scheduler 显示明确 block reason。
- commit tracking：用于判断 `git commit --amend` 等操作是否涉及当前 session 产物。
- tests：114 个单元/集成用例覆盖 destructive pattern、bypass vector 和 auto-mode pipeline 集成。
