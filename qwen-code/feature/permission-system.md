...
> 涉及 PR：#3467、#3726、#4335、#5085、#5105、#5196、#5218、#5258、#5260、#5622、#5743、#5754。
...
6. **远程持久规则管理与 auto 模式硬拦截（#5743/#5754）**：daemon/ACP 客户端需要读写 persistent allow/ask/deny 规则，但不能暴露交互式 `/permissions` dialog；AUTO 模式也不能只依赖 LLM classifier 判断破坏性命令。#5743 增加 workspace permissions API，#5754 在 `evaluateAutoMode()` 前加 deterministic destructive-command pre-filter。

前三点是**单进程内**的纯函数式规则引擎（`packages/core`），第四点是**跨进程多客户端**的协调层（`packages/acp-bridge`，早期落在 `daemon_mode_b_main`，现随 #4490 进入 `main`），第五点把取消/超时结果接回 ACP turn loop，确保权限层的"取消"不会被后续工具继续执行稀释，第六点补齐远程持久规则管理和 AUTO 模式的 deterministic guard。本方案逐层展开。
...
### 3.6 6/23 follow-up：answer index、workspace rules API、AUTO destructive guard
