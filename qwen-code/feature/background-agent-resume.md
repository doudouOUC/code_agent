...
> 涉及 PR：#3739（背景 agent resume/continuation）、#4222（daemon session load/resume）、#5030（SDK / stream-json interrupted turn continuation without synthetic continue）、#5556（completed background sub-agent revive + transcript TTL）、#5679（agent/workflow integer env strict parse）、#5972（subagent output-token display）、#6015（non-VP multi-agent scrollback）
...
3. **SDK / stream-json interrupted turn continuation。** SDK caller 或 stream-json 控制端在进程崩溃、resume 或 mid-stream 中断后，过去只能放弃未完成 turn，或发一个合成 `"continue"` 用户消息污染 transcript。#5030 增加 `continueLastTurn()` / `continue_last_turn` 控制请求，从持久化历史判断 interrupted prompt / interrupted turn / none，并复用普通 turn loop 续跑。

4. **daemon 会话（跨进程，HTTP/SSE 客户端）。** `qwen serve` 守护进程把每个会话托管在一个 ACP 子进程里，远端 SDK/UI 客户端通过 HTTP + SSE 连接（见 daemon/serve 技术方案）。客户端断线重连、或新客户端想接管一个**已持久化但当前未在 daemon 中活跃**的会话时，仅靠 `create`/`attach` 语义不够——需要显式的 `load`（重放历史）和 `resume`（不重放）两条恢复路径。#4222（serve 生命周期工作的 PR6）补齐了 daemon HTTP 路由、ACP bridge 的 restore 生命周期、以及 TypeScript SDK 的 `load`/`resume` 方法。
...
| ③ SDK / stream-json interrupted turn | `Query.continueLastTurn()` / `continue_last_turn` control request | 持久化 chat history | interrupted prompt 复用 Retry；dangling tool calls 合成 error tool result；clean history no-op | `turn-interruption.ts` + non-interactive session work queue |
...
flowchart TB
  ...
  subgraph SDKCONT["③ SDK / stream-json interrupted turn continuation (#5030)"]
    direction TB
    CL["Query.continueLastTurn()<br/>continue_last_turn"] --> CLS["turn-interruption.ts<br/>classify persisted history"]
    CLS --> IP["interrupted_prompt<br/>复用 retry send path"]
    CLS --> IT["interrupted_turn<br/>补 synthetic error tool results"]
    CLS --> NO["none<br/>idempotent no-op"]
  end
  ...
  CORE -.->|"共同原则：落盘事实优先重建上下文"| SDKCONT
  SDKCONT -.->|"共同原则：落盘事实优先重建上下文"| DAEMON
```
...
## 5. completed background agent revive 与 transcript TTL（#5556）
...
## 6. background agent 上限 env 严格解析（#5679）
...
## 6.1 subagent UI 口径与非 VP scrollback（#5972/#6015）

#5972 统一了 subagent / background-agent 的 token 展示口径。此前多个 UI 面使用 `executionSummary.totalTokens`，这个值是所有 API round 的 `total_tokens` 累计，包含大量 prompt / cached prompt throughput。对于本地模型或多轮工具调用子 agent，结果卡可能显示“数百万 tokens”，用户容易误以为超过上下文窗口。修复后 CLI、Web Shell、WebUI 的 agent card / status / panel 统一优先展示 `outputTokens`，即模型实际生成的 token；旧 payload 没有 `outputTokens` 时才 fallback 到 `totalTokens`。

受影响面包括：

| UI 面 | 行为 |
|---|---|
| CLI `ToolMessage` / `InlineParallelAgentsDisplay` / Arena views | 完成卡、并行 agent 展示、arena status 里的 token 数改成 output-only。 |
| `LiveAgentPanel` / `BackgroundTasksDialog` | 运行中和历史背景 agent 的 token 展示与 loading indicator 对齐。 |
| Web Shell / WebUI agent cards | `TasksStatusMessage`、`ToolGroup`、`ParallelAgentsGroup`、`SubAgentPanel`、`AgentToolCall` 读取 optional `outputTokens`，兼容旧 daemon payload。 |

#6015 修的是非 VP（默认）终端模式下 multi-agent 运行时 transcript 无法上滚的问题。根因有两个：纯 parallel inline panel 在 live phase 重复渲染 running subagents，和底部 `LiveAgentPanel` 形成两份名册，把 Ink 动态帧撑过终端高度并触发每帧清屏；同时 collapsed thinking block 为点击展开启用 SGR mouse tracking，使宿主终端不再处理原生 scrollback。修复后 running subagents 交给 `LiveAgentPanel` 独占，inline panel 只保留 terminal rows 并有 `availableTerminalHeight` backstop；`useMouseEvents` 默认只在 VP 模式启用，真正需要鼠标滚动的 surface 用 `bypassVpGate` 明确豁免，非 VP 下 thinking block 仍可用 Alt+T 展开。

这两个 PR 不改变 background-agent 的持久化 / resume / registry 状态机；它们修的是用户理解和观察长跑 subagent 的 UI 表达：token 数代表“实际输出”，默认终端 scrollback 在 `/review` / multi-agent fan-out 时保持可用。

## 7. interrupted turn continuation without synthetic continue（#5030）

#5030 面向 SDK / stream-json callers，和 daemon `load`/`resume` 是不同层次：它不是把 session 拉起来，而是在已恢复的 session 上继续“上一次没有完成的 assistant turn”，并且不向 transcript 塞入假的用户消息。

core 先从持久化 chat history 做纯分类：

| 分类 | 条件 / 行为 |
|---|---|
| `interrupted_prompt` | 历史里有孤立 user prompt、模型还没真正响应；续跑时用 Retry 语义重发该 user content，若没到达模型则 history strip/restore 保持不变。 |
| `interrupted_turn` | assistant turn 留下 dangling tool calls；续跑前合成 error tool result 关闭这些 tool calls，再进入正常 turn loop。 |
| `none` | 历史看起来已经干净完成；幂等 no-op，不产生新 turn。 |

暴露面：

- TypeScript SDK：`Query.continueLastTurn()`；
- stream-json：`continue_last_turn` control request；
- non-interactive session：把 continuation 排进现有 work queue，复用普通 prompt 的 streaming updates、tool execution、file-history snapshots 和 terminal results；
- duplicate continue：如果已有 continue pending/running，会拒绝重复请求；
- continuation failure：输出 terminal error result，而不是半途无终态。

这个 PR 的关键约束是 transcript fidelity：最终 transcript 只保留用户真实说过的原始 prompt，不出现合成 `"continue"`。边界也明确：如果模型文本尾部被截断但历史里没有持久化 stop metadata 或 provider prefill 支持，当前 classifier 仍会把它视为 clean `none`。

---

## 8. 关键流程（时序图 / 调用链）

### 8.1 背景 agent pause → resume（transcript fork + 无消息丢失窗口）
...
### 8.2 daemon 客户端 load vs resume 两条路径 + 并发 restore 守卫
...
1. **transcript-first / persisted-first 恢复（多层共用）。** 不从「当前」父状态重推上下文，而是以落盘的不可变事实为权威源。
   ...
   - completed background agent：revive 仍从 persisted subagent transcript 恢复上下文，只追加新的 continuation instruction，并用 `resumeCount` 区分第几次续跑。
   - SDK / stream-json：`continueLastTurn()` 只根据 persisted chat history 分类 interrupted prompt / interrupted turn / none，不向 transcript 注入假的用户消息。
...
| #5030 | SDK / stream-json continuation | `Query.continueLastTurn()` / `continue_last_turn` 从 persisted history 分类 interrupted prompt/turn/none；续跑不注入合成 `"continue"` 用户消息 |
...
| #5972 | output-token display | subagent/background-agent card、panel、arena、WebShell/WebUI 统一显示 `outputTokens`，旧 payload fallback 到 `totalTokens` |
| #6015 | non-VP multi-agent scrollback | running subagents 只由 `LiveAgentPanel` 展示，inline panel 加高度窗口；mouse tracking 默认 VP-gated，保留非 VP 原生 scrollback |
...
8. **`continueLastTurn` 只能覆盖从历史可推断的中断（#5030）。** truncated assistant text tail 在没有 persisted stop metadata 或 provider prefill 的情况下仍会被判成 `none`；该 PR 也只覆盖 SDK / stdio stream-json surface，早期 ACP continuation surface 已移除。
...
### PR #5972/#6015 — subagent display and scrollback

- #5972：UI token display 从 cumulative API throughput 改为 output-only tokens，避免把 prompt/cached token 累计当成上下文窗口规模展示；WebShell/WebUI 使用 optional `outputTokens` + fallback 兼容旧事件。
- #6015：multi-agent live phase 去重 running-agent roster，避免 InlineParallelAgentsDisplay 和 LiveAgentPanel 重复撑高 Ink live frame；`useMouseEvents` VP-gate 防止非 VP 终端滚轮被 mouse tracking 劫持。

### PR #5030 — continue interrupted turn without synthetic user message

- `turn-interruption.ts`：从 persisted chat history 分类 `interrupted_prompt` / `interrupted_turn` / `none`，并构造 dangling tool call 的 synthetic error tool responses。
- `core/client.ts` / `geminiChat.ts`：continuation 复用 Retry send path，orphan prompt 没有真正到达模型时不会污染 history。
- `nonInteractive/session.ts` / control dispatcher：`continue_last_turn` 排入 session work queue，和普通 prompt 一样发 streaming updates / terminal result。
- `sdk-typescript/src/query/Query.ts`：暴露 `continueLastTurn()`，返回 accepted 状态和 interruption 分类；重复 pending/running continue 被拒绝。
