...
这些层面共享一个核心思想：**以落盘的不可变事实（transcript / 持久化会话）为权威源（source of truth）重建运行时上下文，而不是从「当前」父状态重新推导**。#6502 进一步补齐了损坏 transcript 的可见性：当 persisted session 的 `parentUuid` 链断裂时，交互式 `/resume` 和 ACP/Web Shell replay 只恢复可达 tail，但会显式标记 earlier history lost；它不会把 earlier island 拼回来，避免复活 `/rewind` 已删除内容。下文统一称为 transcript-first / persisted-first 恢复。
...
4. **断链只检测，不跨 island 恢复（#6502）。** `SessionService.reconstructHistory()` 看到缺失 parentUuid 时仍保持旧的 tail-only 截断行为，但返回 `historyGaps` 让有 UI 的 surface 展示 divider。读侧无法区分真实存储损坏和丢失的 `/rewind` marker，因此不能把 earlier island 拼回；background-agent resume 没有可见 marker surface，也不做 half-detection。

5. **非信任目录降级 yolo/auto。** `reconcileResumedApprovalMode` 在 `!isTrustedFolder()` 时把持久化的 `auto-edit/auto/yolo` 强制降到父模式或 `default`，防止「信任时启动 → 撤销信任后恢复」绕过当前信任策略（review [Critical]）。与之配套，恢复路径**总是** wrap override Config（即便 mode 不变），以隔离 `FileReadCache`/ToolRegistry，并在 finally 释放，避免监听器跨 resume 泄漏。
...
| #6502 | broken history gap marker | `buildOrderedUuidChain()` detect-only 缺口，`historyGaps` 透传到 `/resume` 与 ACP replay，显示不可恢复的 history gap |
...
### PR #6502 — broken parentUuid chain gap marker

- `conversation-chain.ts:buildOrderedUuidChain`：共享 parentUuid 链回溯逻辑；缺失 parent 时保持 tail-only 截断，并在 `detectGaps` 打开时返回 `HistoryGap`。
- `sessionService.ts:reconstructHistory`：交互恢复路径返回 `historyGaps`，不拼回 earlier island。
- `history-gap-notice.ts`：本地化 notice formatter + `indexGapsByChild` helper，供终端 `/resume` 与 ACP replay 共用。
- `resumeHistoryUtils.ts`：在 gap child 前插入 info divider，并清空 pending `@` 命令状态，避免 pre-gap `@file` 配给 post-gap 用户消息。
- `HistoryReplayer.ts`：ACP/Web Shell replay 在 gap child 前发 agent message notice。
- `background-agent-resume.ts`：保持不检测 gap；该非交互路径没有可展示 marker 的 surface。
