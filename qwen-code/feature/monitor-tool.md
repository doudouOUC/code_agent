...
> 涉及 PR：#3684（主工具）、#3726（`Monitor(...)` 权限命名空间）、#3792（合并后修复 + UI 路由）、#3933（subagent 通知路由修复）、#5165（通知 batch drain 降 token waste）、#5918（foreground shell near-timeout warning）
...
#5918 补的是相邻的前台 shell UX：当交互式前台命令 timeout 大于 15 秒并进入最后 15 秒时，live output 显示“即将超时，可按 Ctrl+B 提升为后台任务”的提示。它不改变 monitor，也不改变 shell final output / `llmContent` / 默认 timeout；作用是让已有 Ctrl+B promote 路径在真正快超时时可被发现。
...
| **#5918** | foreground shell near-timeout warning | 交互式前台 shell 在最后 15 秒显示 Ctrl+B promote 提示；仅 live display，且只在 interactive + promote callback 已接入时启用。 |
...
### PR #5918 — foreground shell near-timeout warning

- `shell.ts`：当 foreground command timeout 超过 15s 且进入最后 15s，live output 注入 near-timeout warning，提示 Ctrl+B 可 promote 到后台任务。
- `coreToolScheduler.ts` / TUI wiring：只在 interactive session 且 promote callback 已接入时 arm warning；headless/core caller 不显示误导性 Ctrl+B 文案。
- 边界：final shell output、模型看到的 `llmContent`、timeout 默认值和既有 Ctrl+B promote 行为不变；早完成、无 promote callback、abort-before-handle 路径都会取消/不 arm warning。
