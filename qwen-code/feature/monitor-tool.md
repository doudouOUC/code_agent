...
> 涉及 PR：#3684（主工具）、#3726（`Monitor(...)` 权限命名空间）、#3792（合并后修复 + UI 路由）、#3933（subagent 通知路由修复）
...
`monitor` 工具填补的正是这块空白：它 spawn 一个长时运行进程，把它的 stdout/stderr **逐行**转成事件通知（event notification）流式推回给 agent，agent 在 idle 时逐条消费，从而实现「一边监控、一边干别的活」。这是后台任务管理 roadmap（#3634）的 **Phase C**，由 #3666 跟踪。
...
| 输出投递 | turn 结束一次性返回 | `<task-notification>` 增量通知，idle 时消费 |
...
flowchart TB
    ...
    subgraph Consume["通知消费层"]
        ...
        UI["交互 UI · useGeminiStream<br/>通知队列 idle 消费"]
...
- **顶层 monitor**：无 `ownerAgentId`，走全局 `notificationCallback`，由 UI 层注册：交互模式 `useGeminiStream.ts:2570` 推入通知队列、idle 时逐条消费；headless stream-json `nonInteractive/session.ts:197` 转成 SDK 事件；one-shot `nonInteractiveCli.ts:472`。
