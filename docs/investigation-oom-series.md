...
1. **假设验证 repro**（开发者侧）：[`repros/oom-streaming-leak/`](../repros/oom-streaming-leak/README.md) —— 证明这种 pattern 确实泄漏
2. **用户自助诊断**：[`repros/oom-user-diagnostic/`](../repros/oom-user-diagnostic/README.md) —— 用户在自己的 qwen 会话里收集数据，判断 OOM 属于哪类，是否升级 v0.16.0 能修
...
| 2026-05-23 | 仓库内增加 [`repros/oom-streaming-leak/`](../repros/oom-streaming-leak/README.md) 本地复现 — V3 实测 OpenAI SDK 5.11.0 + mock SSE 服务器，确认 ~195 KB/iter RSS 累积 |
| 2026-05-23 | 仓库内增加 [`repros/oom-user-diagnostic/`](../repros/oom-user-diagnostic/README.md) 用户自助诊断工具 — monitor.cjs（NODE_OPTIONS 注入采样） + analyze.cjs（启发式分类），让用户在自己的 OOM 会话上确定属于哪类 |
